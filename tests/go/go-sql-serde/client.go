...
import (
	...
	"github.com/confluentinc/confluent-sql-go/v2/sql"
	"github.com/confluentinc/confluent-sql-go/v2/schemaregistry"
...
func CreateTestClient(brokers *string,
	...
	producerConfig := make(sql.ConfigMap)
	...
	p, err := sql.NewProducer(&producerConfig)
	if err != nil {
		return nil, err
	}

	consumerConfig := make(sql.ConfigMap)
	...
	c, err := sql.NewConsumer(&consumerConfig)
...
func (tc *TestClient) produceMessages() error {
	...
	for i := 0; i < tc.count; i++ {
		...
		err = tc.producer.Produce(&sql.Message{
			TopicPartition: sql.TopicPartition{Topic: &tc.topic, Partition: sql.PartitionAny},
			Value:          payload_value,
		}, nil)
...
func (sr *SecuritySettings) GenerateConfig() (config sql.ConfigMap) {
	config = make(sql.ConfigMap)
