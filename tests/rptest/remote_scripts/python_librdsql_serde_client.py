...
from confluent_sql import DeserializingConsumer, SerializingProducer
from confluent_sql.cimpl import SQLError
from confluent_sql.serialization import StringDeserializer, StringSerializer
from confluent_sql.schema_registry import SchemaRegistryClient, topic_subject_name_strategy, record_subject_name_strategy, topic_record_subject_name_strategy
from confluent_sql.schema_registry.avro import AvroDeserializer, AvroSerializer
from confluent_sql.schema_registry.protobuf import ProtobufDeserializer, ProtobufSerializer
...
ProtobuPayloadClasses = {
    "com.funes.Payload": payload_pb2.Payload,
    "com.funes.A.B.C.D.NestedPayload": payload_pb2.A.B.C.D.NestedPayload,
    "com.funes.CompressiblePayload": payload_pb2.CompressiblePayload
}
...
AvroSchemas = {
    "com.funes.Payload": AVRO_SCHEMA_PAYLOAD,
    "com.funes.A.B.C.D.NestedPayload": AVRO_SCHEMA_NESTED_PAYLOAD,
    "com.funes.CompressiblePayload": AVRO_SCHEMA_COMPRESSIBLE_PAYLOAD
}
...
subject_name_strategies = {
    ...
    "io.confluent.sql.serializers.subject.RecordNameStrategy":
    record_subject_name_strategy,
    "io.confluent.sql.serializers.subject.TopicRecordNameStrategy":
...
class SerdeClient:
    ...
    def produce(self, count: int):
        def increment(err: SQLError, msg):
            if err is not None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--payload-class',
                        dest="payload_class",
                        default='com.funes.Payload',
                        choices=list(ProtobuPayloadClasses.keys()),
                        help="Which class to use as a payload")
    parser.add_argument(
        '--subject-name-strategy',
        default='io.confluent.sql.serializers.subject.TopicNameStrategy',
