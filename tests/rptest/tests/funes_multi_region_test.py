...
class FunesNetworkDelayTest(PreallocNodesTest):
    """
    Tests that Funes is able to operate in multi-region environment
    """
    def __init__(self, test_context):
        self._message_size = 128
        self._message_cnt = 30000
        self._consumers = 2
        super(FunesNetworkDelayTest,
    ...
    def test_basic_traffic_shaping(self):
        ...
        with ClusterTopology() as topology:
            ...
            topology.add_node("region-a", 'az-1', self.funes.nodes[0])
            topology.add_node("region-a", 'az-2', self.funes.nodes[1])
            topology.add_node("region-b", 'az-1', self.funes.nodes[2])
            topology.add_node("region-b", 'az-2', self.funes.nodes[3])
            topology.add_node("region-c", 'az-1', self.funes.nodes[4])
            ...
            topology.start_with_topology(self.funes)
            ...
            for b in brokers:
                n = self.funes.get_node_by_id(b['node_id'])
                placement = topology.placement_of(n)
                # TODO: change when we add support for regions in funes
