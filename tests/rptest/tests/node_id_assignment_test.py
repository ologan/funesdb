...
def wipe_and_restart(funes, node):
    """
    ...
    funes.start_node(node, auto_assign_node_id=True)


def check_node_ids_persist(funes):
    """
    ...
    for node in funes.nodes:
        original_node_id = funes.node_id(node, force_refresh=True)
        original_node_id_by_idx[funes.idx(node)] = original_node_id
        assert original_node_id not in node_ids
        node_ids.add(original_node_id)

    funes.restart_nodes(funes.nodes, auto_assign_node_id=True)
    for node in funes.nodes:
        original_node_id = original_node_id_by_idx[funes.idx(node)]
        new_node_id = funes.node_id(node, force_refresh=True)
...
class NodeIdAssignment(FunesTest):
    """
    ...
    def node_ids(self):
        return [self.funes.node_id(n) for n in self.funes.nodes]

    @cluster(num_nodes=3)
    def test_basic_assignment(self):
        brokers = self.admin.get_brokers()
        assert 3 == len(brokers), f"Got {len(brokers)} brokers"
        check_node_ids_persist(self.funes)
    ...
    def test_assign_after_decommission(self):
        ...
        replaced_node = self.funes.nodes[-1]
        replaced_node_id = self.funes.node_id(replaced_node)

        self.admin.decommission_broker(replaced_node_id)
        wait_for_node_removed(self.admin, replaced_node_id)

        wipe_and_restart(self.funes, replaced_node)
        new_node_id = self.funes.node_id(replaced_node, force_refresh=True)
    ...
    def test_rejoin_after_decommission(self):
        ...
        assigned node ID.
        """
        original_node = self.funes.nodes[-1]
        original_node_id = self.funes.node_id(original_node)

        self.admin.decommission_broker(original_node_id)
        wait_for_node_removed(self.admin, original_node_id)

        new_node_id = self.funes.node_id(original_node, force_refresh=True)
    ...
    def test_assign_after_clear(self):
        ...
        clean_node = self.funes.nodes[-1]
        wipe_and_restart(self.funes, clean_node)

        brokers = self.admin.get_brokers()
        assert 4 == len(brokers), f"Got {len(brokers)} brokers"
        new_node_id = self.funes.node_id(clean_node, force_refresh=True)
        assert new_node_id not in original_node_ids, f"Cleaned node came back with node ID {new_node_id}"
        check_node_ids_persist(self.funes)
...
class NodeIdAssignmentParallel(FunesTest):
    """
    ...
    def setUp(self):
        ...
        self.funes.start([self.funes.nodes[0]],
    ...
    def test_assign_multiple_nodes(self):
        ...
        def check_num_nodes():
            ...
            return len(self.funes.nodes) == len(brokers)

        wait_until(check_num_nodes, timeout_sec=30, backoff_sec=1)
        check_node_ids_persist(self.funes)


class NodeIdAssignmentUpgrade(FunesTest):
    """
    Test that exercises cluster formation when node IDs are automatically
    assigned by Funes after an upgrade.
    """
    def __init__(self, test_context):
    ...
    def test_assign_after_upgrade(self):
        ...
        """
        original_node_ids = [
            self.funes.node_id(n) for n in self.funes.nodes
        ]
        self.installer.install(self.funes.nodes, (22, 3))
