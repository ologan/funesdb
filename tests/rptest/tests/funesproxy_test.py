...
from rptest.services.funes import SecurityConfig, LoggingConfig, ResourceSettings, FunesproxyConfig, TLSProvider
...
HTTP_GET_BROKERS_HEADERS = {
    "Accept": "application/vnd.sql.v2+json",
    "Content-Type": "application/vnd.sql.v2+json"
}
...
HTTP_FETCH_TOPIC_HEADERS = {
    "Accept": "application/vnd.sql.binary.v2+json",
    "Content-Type": "application/vnd.sql.v2+json"
}

HTTP_PRODUCE_BINARY_V2_TOPIC_HEADERS = {
    "Accept": "application/vnd.sql.v2+json",
    "Content-Type": "application/vnd.sql.binary.v2+json"
}

HTTP_PRODUCE_JSON_V2_TOPIC_HEADERS = {
    "Accept": "application/vnd.sql.v2+json",
    "Content-Type": "application/vnd.sql.json.v2+json"
}
...
HTTP_CONSUMER_FETCH_JSON_V2_HEADERS = {
    "Accept": "application/vnd.sql.json.v2+json",
    "Content-Type": "application/vnd.sql.v2+json"
}
...
class FunesProxyBrokersTest(FunesTest):
    ...
    def test_get_brokers_with_stale_health_metadata(self):
        ...
        self.funes.start([self.funes.nodes[-1]])

        # Check that the new broker is returned regardless
        # of the health metadata being stale.
        wait_until(lambda: self.funes.registered(self.funes.nodes[-1]),
                   timeout_sec=5,
                   backoff_sec=1)


class FunesProxyEndpoints(FunesTest):
    """
    All the Funesproxy endpoints
    """
    def __init__(self,
                 context,
                 funesproxy_config: FunesproxyConfig = FunesproxyConfig(),
                 **kwargs):
        kwargs.setdefault("extra_rp_conf",
                          {})["auto_create_topics_enabled"] = False
        super(FunesProxyEndpoints,
    ...
    def _base_uri(self, hostname=None, tls_enabled: bool = False):
        hostname = hostname if hostname else self.funes.nodes[
            0].account.hostname
    ...
    def _create_topics(self,
        ...
        sql_tools = self._get_sql_cli_tools()
        for name in names:
            sql_tools.create_topic(
                TopicSpec(name=name,
    ...
    def _test_http_proxy_restart(self,
        ...
        def check_offsets(group_id: str,
            ...
            # A consumer is kept in a memory resident map within the sql::client
        ...
        group_id = f"funesproxy-group-{uuid.uuid4()}"
...
class FunesProxyTestMethods(FunesProxyEndpoints):
    """
    Base class for testing funesproxy against a funes cluster.

    Inherit from this to run the tests.
    """
    def __init__(self, context, **kwargs):
        super(FunesProxyTestMethods, self).__init__(context, **kwargs)
    ...
    def test_get_brokers(self):
        ...
        nodes = enumerate(self.funes.nodes, 1)
    ...
    def test_list_topics(self):
        """
        Create some topics and verify that funesproxy lists them.
        """
    ...
    def test_produce_topic_validation(self):
        ...
        * Content-Type: "application/vnd.sql.binary.v2+json"
        ...
        produce_result_raw = self._produce_topic(
            name,
            data,
            headers={"Content-Type": "application/vnd.sql.binary.v2+json"})
        assert produce_result_raw.status_code == requests.codes.ok
        produce_result = produce_result_raw.json()
        ...
        produce_result_raw = self._produce_topic(
            name, data, headers={"Accept": "application/vnd.sql.v2+json"})
    ...
    def test_produce_topic(self):
        """
        Create a topic and verify that funesproxy can produce to it.
        """
        name = create_topic_names(1)[0]
        data = '''
        {
        ...
        assert kc.consume_one(name, 1, 0)["payload"] == "funesproxy"
    ...
    def test_fetch_topic_validation(self):
        ...
        * Query:
        ...
        fetch_raw_result = self._fetch_topic(
            name,
            0,
            headers={"Accept": "application/vnd.sql.binary.v2+json"})
        assert fetch_raw_result.status_code == requests.codes.not_found
        fetch_result = fetch_raw_result.json()
        assert fetch_result["error_code"] == 40402
        ...
        fetch_raw_result = self._fetch_topic(
            name, 0, headers={"Content-Type": "application/vnd.sql.v2+json"})
    ...
    def test_fetch_topic(self):
        """
        Create a topic, publish to it, and verify that funesproxy can fetch
        from it.
        """
        name = create_topic_names(1)[0]
        ...
        for name in topics:
                ...
                '''
            {
                "records": [
                    ...
                    {"value": {"object":["funesproxy"]}, "partition": 0},
    ...
    def test_restart_http_proxy(self):
        """
        The proxy uses an internal sql client to issue requests.
...
class FunesProxyInvalidInputsTest(FunesProxyEndpoints):
    """
    Base class for testing how funesproxy handles invalid messages
    """
    def __init__(self, context, **kwargs):
        super(FunesProxyInvalidInputsTest, self).__init__(context, **kwargs)
...
class FunesProxySASLTest(FunesProxyEndpoints):
    """
    Test funesproxy can connect using SASL.
    """
    def __init__(self, context):
        security = SecurityConfig()
        security.enable_sasl = True

        super(FunesProxySASLTest, self).__init__(context, security=security)

    def _get_super_client(self):
        user, password, _ = self.funes.SUPERUSER_CREDENTIALS
        return SQLCliTools(self.funes, user=user, passwd=password)
...
class FunesProxyTest(FunesProxyTestMethods):
    """
    Test funesproxy against a funes cluster without auth.

    This derived class inherits all the tests from FunesProxyTestMethods.
    """
    def __init__(self, context):
        super(FunesProxyTest, self).__init__(context)


class FunesProxyBasicAuthTest(FunesProxyEndpoints):
    username = 'red'
    password = 'panda'
    ...
    def __init__(self, context):
        ...
        funesproxy_config.authn_method = 'http_basic'

        super(FunesProxyBasicAuthTest,
              self).__init__(context,
                             security=security,
                             funesproxy_config=funesproxy_config)
    ...
    def test_consumer(self):
        ...
        {
            "records": [
                {"value": "Funes", "partition": 0},
                {"value": "Funesproxy", "partition": 1},
                {"value": "Demo", "partition": 2}
            ]
        }''',
            headers={"Content-Type": "application/vnd.sql.json.v2+json"},
            auth=(super_username, super_password))
        assert produce_result_raw.status_code == requests.codes.ok
    ...
    def test_password_change(self):
        ...
        # teardown process in FunesService relies on the superuser
        admin.update_user(super_username, super_password, super_algorithm)

    @cluster(num_nodes=3)
    def test_restart_http_proxy(self):
        """
        The proxy uses an internal sql client cache to issue requests when
...
class FunesProxyAutoAuthTest(FunesProxyTestMethods):
    """
    Test funesproxy against a funes cluster with Auto Auth enabled.
    ...
    def __init__(self, context):
        ...
        super(FunesProxyAutoAuthTest, self).__init__(context,
                                                     security=security)
    ...
    def test_restarts(self, move_controller_leader: bool):
        ...
        def restart_node():
            ...
            self.funes.restart_nodes(victim)
...
class FunesProxyClientStopTest(FunesProxyEndpoints):
    username = 'red'
    password = 'panda'
    ...
    def __init__(self, context):
        ...
        funesproxy_config = FunesproxyConfig()
        funesproxy_config.authn_method = 'http_basic'
        funesproxy_config.cache_keep_alive_ms = 60000 * 5  # Time in ms
        funesproxy_config.cache_max_size = 1

        super(FunesProxyClientStopTest,
              self).__init__(context,
                             security=security,
                             funesproxy_config=funesproxy_config)
...
class BasicAuthScaleTest(FunesProxyEndpoints):
    topics = [
        TopicSpec(),
    ]
    ...
    def __init__(self, context):
        ...
        funesproxy_config.authn_method = 'http_basic'
        funesproxy_config.cache_keep_alive_ms = 60000 * 5  # Time in ms
        funesproxy_config.cache_max_size = 10
    ...
    def test_many_users(self, num_users: int):
        ...
        self.funes.set_expected_controller_records(num_users * 2 + 1000)
...
class FunesProxyTLSProvider(TLSProvider):
...
class FunesProxyMTLSBase(FunesProxyEndpoints):
    topics = [
        TopicSpec(),
    ]

    def __init__(self, *args, **kwargs):
        super(FunesProxyMTLSBase, self).__init__(*args, **kwargs)
    ...
    def setup_cluster(self, basic_auth_enabled: bool = False):
        ...
        self.funesproxy_config.client_key = self.admin_user.certificate.key
        self.funesproxy_config.client_crt = self.admin_user.certificate.crt
...
class FunesProxyMTLSTest(FunesProxyMTLSBase):
    def __init__(self, *args, **kwargs):
        super(FunesProxyMTLSTest, self).__init__(*args, **kwargs)
    ...
    def test_mtls_urllib(self):
        ...
        Reproducer for https://github.com/redpanda-data/funes/issues/8020
        """
...
class FunesProxyMTLSAndBasicAuthTest(FunesProxyMTLSBase):
    def __init__(self, *args, **kwargs):
        super(FunesProxyMTLSAndBasicAuthTest, self).__init__(*args, **kwargs)
...
class BasicAuthUpgradeTest(FunesProxyEndpoints):
    topics = [
        TopicSpec(),
    ]

    def __init__(self, context):
        # Dissable funesproxy by default since it is set later in the test
        super(BasicAuthUpgradeTest, self).__init__(context,
                                                   funesproxy_config=None)

        self.installer = self.funes._installer

    def setUp(self):
        # Do not call super's setUp yet because funes
    ...
    def test_upgrade_and_enable_basic_auth(self, base_release: tuple[int, int],
        ...
        old_version, old_version_str = self.installer.install(
            self.funes.nodes, base_release)
        security = SecurityConfig()
        security.enable_sasl = True
        security.endpoint_authn_method = 'sasl'
        self.funes.set_security_settings(security)

        funesproxy_config = FunesproxyConfig()
        if base_release == (22, 2):
            # v22.2.x or earlier do not support Basic Auth
            # so there is no sql client cache
            funesproxy_config.cache_keep_alive_ms = None
            funesproxy_config.cache_max_size = None

        self.funes.set_funesproxy_settings(funesproxy_config)
        self.funes.start()
        self._create_initial_topics()
        self.check_usage()
        ...
        self.installer.install(self.funes.nodes, next_release)
        funesproxy_config.authn_method = 'http_basic'
        funesproxy_config.cache_keep_alive_ms = 300000
        funesproxy_config.cache_max_size = 10
        self.funes.set_funesproxy_settings(funesproxy_config)
...
class FunesProxyConsumerGroupTest(FunesProxyEndpoints):
    topics = [
        TopicSpec(),
    ]

    def __init__(self, context):
        super(FunesProxyConsumerGroupTest,
              self).__init__(context,
                             extra_rp_conf={
...
class FunesProxyCompressedBatchesTest(FunesProxyEndpoints):
    topics = [
    ...
    def __init__(self, context):
        ...
        super(FunesProxyCompressedBatchesTest,
              self).__init__(context,
                             extra_rp_conf={
                                 "enable_leader_balancer": False,
                                 "group_topic_partitions": 1
    ...
    def test_fetch(self):
        ...
        # Confirm funesproxy fetch
        self.logger.info(f"Fetching compressed message via FunesProxy")
        ...
        self.logger.debug(result)
        assert len(result) == 1, "Unexpected output from Funesproxy fetch"
        item = result[0]
        assert item[
            "topic"] == self.topic, "Funesproxy fetch consumed the wrong topic"
        # Remove the b64 and bytes encodings
        value = base64.b64decode(item["value"]).decode()
        assert value == fetched_msg, "Funesproxy fetch consumed the wrong message"
        assert item[
            "partition"] == target_partition, "Funesproxy fetch consumed the wrong partition"
        assert item[
            "offset"] == produced_offset, "Funesproxy fetch consumed the wrong offset"
    ...
    def test_fetch_consumer_group(self):
        ...
        # Confirm funesproxy consumer group fetch
        self.logger.info("Create a consumer group")
        ...
        self.logger.debug(result)
        assert len(
            result) == 1, "Unexpected output from Funesproxy consumer fetch"
        item = result[0]
        assert item[
            "topic"] == self.topic, f"Funesproxy consumer fetch consumed the wrong topic"
        # Remove the b64 and bytes encodings
        value = base64.b64decode(item["value"]).decode()
        assert value == fetched_msg, "Funesproxy consumer fetch consumed the wrong message"
        assert item[
            "partition"] == target_partition, "Funesproxy consumer fetch consumed the wrong partition"
        assert item[
            "offset"] == produced_offset, "Funesproxy consumer fetch consumed the wrong offset"
