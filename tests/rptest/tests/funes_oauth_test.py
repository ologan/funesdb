...
from rptest.services.funes import LoggingConfig, FunesproxyConfig, SchemaRegistryConfig, SecurityConfig, make_funes_service
...
class FunesOIDCTestBase(Test):
    """
    Base class for tests that use the Funes service with OIDC
    """
    def __init__(self,
                 test_context,
                 num_nodes=4,
                 ...
                 **kwargs):
        super(FunesOIDCTestBase, self).__init__(test_context, **kwargs)
        ...
        self.funes = make_funes_service(
            ...
            extra_rp_conf={
                ...
                "sql_sasl_max_reauth_ms": sasl_max_reauth_ms,
            },
            security=security,
...
class FunesOIDCTest(FunesOIDCTestBase):
    @cluster(num_nodes=4)
    def test_init(self):
        kc_node = self.keycloak.nodes[0]
        ...
        # Explicit poll triggers OIDC token flow. Required for librdsql
        ...
        def check_sr_subjects():
            response = requests.get(
                url=
                f'http://{self.funes.nodes[0].account.hostname}:8081/subjects',
                headers={
...
class OIDCReauthTest(FunesOIDCTestBase):
    MAX_REAUTH_MS = 8000
