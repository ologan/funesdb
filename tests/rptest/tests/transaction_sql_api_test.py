...
class TxSQLApiTest(FunesTest):
    ...
    def __init__(self, test_context):
        ...
        self.sql_cli = SQLCliTools(self.funes, "3.0.0")
    ...
    def test_describe_producers(self):
        ...
        for topic in self.topics:
            for partition in range(topic.partition_count):
                ...
                txs_info = self.sql_cli.describe_producers(
    ...
    def test_last_timestamp_of_describe_producers(self):
        ...
        for topic in self.topics:
            for partition in range(topic.partition_count):
                producers = self.sql_cli.describe_producers(
                    topic.name, partition)
                self.funes.logger.debug(json.dumps(producers))
    ...
    def test_describe_transactions(self):
        ...
        tx = self.sql_cli.describe_transaction(tx_id)
    ...
    def test_empty_list_transactions(self):
        txs_info = self.sql_cli.list_transactions()
    ...
    def test_list_transactions(self):
        ...
        for tx_from_list in txs_info:
            tx_from_describe = self.sql_cli.describe_transaction(
