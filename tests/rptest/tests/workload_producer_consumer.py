...
class ProducerConsumerWorkload(PWorkload):
    ...
    def __init__(self, ctx: PreallocNodesTest) -> None:
        self.ctx = ctx
        self.rpk = RpkTool(self.ctx.funes)
        self.last_uploaded_sql_offset = 0
    ...
    def begin(self):
        ...
        assert topic_cfg["funes.remote.read"].value == "true" and topic_cfg[
            "funes.remote.write"].value == "true"
    ...
    def on_cluster_upgraded(self, version: tuple[int, int, int]) -> int:
        """
        query remote storage and compute uploaded_sql_offset, to check that progress is made
        """
        major_version = version[0:2]
        quiesce_uploads(self.ctx.funes, [self.topic.name], 120)
        ...
        bucket = BucketView(self.ctx.funes)
        ...
        # retrieve highest committed sql offset and check that progress is made
        ...
        if uploaded_sql_offset > self.last_uploaded_sql_offset:
            self.last_uploaded_sql_offset = uploaded_sql_offset
