...
# TODO: fix https://github.com/redpanda-data/funes/issues/5629
ALLOWED_LOGS = [
    ...
    # (https://github.com/redpanda-data/funes/issues/5886)
    re.compile("(sql|rpc) - .*std::exception"),

    #  <= 22.2 versions may log bare seastar::condition_variable_timed_out error
    re.compile(
        "(sql|rpc) - Service handler threw an exception: seastar::condition_variable_timed_out"
    ),

    # < 22.2 versions may log a "cannot find consensus group" error message
    # (https://github.com/redpanda-data/funes/pull/5742)
...
class ControllerUpgradeTest(EndToEndTest):
    ...
    def test_updating_cluster_when_executing_operations(self):
        ...
        self.funes = make_funes_service(self.test_context, 5)
        ...
        if prev_version[0] == 22 and prev_version[1] == 2:
            admin_operations = [
                o for o in FunesAdminOperation
                if o != FunesAdminOperation.UPDATE_TOPIC
            ]
        else:
            admin_operations = [o for o in FunesAdminOperation]

        # DeleteRecords is not an allowable admin operation until the cluster
        # has been fully upgraded
        admin_operations = [
            o for o in admin_operations
            if o != FunesAdminOperation.DELETE_RECORDS
        ]

        installer.install(self.funes.nodes, prev_version)

        self.funes.start()
        admin_fuzz = AdminOperationsFuzzer(self.funes,
        ...
        # reset to use latest Funes version
        ...
        for n in self.funes.nodes:
            ...
            self.funes.restart_nodes([n], stop_timeout=60, start_timeout=90)
