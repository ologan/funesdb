...
class LeadersInfoApiTest(FunesTest):
    ...
    def reset_leaders_info_test(self):
        def check_reset_leaders():
            ...
            for partition in range(self.topics[0].partition_count):
                leader = self.admin.get_partition_leader(namespace="sql",
                                                         topic=self.topics[0],
                                                         partition=partition,
                                                         node=node)
                if leader == -1:
    ...
    def get_leaders_info_test(self):
        ...
        def check_get_leaders():
            ...
            leaders_node1 = self.admin.get_leaders_info(self.funes.nodes[0])
            leaders_node1.sort(key=compare_key)
            leaders_node2 = self.admin.get_leaders_info(self.funes.nodes[1])
