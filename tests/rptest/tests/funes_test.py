...
from rptest.services.funes import CloudTierName, make_funes_service, CloudStorageType
...
class FunesTest(Test):
    """
    Base class for tests that use the Funes service.
    """
    ...
    def __init__(self,
        ...
        FunesService constructor.
        """
        super(FunesTest, self).__init__(test_context)
    ...
    def cloud_storage_client(self):
        return self.funes.cloud_storage_client
    ...
    def _create_initial_topics(self):
        config = self.funes.security_config()
    ...
    def load_version_range(self, initial_version: FunesVersion):
        """
        ...
        k = 0
        v = self.funes._installer.head_version()
        versions = [v]
        while (v[0], v[1]) != initial_version[0:2]:
            k += 1

            v = self.funes._installer.highest_from_prior_feature_version(v)
            versions.insert(0, v)
    ...
    def upgrade_through_versions(
        self,
        versions_in: list[FunesVersion],
        already_running: bool = False,
        auto_assign_node_id: bool = False,
        mid_upgrade_check: Callable[[dict[Any, FunesVersion]],
                                    None] = lambda x: None):
        """
        ...
        # replace all the instances of FunesInstaller.HEAD with the head_version
        versions: list[FunesVersionTriple | FunesVersionLine] = [
            v if v != FunesInstaller.HEAD else
            self.funes._installer.head_version() for v in versions_in
        ]

        def install_next():
            v = versions.pop(0)
            self.logger.info(f"Installing version {v}...")
            self.funes._installer.install(self.funes.nodes, v)
            return v
        ...
        def logical_version_stable(old_logical_version: int):
            ...
            for node in self.funes.nodes:
                features = self.funes._admin.get_features(node=node)

                if 'node_latest_version' in features:
                    # Only Funes >= v23.2 has this field
        ...
        def await_consumer_offsets():
            ...
            self.funes.wait_until(_consumer_offsets_present,
                                     timeout_sec=90,
                                     backoff_sec=3)

        if already_running:
            old_logical_version:int =  \
                self.funes._admin.get_features()['cluster_version']
        else:
        ...
        if not already_running:
            ...
            FunesTest.setUp(self)
            yield current_version
        else:
            old_version = self.funes.get_version_int_tuple(
                self.funes.nodes[0])
        ...
        while len(versions) != 0:
            ...
            rp_nodes: list[Any] = self.funes.nodes
            ...
            old_logical_version = self.funes._admin.get_features(
            )['cluster_version']
