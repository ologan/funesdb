...
from rptest.services.funes import FunesService, FunesproxyConfig
...
class ConfigurationUpdateTest(FunesTest):
    """
    Updates RPC port on two nodes.
    """
    @cluster(num_nodes=3)
    def test_two_nodes_update(self):
        node_1 = self.funes.get_node(1)
        node_2 = self.funes.get_node(2)

        orig_partitions = self.funes.storage().partitions(
            "funes", "controller")
        # stop both nodes
        self.funes.stop_node(node_1)
        self.funes.stop_node(node_2)
        # change both ports
        ...
        # start both nodes
        self.funes.start_node(node_1, altered_port_cfg_1)
        self.funes.start_node(node_2, altered_port_cfg_2)
        ...
        def controller_log_replicated():
            ...
            for p in self.funes.storage().partitions(
                    "funes", "controller"):
    ...
    """
    Should allow to update port of advertised sql API on all of the nodes at once
    """

    @cluster(num_nodes=3)
    def test_update_advertised_sql_api_on_all_nodes(self):

        node_1 = self.funes.get_node(1)
        node_2 = self.funes.get_node(2)
        node_3 = self.funes.get_node(3)

        # stop all nodes
        self.funes.stop_node(node_1)
        self.funes.stop_node(node_2)
        self.funes.stop_node(node_3)

        def make_new_address(node, port):
            return dict(address=node.name, port=port)

        # change ports
        altered_cfg_1 = dict(sql_api=make_new_address(node_1, 10091),
                             advertised_sql_api=make_new_address(
                                 node_1, 10091))
        altered_cfg_2 = dict(sql_api=make_new_address(node_2, 10092),
                             advertised_sql_api=make_new_address(
                                 node_2, 10092))
        altered_cfg_3 = dict(sql_api=make_new_address(node_3, 10093),
                             advertised_sql_api=make_new_address(
                                 node_3, 10093))

        # start all
        self.funes.start_node(node_1, override_cfg_params=altered_cfg_1)
        self.funes.start_node(node_2, override_cfg_params=altered_cfg_2)
        self.funes.start_node(node_3, override_cfg_params=altered_cfg_3)

        def metadata_updated():
            brokers = self.client().brokers()
            self.funes.logger.debug(f"brokers metadata: {brokers}")
    ...
    def test_update_invalid_advertised_api(self):
        ...
        altered_cfg_1 = dict(sql_api=make_new_address(node_1, 10091),
                             advertised_sql_api=dict(address="0.0.0.0",
                                                       port=10091))
        ...
        altered_cfg_3 = FunesproxyConfig()
        altered_cfg_3.advertised_api_host = "0.0.0.0"

        self.funes.start_node(node_1,
                                 override_cfg_params=altered_cfg_1,
                                 expect_fail=True,
                                 timeout=10)

        n_err_logs = self.funes.count_log_node(
            node_1, "'advertised_sql_api' validation error")
        assert (n_err_logs == 1)

        self.funes.start_node(node_2,
        ...
        self.funes.set_funesproxy_settings(altered_cfg_3)

        self.funes.start_node(node_3, expect_fail=True, timeout=10)

        n_err_logs = self.funes.count_log_node(
            node_3, "'advertised_funesproxy_api' validation error")
        assert (n_err_logs == 1)

    @cluster(num_nodes=3)
    def test_updating_address_after_data_deletion(self):
        node = self.funes.get_node(2)
        ...
        # change rpc port & sql advertised address
        altered_port_cfg_1 = dict(
            rpc_server=dict(address="{}".format(node.name),
                            port=ALTERNATIVE_RPC_PORTS[1]),
            sql_api=make_new_address(node, 10091),
            advertised_sql_api=make_new_address(node, 10091))
        # remove node data folder
        node.account.remove(f"{FunesService.PERSISTENT_ROOT}/*")

        # start node
        self.funes.start_node(node, altered_port_cfg_1)
