...
class SaramaTest(FunesTest):
    ...
    def test_sarama_http_server(self):
        sarama_example = SaramaExamples.SaramaHttpServer(self.funes)
        example = ExampleRunner(self._ctx,
                                sarama_example,
                                timeout_sec=self._timeout)
        ...
        # funes is in the middle of a leadership election when
        # we try to http get.
        wait_until(try_curl,
                   timeout_sec=self._timeout,
                   backoff_sec=5,
    ...
    def test_sarama_consumergroup(self):
        ...
        def until_partitions():
            ...
            return len(list(storage.partitions("sql", self.topic))) == 3
...
class SaramaScramTest(FunesTest):
    """
    ...
    def test_sarama_sasl_scram(self):
        ...
        cmd = SaramaExamples.sarama_sasl_scram(self.funes, self.topic)
        node = random.choice(self.funes.nodes)

        def try_cmd():
            # Allow fail because the process exits with
            # non-zero if funes is in the middle of a
        ...
        # Using wait_until for auto-retry because sometimes
        # funes is in the middle of a leadership election
        wait_until(try_cmd,
                   timeout_sec=60,
                   backoff_sec=5,
