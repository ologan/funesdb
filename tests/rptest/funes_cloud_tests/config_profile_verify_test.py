...
class ConfigProfileVerifyTest(Test):
    ...
    def setUp(self):
        self.funes.start()
        self._ip = self._ipClient.getInstallPack(self.funes.get_version(0))
        self._clusterId = self.funes._cloud_cluster.config.id
        self._configProfile = self._ip['config_profiles'][
            self.funes._cloud_cluster.config.config_profile_name]

    @cluster(num_nodes=3, check_allowed_error_logs=False)
    def test_config_profile_verify(self):
        self.logger.debug("Here we go")

        # assert isinstance(self.funes, FunesServiceCloud)
    ...
    def _check_rp_config(self):
        confRes = self.funes._kubectl.exec(
            "rpk funes admin config print --host 0")
    ...
    def _check_gcp_nodes(self):
        cmd = self.funes._kubectl._ssh_prefix() + [
            'gcloud', 'compute', 'instances', 'list', '--filter',
            '"tags.items=funes-node tags.items=funes-{}"'.format(
        ...
        for n in resd:
            ...
            self.logger.debug(
                "asserting storage for funes node {}: expected: {}, actual: {}"
    ...
    def _check_aws_nodes(self):
        cmd = self.funes._kubectl._ssh_prefix() + [
            ...
            '--filters="Name=tag:Name, Values=funes-{}-rp"'.format(
                self._clusterId),
