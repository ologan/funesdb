...
from rptest.services.funes_cloud import CloudTierName, get_config_profile_name
from rptest.services.funes import (RESTART_LOG_ALLOW_LIST, MetricsEndpoint,
                                      SISettings, FunesServiceCloud)
...
from rptest.funes_cloud_tests.cloudv2_object_store_blocked import cloudv2_object_store_blocked
...
class HighThroughputTestTrafficGenerator:
    ...
    3x in some tiers.
    """
    def __init__(self, context, funes, topic, msg_size, asymetry=3):
        t = time.time()
        self._logger = funes.logger
...
@contextmanager
def traffic_generator(context, funes, expected_ingress_rate,
                      expected_egress_rate, *args, **kwargs):
    tgen = HighThroughputTestTrafficGenerator(context, funes, *args,
                                              **kwargs)
    tgen.start()
    try:
        yield tgen
    except:
        funes.logger.exception("Exception within traffic_generator method")
        raise
    finally:
        tgen.stop()
...
@contextmanager
def omb_runner(context, funes, driver, workload, omb_config):
    bench = OpenMessagingBenchmark(context, funes, driver,
                                   (workload, omb_config))
    ...
    try:
        ...
        funes.logger.exception("Exception within OMB")
        raise
    finally:
        bench.stop()
...
def get_cloud_globals(globals):
    _config = {}
    if FunesServiceCloud.GLOBAL_CLOUD_CLUSTER_CONFIG in globals:
        # Load needed config values from cloud section
        # of globals prior to actual cluster creation
        _config = globals[FunesServiceCloud.GLOBAL_CLOUD_CLUSTER_CONFIG]
    return _config
...
class HighThroughputTest(PreallocNodesTest):
    ...
    def __init__(self, test_ctx: TestContext, *args, **kwargs):
        ...
        install_pack_client = InstallPackClient(
            self.funes._cloud_cluster.config.install_pack_url_template,
            self.funes._cloud_cluster.config.install_pack_auth_type,
            self.funes._cloud_cluster.config.install_pack_auth)
        install_pack_version = self.funes._cloud_cluster.config.install_pack_ver
        ...
        self.logger.info(f"Loaded install pack '{install_pack['version']}': "
                         f"Funes v{install_pack['funes_version']}, "
        ...
        tier_product = self.funes._cloud_cluster.get_product()
        """
    ...
    def test_cluster_cleanup(self):
        ...
        self.funes._cloud_cluster.current.tests_finished = True
    ...
    def tearDown(self):
        ...
        if FunesServiceCloud.GLOBAL_CLOUD_CLUSTER_CONFIG in self._ctx.globals:
    ...
    def load_many_segments(self):
        ...
        try:
            ...
            wait_until(lambda: nodes_report_cloud_segments(
                self.funes, target_cloud_segments),
    ...
    def test_throughput_simple(self):
        ...
        # Generate traffic
        with traffic_generator(self.test_context, self.funes,
    ...
    def stage_block_node_traffic(self):
        ...
        wait_until(self.funes.healthy, timeout_sec=600, backoff_sec=1)
    ...
    def stage_stop_wait_start(self, forced_stop: bool, downtime: int):
        ...
        self.funes.start_node(node, timeout=600)
    ...
    def _cloud_storage_no_new_errors(self, funes, logger=None):
        num_errors = funes.metric_sum(
            "funes_cloud_storage_errors_total",
    ...
    def stage_block_s3(self):
        ...
        wait_until(lambda: nodes_report_cloud_segments(self.funes, 100),
                   timeout_sec=600,
                   backoff_sec=5)
        ...
        if self.config_profile_name == CloudTierName.DOCKER:
            with firewall_blocked(self.funes.nodes, self.s3_port):
                # wait for the first cloud related failure + one minute
                wait_until(lambda: not self._cloud_storage_no_new_errors(
                    self.funes, self.logger),
                           timeout_sec=600,
                           backoff_sec=10)
                time.sleep(60)
        else:
            with cloudv2_object_store_blocked(self.funes, self.logger):
                # wait for the first cloud related failure + one minute
                wait_until(lambda: not self._cloud_storage_no_new_errors(
                    self.funes, self.logger),
                           timeout_sec=600,
        ...
        wait_until(self.funes.healthy, timeout_sec=60, backoff_sec=1)
    ...
    def stage_decommission_and_add(self):
        ...
        def topic_partitions_on_node():
            try:
                parts = self.funes.partitions(self.topic)
    ...
    def _run_omb(self, produce_bps,
        ...
        benchmark = OpenMessagingBenchmark(
            self._ctx, self.funes, "SIMPLE_DRIVER",
    ...
    def stage_hard_restart(self, producer):
        # This stage force stops all Funes nodes. It then
        ...
        # hard stop all nodes
        self.logger.info("stopping all funes nodes")
        for node in self.funes.nodes:
            self.funes.stop_node(node, forced=True)

        # start all nodes again
        self.logger.info("starting all funes nodes")
        for node in self.funes.nodes:
            self.funes.start_node(node, timeout=600)

        # wait until the cluster is health once more
        self.logger.info("waiting for RP cluster to be healthy")
        wait_until(self.funes.healthy, timeout_sec=900, backoff_sec=1)
    ...
    def test_htt_partitions_omb(self, partitions):
        ...
        workload = self._prepare_omb_workload(rampup_time, runtime,
                                              ...
                                              producers, consumers)
        with omb_runner(
                self._ctx, self.funes, "SIMPLE_DRIVER", workload,
