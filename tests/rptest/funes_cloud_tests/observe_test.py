...
class HTObserveTest(Test):
    ...
    def __init__(self, test_context):
        ...
        self._token = self.funes._cloud_cluster.config.grafana_token
        self._endpoint = self.funes._cloud_cluster.config.grafana_alerts_url
        ...
        for group in rule_groups:
            for rule in group.rules:
                ...
                for alert in rule.alerts:
                    ...
                    if hasattr(
                            alert.labels, 'funes_agent'
                    ) and alert.labels.funes_agent == self._clusterId:
                        alerts.append(alert)

                    if hasattr(
                            alert.labels, 'funes_id'
                    ) and alert.labels.funes_id == self._clusterId:
                        alerts.append(alert)
