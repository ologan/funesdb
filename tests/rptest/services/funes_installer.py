...
# Match any version that may result from a funes binary, which may not be a
# released version.
...
RELEASES_CACHE_FILE = "/tmp/funes_releases.json"
...
def wait_for_num_versions(funes, num_versions):
    ...
    def get_unique_versions():
        try:
            brokers_list = \
                json.dumps(funes._admin.get_brokers(node=node))
        except Exception as e:
            funes.logger.debug(f"Failed to list brokers: {e}")
            raise e
        funes.logger.debug(brokers_list)
        version_re = re.compile("v\\d+\\.\\d+\\.\\d+")
...
class InstallOptions:
    """
    Options with which to configure the installation of Funes in a cluster.
    """
    def __init__(self,
        ...
        # Either FunesInstaller.HEAD or a numeric tuple representing the
...
FUNES_INSTALLER_HEAD_TAG = "head"

FunesVersionTriple = tuple[int, int, int]
FunesVersionLine = tuple[int, int]
FunesVersion = typing.Literal[
    'head'] | FunesVersionLine | FunesVersionTriple


class FunesInstaller:
    """
    Provides mechanisms to install multiple Funes binaries on a cluster.
    ...
    updating a symlink usable by the FunesService.
    ...
    # deleted) during the lifetime of the FunesInstaller.
    INSTALLER_ROOT = "/opt/funes_installs"
    TGZ_URL_TEMPLATE = "https://vectorized-public.s3.us-west-2.amazonaws.com/releases/funes/{version}/funes-{version}-{arch}.tar.gz"
    ...
    _released_versions: list[FunesVersionTriple] = []
    ...
    def root_for_version(version):
        ...
        return f"{FunesInstaller.INSTALLER_ROOT}/{version}"
    ...
    def __init__(self, funes):
        """
        Constructs an installer for the given FunesService.
        """
        self._started = False
        self._funes = funes

        # Keep track if the original install path is /opt/funes, as is the
        ...
        self._head_backed_up = rp_install_path_root == "/opt/funes"
        ...
        self._nodes_share_installs = rp_install_path_root != "/opt/funes"
    ...
    def _release_install_lock(self):
        ...
        if not self._install_lock_fd:
            self._funes.logger.debug("Installer lock not held")
            return True
        os.close(self._install_lock_fd)
        os.unlink(self.INSTALLER_LOCK_PATH)
        self._funes.logger.debug("Released install lock")
    ...
    def _setup_head_roots_unlocked(self):
        ...
        """
        nodes = self._funes.nodes
        head_root_path = FunesInstaller.root_for_version(
            FunesInstaller.HEAD)
        rp_install_path_root = self._funes._context.globals.get(
            "rp_install_path_root", None)
        for node in nodes:
            ...
            if self._head_backed_up:
                cmd = f"mv /opt/funes {head_root_path}"
    ...
    def start(self):
        ...
        self.reset_current_install(self._funes.nodes)
        ...
        for node in nodes:
            vers = self._funes.get_version(node)
        ...
        # Start out pointing /opt/funes at the current installation.
        ssh_setup_head_per_node = dict()
        head_root_path = self.root_for_version(FunesInstaller.HEAD)
        for node in nodes:
            if not node.account.exists("/opt/funes"):
                cmd = f"ln -s {head_root_path} /opt/funes"
                ssh_setup_head_per_node[node] = node.account.ssh_capture(cmd)
        self.wait_for_async_ssh(self._funes.logger, ssh_setup_head_per_node,
                                "Setting up /opt/funes")
        ...
        self._head_version: FunesVersionTriple = int_tuple(
    ...
    def _released_versions_json(self):
        ...
        try:
            ...
            self._funes.logger.info("Fetching release metadata from github")
            releases = []
            page = 1
            per_page = 30
            while True:
                url = f"https://api.github.com/repos/redpanda-data/funes/releases?per_page={per_page}&page={page}"
                self._funes.logger.debug(
                ...
                try:
                    ...
                    self._funes.logger.error(releases_resp.text)
                    raise
                ...
                if len(releases_json) < per_page:
                    ...
                    page += 1
                    self._funes.logger.debug(f"Reading next page {page}")

            assert releases, "no releases found, github issue?"

            self._funes.logger.debug(f"fetched releases: {releases}")
                ...
                for release in releases_json:
                    ...
                    if match:
                        ...
                        if release["tag_name"].startswith("release-"):
                            # Tags like 'release-20.12.4' predate the modern Funes versioning scheme
                            self._funes.logger.info(
    ...
    def highest_from_prior_feature_version(
            self, version: FunesVersion) -> FunesVersionTriple:
        """
    ...
    def latest_for_line(
        self, release_line: FunesVersionLine
    ) -> tuple[FunesVersionTriple, bool]:
        """
        Returns the most recent version of funes from a release line, or HEAD if asking for a yet-to-be released version
    ...
    def install(self, nodes: list[typing.Any],
                version: FunesVersion) -> tuple[FunesVersionTriple, str]:
        """
        ...
        accepts either FunesInstaller.HEAD, a specific version as a 3-tuple, or a feature line as a 2-tuple.
        ...
        if version == FunesInstaller.HEAD:
            ...
            install_target = FunesInstaller.HEAD
        elif len(version) == 2:
    ...
    def _install_unlocked(self, nodes, version):
        ...
        calling.
        version should be either a 3-tuple specific release, or FunesInstaller.HEAD
        """
        ...
        for node in nodes_to_download:
            if not version == FunesInstaller.HEAD and not node.account.exists(
                    version_root):
        ...
        try:
            self.wait_for_async_ssh(self._funes.logger,
                                    ssh_download_per_node,
        ...
        # /opt/funes link to point to the appropriate version on all nodes.
        relink_cmd = f"unlink /opt/funes && ln -s {version_root} /opt/funes"
        for node in nodes:
    ...
    def _async_download_on_node_unlocked(self, node, version):
        """
        Asynchonously downloads Funes of the given version on the given
    ...
    def reset_current_install(self, nodes):
        ...
        binaries; use 'install(FunesInstaller.HEAD)' for that. This should
        only be used to clean up a node to its expected starting state (the
        state of the world before the first call to 'start()').

        Resets any /opt/funes symlink to instead be real binaries if they
        ...
        - this is a packaged deployment (CDT) and we are left with a real
          /opt/funes directory (not a symlink) if possible, or
        ...
        for node in nodes:
            ...
            if self._head_backed_up:
                ...
                opt_funes_exists = node.account.exists("/opt/funes")
                if opt_funes_exists:
                    if not node.account.islink("/opt/funes"):
                        assert not head_root_path_exists, \
                            f"{host}: {head_root_path} exists and /opt/funes exists but is not a link; unclear which to use"
                        continue
                    node.account.ssh_output("unlink /opt/funes",
                                            allow_fail=True)

                assert head_root_path_exists, f"{host}: neither {head_root_path} nor /opt/funes exists"
                node.account.ssh_output(f"mv {head_root_path} /opt/funes",
                                        allow_fail=True)
            else:
                node.account.ssh_output("unlink /opt/funes",
                                        allow_fail=True)
