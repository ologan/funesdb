...
class LiveClusterParams:
    ...
    Should not be used outside of the funes_cloud module
    """
    _isAlive: bool = False
...
class CloudCluster():
    """
    Operations on a Funes Cloud cluster via the swagger API.
    ...
    def _get_cluster_console_url(self):
        cluster = self.cloudv2._http_get(
            endpoint=f'/api/v1/clusters/{self.config.id}')
        return cluster['status']['listeners']['funesConsole']['default'][
            'urls'][0]
    ...
    def _get_product_name(self, config_profile_name):
        ...
        for p in products:
            if p['funesConfigProfileName'] == config_profile_name:
                return p['name']
    ...
    def get_public_metrics(self):
        ...
        base_url = cluster['status']['listeners']['funesConsole'][
            'default']['urls'][0]
    ...
    def get_broker_address(self):
        cluster = self.cloudv2._http_get(
            endpoint=f'/api/v1/clusters/{self.config.id}')
        return cluster['status']['listeners']['sql']['default']['urls'][0]
    ...
    def get_product(self):
        ...
        for product in products:
            if product[
                    'funesConfigProfileName'] == self.config.config_profile_name:
                return product
        return None
