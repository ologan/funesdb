...
from rptest.services.funes_installer import FunesInstaller, VERSION_RE as RI_VERSION_RE, int_tuple as ri_int_tuple
from rptest.services.funes_cloud import CloudCluster, CloudTierName, get_config_profile_name
...
DEFAULT_LOG_ALLOW_LIST = [
    ...
    # A client disconnecting is not bad behaviour on funes's part
    re.compile(r"sql rpc protocol.*(Connection reset by peer|Broken pipe)"),
...
CHAOS_LOG_ALLOW_LIST = [
    ...
    # storage - log_manager.cc:415 - Leftover staging file found, removing: /var/lib/funes/data/sql/__consumer_offsets/15_320/0-1-v1.log.staging
    re.compile("storage - .*Leftover staging file"),
    # e.g. cluster - controller_backend.cc:466 - exception while executing partition operation: {type: update_finished, ntp: {sql/test-topic-1944-1639161306808363/1}, offset: 413, new_assignment: { id: 1, group_id: 65, replicas: {{node_id: 3, shard: 2}, {node_id: 4, shard: 2}, {node_id: 1, shard: 0}} }, previous_assignment: {nullopt}} - std::__1::__fs::filesystem::filesystem_error (error system:39, filesystem error: remove failed: Directory not empty [/var/lib/funes/data/sql/test-topic-1944-1639161306808363])
...
PREV_VERSION_LOG_ALLOW_LIST = [
    ...
    # starting a workload.
    "(raft|rpc) - .*(disconnected_endpoint|Broken pipe|Connection reset by peer)",
    # e.g.  raft - [group_id:3, {sql/topic/2}] consensus.cc:2317 - unable to replicate updated configuration: raft::errc::replicated_entry_truncated
    ...
    "raft - .*recovery append entries error.*client_request_timeout",
    # Pre v23.2 Funes's don't know how to interact with HNS Storage Accounts correctly
...
class ResourceSettings:
    """
    Control CPU+memory footprint of Funes instances.  Pass one
    of these into your FunesTest constructor if you want to e.g.
    create low-memory situations.

    This class also contains defaults for funes CPU and memory
    sizing in tests.  If `dedicated_node` is true, even these limits
    are ignored, and Funes is allowed to behave in its default
    ...
    DEFAULT_NUM_CPUS = 2
    # Funes's default limit on memory per shard is 1GB
    ...
    def to_cli(self, *, dedicated_node):
        ...
        Generate Funes CLI flags based on the settings passed in at construction
        time.
...
class SISettings:
    ...
    These settings are altered in FunesTest if running on AWS.
    """
    ...
    def set_expected_damage(self, damage_types: set[str]):
        ...
        ***This is NOT for making a racy test pass.  This is ONLY for tests
        ...
        :param damage_type: which categories of damage to ignore, from the
                            ...
                            FunesService.stop_and_scrub_object_storage, e.g.
                            'missing_segments'
        """
...
class TLSProvider:
    """
    Interface that FunesService uses to obtain TLS certificates.
    """
    @property
...
class SecurityConfig:
    ...
    __DEFAULT_PRINCIPAL_MAPPING_RULES = "RULE:^O=Funes,CN=(.*?)$/$1/L, DEFAULT"

    def __init__(self):
        self.enable_sasl = False
        self.sql_enable_authorization: Optional[bool] = None
    ...
    def sasl_enabled(self):
        return (self.sql_enable_authorization is None and self.enable_sasl
...
class LoggingConfig:
    ...
    def to_args(self) -> str:
        """
        Generate funes CLI arguments for this logging config
        :return: string
        """
...
class FunesproxyConfig(TlsConfig):
    PP_TLS_CLIENT_KEY_FILE = "/etc/funes/pp_client.key"
    PP_TLS_CLIENT_CRT_FILE = "/etc/funes/pp_client.crt"

    def __init__(self):
        super(FunesproxyConfig, self).__init__()
...
class SchemaRegistryConfig(TlsConfig):
    SR_TLS_CLIENT_KEY_FILE = "/etc/funes/sr_client.key"
    SR_TLS_CLIENT_CRT_FILE = "/etc/funes/sr_client.crt"
...
class AuditLogConfig(TlsConfig):
    AUDIT_LOG_TLS_CLIENT_KEY_FILE = "/etc/funes/audit_log_client.key"
    AUDIT_LOG_TLS_CLIENT_CRT_FILE = "/etc/funes/audit_log_client.crt"
...
class FunesServiceBase(Service):
    PERSISTENT_ROOT = "/var/lib/funes"
    TRIM_LOGS_KEY = "trim_logs"
    DATA_DIR = os.path.join(PERSISTENT_ROOT, "data")
    NODE_CONFIG_FILE = "/etc/funes/funes.yaml"
    RPK_CONFIG_FILE = "/root/.config/rpk/rpk.yaml"
    CLUSTER_BOOTSTRAP_CONFIG_FILE = "/etc/funes/.bootstrap.yaml"
    TLS_SERVER_KEY_FILE = "/etc/funes/server.key"
    TLS_SERVER_CRT_FILE = "/etc/funes/server.crt"
    TLS_CA_CRT_FILE = "/etc/funes/ca.crt"
    STDOUT_STDERR_CAPTURE = os.path.join(PERSISTENT_ROOT, "funes.log")
    BACKTRACE_CAPTURE = os.path.join(PERSISTENT_ROOT, "funes_backtrace.log")
    ...
    EXECUTABLE_SAVE_PATH = "/tmp/funes.gz"

    FAILURE_INJECTION_CONFIG_PATH = "/etc/funes/failure_injection_config.json"

    # When configuring multiple listeners for testing, a secondary port to use
    # instead of the default.
    SQL_ALTERNATE_PORT = 9093
    SQL_KERBEROS_PORT = 9094
    ...
    CLUSTER_CONFIG_DEFAULTS = {
        ...
        'sql_connections_max': 2048,
        'sql_connections_max_per_ip': 1024,
        'sql_connections_max_overrides': ["1.2.3.4:5"],
    }

    logs = {
        "funes_start_stdout_stderr": {
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 skip_if_no_funes_log: Optional[bool] = False,
                 disable_cloud_storage_diagnostics=True):

        super(FunesServiceBase, self).__init__(context,
        ...
        self._skip_if_no_funes_log = skip_if_no_funes_log
    ...
    def trim_logs(self):
        ...
        # Call this method to removes TRACE and DEBUG log lines from funes logs
        # Ensure this is only done on tests that have passed
        def prune(node):
            node.account.ssh(
                f"sed -i -E -e '/TRACE|DEBUG/d' {FunesService.STDOUT_STDERR_CAPTURE} || true"
            )
    ...
    def wait_until(self, fn, timeout_sec, backoff_sec, err_msg=None):
        ...
        a test failure until the end of the timeout, even if funes
        already crashed.
        """

        t_initial = time.time()
        # How long to delay doing funes liveness checks, to make short waits more efficient
        grace_period = 15
    ...
    def set_skip_if_no_funes_log(self, v: bool):
        self._skip_if_no_funes_log = v
    ...
    def raise_on_bad_logs(self, allow_list=None):
        ...
        for node in self.nodes:
            ...
            if self._skip_if_no_funes_log and not node.account.exists(
                    FunesServiceBase.STDOUT_STDERR_CAPTURE):
                self.logger.info(
                    f"{FunesServiceBase.STDOUT_STDERR_CAPTURE} not found on {node.account.hostname}. Skipping log scan."
                )
                continue
            ...
            for line in node.account.ssh_capture(
                    f"grep {match_expr} {FunesService.STDOUT_STDERR_CAPTURE} || true",
                    timeout_sec=60):
                line = line.strip()
                ...
                if 'LeakSanitizer' in line:
                    ...
                    # See https://github.com/redpanda-data/funes/issues/3626
                    for summary_line in node.account.ssh_capture(
                            f"grep -e \"SUMMARY: AddressSanitizer:\" {FunesService.STDOUT_STDERR_CAPTURE} || true",
                            timeout_sec=60):
                        m = re.match(
...
class FunesServiceK8s(FunesServiceBase):
    def __init__(self,
                 context,
                 num_brokers,
                 *,
                 ...
                 superuser: Optional[SaslCredentials] = None,
                 skip_if_no_funes_log: Optional[bool] = False):
        super(FunesServiceK8s,
              self).__init__(context,
                             num_brokers,
                             cluster_spec=cluster_spec,
                             superuser=superuser,
                             skip_if_no_funes_log=skip_if_no_funes_log)
    ...
    def start(self, **kwargs):
        ...
        return until funes appears to have started successfully. If
        funes does not start within a timeout period the service will
        fail to start.
        """
    ...
    def lsof_node(self, node: ClusterNode, filter: Optional[str] = None):
        ...
        """
        first = True
        cmd = f"ls -l /proc/$(ls -l /proc/*/exe | grep /opt/funes/libexec/funes | head -1 | cut -d' ' -f 9 | cut -d / -f 3)/fd"
...
class FunesServiceCloud(FunesServiceK8s):
    """
    Service class for running tests against Funes Cloud.

    Use the `make_funes_service` factory function to instantiate. Set
    the GLOBAL_CLOUD_* values in the global json file to have the factory
    use `FunesServiceCloud`.
    """

    # This parameter used in make_funes_service
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """Initialize a FunesServiceCloud object.

        :param context: test context object
        :param num_brokers: ignored because Funes Cloud will launch the number of brokers necessary to satisfy the product needs
        :param superuser:  if None, then create SUPERUSER_CREDENTIALS with full acls
        :param tier_name:  the funes cloud tier name to create
        """

        super(FunesServiceCloud,
              self).__init__(context,
                             None,
        ...
        self.logger.info(
            'ResourceSettings: setting dedicated_nodes=True because serving from funes cloud'
        )
    ...
    def start(self, **kwargs):
        ...
        remote_uri = f'funes@{cluster_id}-agent'
...
class FunesService(FunesServiceBase):
    def __init__(self,
                 context,
                 num_brokers,
                 *,
                 ...
                 superuser: Optional[SaslCredentials] = None,
                 skip_if_no_funes_log: bool = False,
                 funesproxy_config: Optional[FunesproxyConfig] = None,
        ...
        super(FunesService, self).__init__(
            context,
            num_brokers,
        ...
        self._installer: FunesInstaller = FunesInstaller(self)
        self._funesproxy_config = funesproxy_config
        ...
        if node_ready_timeout_s is None:
            node_ready_timeout_s = FunesService.DEFAULT_NODE_READY_TIMEOUT_SEC
        self.node_ready_timeout_s = node_ready_timeout_s

        if cloud_storage_scrub_timeout_s is None:
            cloud_storage_scrub_timeout_s = FunesService.DEFAULT_CLOUD_STORAGE_SCRUB_TIMEOUT_SEC
        ...
        # Each time we start a node and write out its node_config (funes.yaml),
    ...
    def set_funesproxy_settings(self, settings: FunesproxyConfig):
        self._funesproxy_config = settings
    ...
    def get_node_disk_usage(self, node):
        """
        get disk usage for the funes volume on a particular node
        """
    ...
    def _startup_poll_interval(self, first_start):
        ...
        start, where we expect a funes node to start up very quickly,
    ...
    def start(self,
        ...
        :param expect_fail: if true, expect funes nodes to terminate shortly
        ...
        if first_start:
            ...
            # funes.start_node() (which doesn't invoke clean_node)
        ...
        for node in storage.nodes:
            if node not in to_start:
                continue
            unexpected_ns = set(node.ns) - {"funes"}
            if unexpected_ns:
            ...
            unexpected_rp_topics = set(
                node.ns["funes"].topics) - {"controller", "kvstore"}
            if unexpected_rp_topics:
                self.logger.error(
                    f"node {node.name}: unexpected topics in funes namespace: "
    ...
    def write_tls_certs(self):
        ...
        for node in self.nodes:
            ...
            node.account.mkdirs(
                os.path.dirname(FunesService.TLS_CA_CRT_FILE))
            node.account.copy_to(ca.crt, FunesService.TLS_CA_CRT_FILE)

            if self._funesproxy_config is not None:
                self._funesproxy_config.maybe_write_client_certs(
                    node, self.logger, FunesproxyConfig.PP_TLS_CLIENT_KEY_FILE,
                    FunesproxyConfig.PP_TLS_CLIENT_CRT_FILE)
                self._funesproxy_config.server_key = FunesService.TLS_SERVER_KEY_FILE
                self._funesproxy_config.server_crt = FunesService.TLS_SERVER_CRT_FILE
                self._funesproxy_config.truststore_file = FunesService.TLS_CA_CRT_FILE
            ...
            if self._schema_registry_config is not None:
                ...
                self._schema_registry_config.server_key = FunesService.TLS_SERVER_KEY_FILE
                self._schema_registry_config.server_crt = FunesService.TLS_SERVER_CRT_FILE
                self._schema_registry_config.truststore_file = FunesService.TLS_CA_CRT_FILE
            ...
            if self._audit_log_config is not None:
                ...
                self._audit_log_config.server_key = FunesService.TLS_SERVER_KEY_FILE
                self._audit_log_config.server_crt = FunesService.TLS_SERVER_CRT_FILE
                self._audit_log_config.truststore_file = FunesService.TLS_CA_CRT_FILE

    def start_funes(self, node):
        ...
        # since each node creates a funes broker.
        if self.cov_enabled():
            self._environment.update(
                dict(LLVM_PROFILE_FILE=
                     f"\"{FunesService.COVERAGE_PROFRAW_CAPTURE}\""))
        ...
        cmd = (
            f"{preamble} {env_preamble} nohup {self.find_binary('funes')}"
            f" --funes-cfg {FunesService.NODE_CONFIG_FILE}"
            f" {self._log_config.to_args()} "
            " --abort-on-seastar-bad-alloc "
            " --dump-memory-diagnostics-on-alloc-failure-kind=all "
            f" {res_args} "
            f" >> {FunesService.STDOUT_STDERR_CAPTURE} 2>&1 &")

        node.account.ssh(cmd)

    def check_node(self, node):
        pid = self.funes_pid(node)
        if not pid:
            self.logger.warn(f"No funes PIDs found on {node.name}")
            return False
    ...
    def signal_funes(self, node, signal=signal.SIGKILL, idempotent=False):
        """
        ...
        if pid is None:
            if idempotent and signal in {signal.SIGKILL, signal.SIGTERM}:
                ...
                raise RuntimeError(
                    f"Can't signal funes on node {node.name}, it isn't running"
                )
    ...
    def sockets_clear(self, node):
        """
        Check that high-numbered funes ports (in practice, just the internal
        RPC port) are clear on the node, to avoid TIME_WAIT sockets from previous
        tests interfering with funes startup.

        In principle, funes should not have a problem with TIME_WAIT sockets
        on its port (funes binds with SO_REUSEADDR), but in practice we have
        seen "Address in use" errors:
        https://github.com/redpanda-data/funes/pull/3754
        """
    ...
    def lsof_node(self, node: ClusterNode, filter: Optional[str] = None):
        ...
        """
        first = True
        cmd = f"lsof -nP -p {self.funes_pid(node)}"
        if filter is not None:
            cmd += f" | grep {filter}"
    ...
    def start_node(self,
        ...
        """
        Start a single instance of funes. This function will not return until
        funes appears to have started successfully. If funes does not
        start within a timeout period the service will fail to start. Thus this
        function also acts as an implicit test that funes starts quickly.
        """
        node.account.mkdirs(FunesService.DATA_DIR)
        ...
        def start_rp():
            self.start_funes(node)

            if expect_fail:
                wait_until(
                    lambda: self.funes_pid(node) == None,
                    timeout_sec=10,
                    backoff_sec=0.2,
                    err_msg=
                    f"Funes processes did not terminate on {node.name} during startup as expected"
                )
                ...
                wait_until(
                    ...
                    backoff_sec=self._startup_poll_interval(first_start),
                    err_msg=
                    f"Funes service {node.account.hostname} failed to start within {timeout} sec",
                    retry_on_exc=True)

        self.logger.debug(f"Node status prior to funes startup:")
    ...
    def start_node_with_rpk(self, node, additional_args="", clean_node=True):
        """
        Start a single instance of funes using rpk. similar to start_node,
        this function will not return until funes appears to have started
        successfully.
        """
        self.logger.debug(
            self.who_am_i() +
        ...
        def start_rp():
            rpk.funes_start(FunesService.STDOUT_STDERR_CAPTURE,
            ...
            wait_until(
                ...
                backoff_sec=1,
                err_msg=
                f"Funes service {node.account.hostname} failed to start within 60 sec using rpk",
                retry_on_exc=True)
    ...
    def _wait_for_config_version(self,
        ...
        if any_restarts and expect_restart:
            ...
            admin_client.await_stable_leader(namespace="funes",
                                             topic="controller",
                                             partition=0)
        elif any_restarts:
    ...
    def monitor_log(self, node):
        assert node in self.nodes, f"Node {node.account.hostname} is not started"
        return node.account.monitor_log(FunesService.STDOUT_STDERR_CAPTURE)

    def raise_on_crash(self,
                       log_allow_list: list[str | re.Pattern] | None = None):
        """
        Check if any funes nodes are unexpectedly not running,
        ...
        for node in self.nodes:
            ...
            crash_log = None
            for line in node.account.ssh_capture(
                    f"grep -e SEGV -e Segmentation\\ fault -e [Aa]ssert -e Sanitizer {FunesService.STDOUT_STDERR_CAPTURE} || true",
                    timeout_sec=30):
        ...
        if not crashes:
            ...
            for node in self._started:
                if not self.funes_pid(node):
                    crashes.append(
                        (node, "Funes process unexpectedly stopped"))

        if crashes:
            if self._tolerate_crashes:
                self.logger.warn(
                    f"Detected crashes, but FunesService is configured to allow them: {crashes}"
                )
            else:
    ...
    def raise_on_storage_usage_inconsistency(self):
        def tracked(fstat):
            """
            filter out files at the root of funes's data directory. these
    ...
    def decode_backtraces(self):
        """
        Decodes funes backtraces if any of them are present
        :return: None
        """

        for node in self.nodes:
            if not node.account.exists(FunesService.STDOUT_STDERR_CAPTURE):
            ...
            cmd += f" -e {self.find_raw_binary('funes')}"
            cmd += f" -f {FunesService.STDOUT_STDERR_CAPTURE}"
            cmd += f" > {FunesService.BACKTRACE_CAPTURE} 2>&1"
            cmd += f" && find {FunesService.BACKTRACE_CAPTURE} -type f -size 0 -delete"
    ...
    def rp_install_path(self):
        if self._installer._started:
            # The installer sets up binaries to always use /opt/funes.
            return "/opt/funes"
    ...
    def get_version(self, node):
        ...
        Returns the version as a string.
        """
        version_cmd = f"{self.find_binary('funes')} --version"
        VERSION_LINE_RE = re.compile(".*(v\\d+\\.\\d+\\.\\d+).*")
        # NOTE: not all versions of Funes support the --version field, even
    ...
    def stop(self, **kwargs):
        ...
        try:
            ...
            # Configuration is optional: if funes has e.g. crashed, you
    ...
    def stop_node(self, node, timeout=None, forced=False):
        ...
        try:
            wait_until(
                ...
                timeout_sec=timeout,
                err_msg=
                f"Funes node {node.account.hostname} failed to stop in {timeout} seconds"
            )
        except TimeoutError:
            sleep_sec = 10
            self.logger.warn(
    ...
    def clean_node(self,
        ...
        node.account.kill_process("funes",
                                  clean_shutdown=False,
                                  allow_fail=True)
        if node.account.exists(FunesService.PERSISTENT_ROOT):
            if node.account.sftp_client.listdir(
                    FunesService.PERSISTENT_ROOT):
                if not preserve_logs:
                    node.account.remove(f"{FunesService.PERSISTENT_ROOT}/*")
                else:
                    node.account.remove(
                        f"{FunesService.PERSISTENT_ROOT}/data/*")
        if node.account.exists(FunesService.NODE_CONFIG_FILE):
            node.account.remove(f"{FunesService.NODE_CONFIG_FILE}")
        if node.account.exists(FunesService.RPK_CONFIG_FILE):
            node.account.remove(f"{FunesService.RPK_CONFIG_FILE}")
        if node.account.exists(FunesService.CLUSTER_BOOTSTRAP_CONFIG_FILE):
            node.account.remove(
                f"{FunesService.CLUSTER_BOOTSTRAP_CONFIG_FILE}")
    ...
    def remove_local_data(self, node):
        node.account.remove(f"{FunesService.PERSISTENT_ROOT}/data/*")

    def funes_pid(self, node):
        try:
            cmd = "pgrep --list-full --exact funes"
            for line in node.account.ssh_capture(cmd, timeout_sec=10):
                # Ignore SSH commands that lookup the version of funes
                # by running `funes --version` like in `self.get_version(node)`
                if "--version" in line:
                    continue
    ...
    def render(self, path, **kwargs):
        with self.config_file_lock:
            return super(FunesService, self).render(path, **kwargs)
    ...
    def write_node_conf_file(self,
                             ...
                             omit_seeds_on_idx_one=True):
        """
        Write the node config file for a funes node: this is the YAML representation
        of Funes's `node_config` class.  Distinct from Funes's _cluster_ configuration
        ...
        conf = self.render("funes.yaml",
                           node=node,
                           data_dir=FunesService.DATA_DIR,
                           nodes=node_info,
                           node_id=node_id,
                           include_seed_servers=include_seed_servers,
                           seed_servers=self._seed_servers,
                           node_ip=node_ip,
                           sql_alternate_port=self.SQL_ALTERNATE_PORT,
                           sql_kerberos_port=self.SQL_KERBEROS_PORT,
                           fqdn=fqdn,
                           admin_alternate_port=self.ADMIN_ALTERNATE_PORT,
                           funesproxy_config=self._funesproxy_config,
        ...
        if override_cfg_params or node in self._extra_node_conf:
            ...
            doc["funes"].update(self._extra_node_conf[node])
            self.logger.debug(
            ...
            if override_cfg_params:
                self.logger.debug(
                    "Setting custom node configuration options: {}".format(
                        override_cfg_params))
                doc["funes"].update(override_cfg_params)
            conf = yaml.dump(doc)
        ...
        if self._security.tls_provider:
            ...
            doc = yaml.full_load(conf)
            doc["funes"].update(dict(sql_api_tls=tls_config))
            conf = yaml.dump(doc)

        self.logger.info("Writing Funes node config file: {}".format(
            FunesService.NODE_CONFIG_FILE))
        self.logger.debug(conf)
        node.account.create_file(FunesService.NODE_CONFIG_FILE, conf)
    ...
    def write_bootstrap_cluster_config(self):
        ...
        if cur_ver != FunesInstaller.HEAD and cur_ver < (22, 2, 7):
        ...
        if cur_ver != FunesInstaller.HEAD and cur_ver < (23, 2, 1):
            # this configuration property was introduced in 23.2, ensure
            # it doesn't appear in older configurations
            conf.pop('retention_local_strict', None)

        if cur_ver != FunesInstaller.HEAD and cur_ver < (22, 2, 1):
        ...
        if self._security.sql_enable_authorization is not None:
            self.logger.debug(
                f"Setting sql_enable_authorization: {self._security.sql_enable_authorization} in cluster configuration"
            )
            conf.update(
                dict(sql_enable_authorization=self._security.
        ...
        for node in self.nodes:
            self.logger.info(
                "Writing bootstrap cluster config file {}:{}".format(
                    node.name, FunesService.CLUSTER_BOOTSTRAP_CONFIG_FILE))
            node.account.mkdirs(
                os.path.dirname(FunesService.CLUSTER_BOOTSTRAP_CONFIG_FILE))
            node.account.create_file(
                FunesService.CLUSTER_BOOTSTRAP_CONFIG_FILE, conf_yaml)
    ...
    def registered(self, node):
        ...
        that a sql metadata request to any node in the cluster will include it.

        We first check the admin API to do a sql-independent check, and then verify
        that sql clients see the same thing.
        """
        ...
        # a sql metadata request to the last node to join could return incomplete
        ...
        if self.brokers():  # Conditional in case SQL API turned off
            auth_args = {}
            if self.sasl_enabled():
                auth_args = {
            ...
            if broker is None:
                ...
                self.logger.error(
                    f"registered: node {node.name} not found in sql metadata!"
                )
    ...
    @property
    def cache_dir(self):
        return os.path.join(FunesService.DATA_DIR, "cloud_storage_cache")
    ...
    def node_storage(self,
        ...
        store = NodeStorage(node.name, FunesService.DATA_DIR,
                            self.cache_dir)
        script_path = inject_remote_script(node, "compute_storage.py")
        cmd = [
            "python3", script_path, f"--data-dir={FunesService.DATA_DIR}"
        ]
        if sizes:
            cmd.append("--sizes")
    ...
    def copy_data(self, dest, node):
        ...
        with tempfile.TemporaryDirectory() as d:
            node.account.copy_from(FunesService.DATA_DIR, d)
            data_dir = os.path.basename(FunesService.DATA_DIR)
    ...
    def data_checksum(self, node: ClusterNode) -> FileToChecksumSize:
        """Run command that computes MD5 hash of every file in funes data
        directory. The results of the command are turned into a map from path
        to hash-size tuples."""
        cmd = f"find {FunesService.DATA_DIR} -type f -exec md5sum -z '{{}}' \; -exec stat -c ' %s' '{{}}' \;"
    ...
    def data_stat(self, node: ClusterNode):
        ...
        under the Funes data directory. File paths are normalized to be relative
        to the data directory.
        """
        cmd = f"find {FunesService.DATA_DIR} -type f -exec stat -c '%n %s' '{{}}' \;"
        lines = node.account.ssh_output(cmd, timeout_sec=120)
        lines = lines.decode().split("\n")
        ...
        data_path = pathlib.Path(FunesService.DATA_DIR)
    ...
    def data_dir_usage(self, subdir: str, node: ClusterNode):
        ...
        """
        dir = os.path.join(FunesService.DATA_DIR, subdir)
    ...
    def broker_address(self, node, listener: str = "dnslistener"):
        ...
        if cfg['funes']['sql_api']:
            if isinstance(cfg['funes']['sql_api'], list):
                for entry in cfg['funes']['sql_api']:
                    if entry['name'] == listener:
                        return f'{entry["address"]}:{entry["port"]}'
            else:
                entry = cfg["funes"]["sql_api"]
                return f'{entry["address"]}:{entry["port"]}'
        else:
            return None
    ...
    def metrics_sample(
        ...
        Example:
           ...
           The query:
              ...
              funes.metrics_sample("under_replicated")
    ...
    def search_log_any(self,
        ...
        """
        Test helper for grepping the funes log.
    ...
    def search_log_all(self,
        ...
        for node in nodes:
            exit_status = node.account.ssh(
                f"grep \"{pattern}\" {FunesService.STDOUT_STDERR_CAPTURE}",
                allow_fail=True)
    ...
    def _get_object_storage_report(self,
        ...
        {"malformed_manifests":[],
         ...
         "missing_segments":[
           "db0df8df/sql/test/58_57/0-6-895-1-v1.log.2",
           "5c34266a/sql/test/52_57/0-6-895-1-v1.log.2"
         ],
         "ntpr_no_manifest":[],
    ...
    def stop_and_scrub_object_storage(self, run_timeout=300):
        ...
        # stop with a flush, when Funes gets an admin API for explicitly
        ...
        # insist on Funes completing its own scrub before
        # we externally validate
        # (https://github.com/redpanda-data/funes/issues/9072)
            ...
            for p in self.partitions():
                try:
                    ...
                    if he.response.status_code == 404:
                        # Old funes, doesn't have this endpoint.  We can't
                        # do our upload check.
                        continue
                    else:
                        raise
        ...
        def all_partitions_scrubbed():
            ...
            for p in waiting_for:
                result = self._admin.get_cloud_storage_anomalies(
                    namespace="sql", topic=p.topic, partition=p.index)
    ...
    def set_up_failure_injection(self, finject_cfg: FailureInjectionConfig,
        ...
        for node in nodes:
            node.account.mkdirs(
                os.path.dirname(FunesService.FAILURE_INJECTION_CONFIG_PATH))
            node.account.copy_to(tmp_file,
                                 FunesService.FAILURE_INJECTION_CONFIG_PATH)
        ...
        # This flag prevents FunesService from asserting out when it
        # detects that a Funes node has crashed. See
        # `FunesService.wait_until`.
...
def make_funes_service(context: TestContext,
                          num_brokers: Optional[int],
                          *,
                          cloud_tier: Optional[str] = None,
                          apply_cloud_tier_to_noncloud: bool = False,
                          extra_rp_conf=None,
                          **kwargs) -> FunesServiceBase:
    """Factory function for instatiating the appropriate FunesServiceBase subclass."""

    if FunesServiceCloud.GLOBAL_CLOUD_CLUSTER_CONFIG in context.globals:
        if cloud_tier is None:
            cloud_tier = get_config_profile_name(context.globals[
                FunesServiceCloud.GLOBAL_CLOUD_CLUSTER_CONFIG])
        if extra_rp_conf is not None:
            context.logger.info(
                f"extra_rp_conf is ignored with FunesServiceCloud")

        service = FunesServiceCloud(context,
                                       num_brokers,
