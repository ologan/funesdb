...
class SQLCliConsumer(BackgroundThreadService):
    def __init__(self,
                 context,
                 funes,
                 topic,
                 ...
                 instance_name=None):
        super(SQLCliConsumer, self).__init__(context, num_nodes=1)
        self._funes = funes
        self._topic = topic
        self._group = group
        self._offset = offset
        ...
        self._cli = SQLCliTools(self._funes)
        self._message_cnt = 0

    def script(self):
        return self._cli._script("sql-console-consumer.sh")
