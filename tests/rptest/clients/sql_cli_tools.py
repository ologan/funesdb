...
class SQLCliTools:
    """
    Wrapper around the SQL admin command line tools.
    """
    ...
    def __init__(self,
        ...
        assert self._version is None or \
                self._version in SQLCliTools.VERSIONS
...
sasl.jaas.config=org.apache.sql.common.security.scram.ScramLoginModule required username="{user}" password="{passwd}";
"""
    ...
    def instances(cls):
        def make_factory(version):
            return lambda funes: cls(funes, version)

        return list(map(make_factory, cls.VERSIONS))

    def create_topic(self, spec: TopicSpec):
        self._funes.logger.debug("Creating topic: %s", spec.name)
        args = ["--create"]
        ...
        return self._run("sql-topics.sh", args)

    def create_topic_partitions(self, topic, partitions):
        self._funes.logger.debug("Adding %d partitions to topic: %s",
                                    partitions, topic)
        args = ['--alter']
        args += ["--topic", topic]
    ...
    def create_topic_with_config(self, name, partitions, replication_factor,
        ...
        self._funes.logger.debug("Creating topic: %s", name)
        args = ["--create"]
        args += ["--topic", name]
    ...
    def delete_topic(self, topic):
        self._funes.logger.debug("Deleting topic: %s", topic)
        args = ["--delete"]
        args += ["--topic", topic]
        return self._run("sql-topics.sh", args)

    def list_topics(self):
        self._funes.logger.debug("Listing topics")
        args = ["--list"]
        res = self._run("sql-topics.sh", args)
        topics = res.split()
        self._funes.logger.debug("Found topics: %s", topics)
        return topics

    def describe_topics(self):
        self._funes.logger.debug("Describing topics")
        args = ["--describe"]
        res = self._run("sql-topics.sh", args)
        self._funes.logger.debug("Describe topics result: %s", res)
        return res

    def describe_topic_config(self, topic):
        self._funes.logger.debug("Describing topic configs")
        args = ["--describe", "--topic", topic, "--all"]
        res = self._run("sql-configs.sh", args).strip()
        self._funes.logger.debug("Describe topic configs result: %s", res)
        if res is None:
    ...
    def describe_topic(self, topic):
        ...
        self._funes.logger.debug(f"Describe topics configs: {configs}")
        configs = [
    ...
    def describe_broker_config(self):
        self._funes.logger.debug("Describing brokers")
        args = ["--describe", "--entity-type", "brokers", "--all"]
        res = self._run("sql-configs.sh", args)
        self._funes.logger.debug("Describe brokers config result: %s", res)
        return res

    def alter_topic_config(self, topic, configuration_map):
        self._funes.logger.debug("Altering topic %s configuration with %s",
        ...
        return self._run("sql-configs.sh", args)
    ...
    def produce(self,
                ...
                throughput=-1,
                batch_size=81960):
        self._funes.logger.debug("Producing to topic: %s", topic)
        cmd = [self._script("sql-producer-perf-test.sh")]
        cmd += ["--topic", topic]
        ...
        cmd += [
            ...
            "bootstrap.servers=%s" % self._funes.brokers()
        ]
        self._execute(cmd)

    def list_acls(self):
        args = ["--list"]
        return self._run("sql-acls.sh", args)
    ...
    def get_api_versions(self):
        return self._run("sql-run-class.sh", [],
                         "sql.admin.BrokerApiVersionsCommand")
    ...
    def reassign_partitions(self,
        ...
        try:
            ...
            def do_reassign_partitions():
                nonlocal output
                output = self._run("sql-reassign-partitions.sh", args)
    ...
    def describe_producers(self, topic, partition):
        ...
        cmd = [self._script("sql-transactions.sh")]
        cmd += ["--bootstrap-server", self._funes.brokers()]
    ...
    def list_transactions(self):
        ...
        self._funes.logger.debug("List transactions")
        cmd = [self._script("sql-transactions.sh")]
        cmd += ["--bootstrap-server", self._funes.brokers()]
        cmd += ["list"]
        res = self._execute(cmd)
    ...
    def describe_transaction(self, tx_id):
        ...
        self._funes.logger.debug("Describe transaction tx_id: %s", tx_id)
        cmd = [self._script("sql-transactions.sh")]
        cmd += ["--bootstrap-server", self._funes.brokers()]
        cmd += ["describe"]
    ...
    def _execute(self, cmd):
        ...
        try:
            ...
            self._funes.logger.debug("Error (%d) executing command: %s",
    ...
    def _script(self, script):
        version = self._version or SQLCliTools.VERSIONS[0]
