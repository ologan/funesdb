...
class DefaultClient:
    ...
    def create_topic(self, specs):
        if isinstance(specs, TopicSpec):
            specs = [specs]
        client = SQLCliTools(self._funes)
        for spec in specs:
    ...
    def brokers(self):
        ...
        in the self._funes service.
        """
        client = PythonLibrdsql(self._funes)
    ...
    def alter_topic_configs(self, topic: str,
        ...
        """
        sql_tools = SQLCliTools(self._funes)
        sql_tools.alter_topic_config(topic, props)
