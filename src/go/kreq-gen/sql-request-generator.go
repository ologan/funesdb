...
func makeRandomSQLData(version int16) []byte {
	var bytearr []byte
	if version >= 3 {
...
func randomFill(v reflect.Value, version int16) {
	...
	case reflect.Struct:
		...
		// Special cases, the following byte arrays must be populated with sql
		// record & recordBatch formatted data
		if v.Type().Name() == "ProduceRequestTopicPartition" {
			recordBatchData := makeRandomSQLData(version)
...
func main() {
	...
	isRequest := flag.Bool("is-request", true, "Generate sql request if true, response if false")
