...
func TestTuneFstrimDirectExecutorRollback(t *testing.T) {
	...
	exists, err := afero.Exists(fs, systemd.UnitPath("funes-fstrim.service"))
...
cat << EOF > /etc/systemd/system/funes-fstrim.service
[Unit]
...
cat << EOF > /etc/systemd/system/funes-fstrim.timer
[Unit]
...
EOF
sudo systemctl daemon-reload
sudo systemctl start funes-fstrim.timer
`,
		},
		{
...
`,
		...
		{
			name: "it should just start the funes-fstrim timer if it exists",
			unitState: func(name string) (systemd.LoadState, systemd.ActiveState, error) {
				// Mock the default units' state as Unknown and
				// the funes ones as Inactive
				if strings.HasPrefix(name, "funes-") {
					return systemd.LoadStateLoaded, systemd.ActiveStateInactive, nil
				}
				return systemd.LoadStateUnknown, systemd.ActiveStateUnknown, nil
			},
...
`,
		...
		{
			name: "it shouldn't do anything if the funes units are started",
			unitState: func(name string) (systemd.LoadState, systemd.ActiveState, error) {
				// Mock the default units' state as Unknown and
				// the funes ones as Inactive
				if strings.HasPrefix(name, "funes-") {
