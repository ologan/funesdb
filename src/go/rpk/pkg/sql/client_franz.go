...
// NewFranzClient returns a franz-go based sql client.
func NewFranzClient(fs afero.Fs, p *config.RpkProfile, extraOpts ...kgo.Opt) (*kgo.Client, error) {
	...
	opts := []kgo.Opt{
		...
		// Funes may indicate one leader just before rebalancing the
		// leader to a different server. During this rebalance,
		// Funes may return stale metadata. We always want fresh
	...
	if id := d.SQLProtocolReqClientID; id != "" {
...
// error message string corresponding to the SQL error for code.
