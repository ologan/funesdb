...
func updateConfigWithFlags(y *config.FunesYaml, flags *pflag.FlagSet) {
	...
	if flags.Changed(nodeIDFlag) {
		y.Funes.ID = new(int)
		*y.Funes.ID, _ = flags.GetInt(nodeIDFlag)
	}
}
...
func NewStartCommand(fs afero.Fs, p *config.Params, launcher rp.Launcher) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		FParseErrWhitelist: cobra.FParseErrWhitelist{
			...
			// through to funes/seastar without the need to pass '--'
			// (POSIX standard)
			UnknownFlags: true,
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// or flag overrides. This command itself has all funes specific
			// flags installed, and handles funes specific env vars itself.
			// The magic `--set` flag is what modifies any funes.yaml fields.
			// Thus, we can ignore any env / flags that would come from rpk
			// configuration itself.
			y := cfg.ActualFunesYamlOrDefaults()
			if y.Funes.DeveloperMode && len(mode) == 0 {
				mode = "dev-container"
			}
			switch mode {
			case "dev-container":
			...
			if len(seeds) == 0 {
				...
				// env var.
				envSeeds := os.Getenv("FUNES_SEEDS")
				if envSeeds != "" {
					seeds = strings.Split(
						envSeeds,
						",",
			...
			if len(seedServers) != 0 {
				y.Funes.SeedServers = seedServers
			}

			sqlAddr = stringSliceOr(
				sqlAddr,
				strings.Split(
					os.Getenv("FUNES_SQL_ADDRESS"),
					",",
				),
			)
			sqlAPI, err := parseNamedAuthNAddresses(
				sqlAddr,
				config.DefaultSQLPort,
			)
			if err != nil {
				return err
			}
			if len(sqlAPI) > 0 {
				y.Funes.SQLAPI = sqlAPI
			}

			proxyAddr = stringSliceOr(
				proxyAddr,
				strings.Split(
					os.Getenv("FUNES_FUNESPROXY_ADDRESS"),
					",",
				),
			)
			...
			if len(proxyAPI) > 0 {
				if y.Funesproxy == nil {
					y.Funesproxy = config.DevDefault().Funesproxy
				}
				y.Funesproxy.FunesproxyAPI = proxyAPI
			}

			schemaRegAddr = stringSliceOr(
				schemaRegAddr,
				strings.Split(
					os.Getenv("FUNES_SCHEMA_REGISTRY_ADDRESS"),
					",",
				),
			)
			...
			if rpcServer != nil {
				y.Funes.RPCServer = *rpcServer
			}

			advertisedSQL = stringSliceOr(
				advertisedSQL,
				strings.Split(
					os.Getenv("FUNES_ADVERTISE_SQL_ADDRESS"),
					",",
				),
			)
			advSQLAPI, err := parseNamedAddresses(
				advertisedSQL,
				config.DefaultSQLPort,
			)
			if err != nil {
			...
			if len(advSQLAPI) > 0 {
				y.Funes.AdvertisedSQLAPI = advSQLAPI
			}

			advertisedProxy = stringSliceOr(
				advertisedProxy,
				strings.Split(
					os.Getenv("FUNES_ADVERTISE_FUNESPROXY_ADDRESS"),
					",",
				),
			)
			...
			if len(advProxyAPI) > 0 {
				...
				y.Funesproxy.AdvertisedFunesproxyAPI = advProxyAPI
			}

			advertisedRPC = stringOr(
				advertisedRPC,
				os.Getenv("FUNES_ADVERTISE_RPC_ADDRESS"),
			)
			...
			if advRPCApi != nil {
				y.Funes.AdvertisedRPCAPI = advRPCApi
			}
			installDirectory, err := getOrFindInstallDir(fs, installDirFlag)
			if err != nil {
				return err
			}
			rpArgs, err := buildFunesFlags(
				fs,
				y,
				filteredArgs,
				sFlags,
			...
			if y.Funes.Directory == "" {
				y.Funes.Directory = config.DevDefault().Funes.Directory
			}
			...
			fmt.Print(`We'd love to hear about your experience with Funes:
https://redpanda.com/feedback
`)
			fmt.Println("Starting funes...")
			return launcher.Start(installDirectory, rpArgs)
		},
	}

	f := cmd.Flags()
	f.IntVar(&nodeID, nodeIDFlag, -1, "The node ID. Must be an integer and must be unique within a cluster. If unset, Funes will assign one automatically")
	...
	f.StringSliceVar(&sqlAddr, "sql-addr", nil, "A comma-separated list of SQL listener addresses to bind to (scheme://host:port|name)")
	f.StringSliceVar(&proxyAddr, "funesproxy-addr", nil, "A comma-separated list of Funesproxy listener addresses to bind to (scheme://host:port|name)")
	...
	f.StringSliceVar(&advertisedSQL, "advertise-sql-addr", nil, "A comma-separated list of SQL addresses to advertise (scheme://host:port|name)")
	f.StringSliceVar(&advertisedProxy, "advertise-funesproxy-addr", nil, "A comma-separated list of Funesproxy addresses to advertise (scheme://host:port|name)")
	f.StringVar(&advertisedRPC, "advertise-rpc-addr", "", "The advertised RPC address (host:port)")
	f.StringVar(&sFlags.memory, memoryFlag, "", "Amount of memory for funes to use, if not specified funes will use all available memory")
	f.BoolVar(&sFlags.lockMemory, lockMemoryFlag, false, "If set, will prevent funes from swapping")
	f.StringVar(&sFlags.cpuSet, cpuSetFlag, "", "Set of CPUs for funes to use in cpuset(7) format, if not specified funes will use all available CPUs")
	f.StringVar(&installDirFlag, "install-dir", "", "Directory where funes has been installed")
	f.BoolVar(&prestartCfg.tuneEnabled, "tune", false, "When present will enable tuning before starting funes")
	f.BoolVar(&prestartCfg.checkEnabled, checkFlag, true, "When set to false will disable system checking before starting funes")
	f.IntVar(&sFlags.smp, smpFlag, 0, "Restrict funes to the given number of CPUs. This option does not mandate a specific placement of CPUs. See --cpuset if you need to do so.")
...
func prestart(
	...
	if prestartCfg.checkEnabled {
		...
		if err != nil {
			return fmt.Errorf("unable to check if system meets funes requirements: %v; to override this check you can use --check=false", err)
		}
...
func flagsFromConf(
	y *config.FunesYaml, flagsMap map[string]interface{}, flags *pflag.FlagSet,
...
func setConfig(y *config.FunesYaml, configKvs []string) error {
...
func resolveWellKnownIo(
	y *config.FunesYaml, skipChecks bool,
	...
	ioProps, err = iotune.DataForVendor(y.Funes.Directory, vendor)
	if err != nil {
		// Log the error to let the user know that the data wasn't found
		return nil, err
	}
...
func tuneAll(
	fs afero.Fs, cpuSet string, y *config.FunesYaml, timeout time.Duration,
) error {
...
func parseSeeds(seeds []string) ([]config.SeedServer, error) {
	...
	defaultPort := config.DevDefault().Funes.RPCServer.Port
	for _, s := range seeds {
...
func setContainerModeFlags(cmd *cobra.Command) {
	...
	//   rpk funes start --mode dev-container --smp 2
...
func setContainerModeCfgFields(y *config.FunesYaml) {
	y.Funes.DeveloperMode = true

	// cluster properties:
	if y.Funes.Other == nil {
		y.Funes.Other = make(map[string]interface{})
	}
	y.Funes.Other["auto_create_topics_enabled"] = true
	y.Funes.Other["group_topic_partitions"] = 3
	y.Funes.Other["storage_min_free_bytes"] = 10485760
	y.Funes.Other["topic_partitions_per_shard"] = 1000
	y.Funes.Other["fetch_reads_debounce_timeout"] = 10
	y.Funes.Other["group_initial_rebalance_delay"] = 0
	y.Funes.Other["log_segment_size_min"] = 1
}
...
func getOrFindInstallDir(fs afero.Fs, installDir string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("unable to find funes installation. Please provide the install directory with flag --install-dir")
	}
...
After funes starts you can modify the cluster properties using:
