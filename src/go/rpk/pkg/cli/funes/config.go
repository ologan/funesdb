...
func set(fs afero.Fs, p *config.Params) *cobra.Command {
	c := &cobra.Command{
		Use:   "set [KEY] [VALUE]",
		Short: "Set configuration values, such as the funes node ID or the list of seed servers",
		Long: `Set configuration values, such as the funes node ID or the list of seed servers

This command modifies the funes.yaml you have locally on disk. The first
...
would like to set. Nested fields can be accessed through a dot:
  ...
  rpk funes config set funes.developer_mode true
...
be set:
  ...
  rpk funes config set rpk.tune_disk_irq true
  rpk funes config set funes.rpc_server '{address: 3.250.158.1, port: 9092}'

You can set an entire array by wrapping all items with braces, or by using one
struct:

  rpk funes config set funes.advertised_sql_api '[{address: 0.0.0.0, port: 9092}]'
  rpk funes config set funes.advertised_sql_api '{address: 0.0.0.0, port: 9092}' # same

Indexing can be used to set specific items in an array. You can index one past
the end of an array to extend it:

  rpk funes config set funes.advertised_sql_api[1] '{address: 0.0.0.0, port: 9092}'

You may also use <key>=<value> notation for setting configuration properties:

  rpk funes config set funes.sql_api[0].port=9092
`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(args) == 1 && strings.Contains(args[0], "=") {
				...
				out.Die("invalid arguments: %v, please use one of 'rpk funes config set <key> <value>' or 'rpk funes config set <key>=<value>'", args)
			}
			y := cfg.ActualFunesYamlOrDefaults() // we set fields in the raw file without writing env / flag overrides
...
This command generates a funes.yaml configuration file to bootstrap a
...
By default, funes expects your machine to have one private IP address, and
funes will listen on it. If your machine has multiple private IP addresses,
you must use the --self flag to specify which ip funes should listen on.
`,
		Args: cobra.ExactArgs(0),
		Run: func(cmd *cobra.Command, args []string) {
			cfg, err := p.Load(fs)
			out.MaybeDie(err, "unable to load config: %v", err)
			y := cfg.ActualFunesYamlOrDefaults() // we modify fields in the raw file without writing env / flag overrides
			...
			// Defaults returns one RPC, one SQLAPI, and one
			...
			if len(defaults.Funes.SQLAPI) != 1 || len(defaults.Funes.AdminAPI) != 1 {
				panic("defaults now have more than one sql / admin api address, bug!")
			}

			if a := &y.Funes.RPCServer.Address; *a == config.DefaultListenAddress {
				*a = selfIP
			}
			if a := &y.Funes.SQLAPI; len(*a) == 1 {
	...
	c.Flags().StringVar(&self, "self", "", "Optional IP address for funes to listen on; if empty, defaults to a private address")
	c.Flags().IntVar(&id, "id", -1, "This node's ID. If unset, funes will assign one automatically")
