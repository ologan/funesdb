...
`,
...

		{
			...
			// sql_api address is default, port modified
...
`,
...

		{
			...
			// sql_api modified, 0.0.0.0 kept because multiple of elements
...
`,
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.cfgFile != "" {
				err := afero.WriteFile(fs, config.DefaultFunesYamlPath, []byte(test.cfgFile), 0o644)
				if err != nil {
...
schema_registry: {}
`,
			args: []string{"funes.rack", "funes-rack"},
		}, {
...
`,
			...
			args: []string{"funes.rpc_server.port=-1"},
			cfgFile: `funes:
    data_directory: data/dir
    rack: funes-rack
    seed_servers: []
...
`,
		...
		// We create a config file in default funes location
		if test.cfgFile != "" {
			err := afero.WriteFile(fs, "/etc/funes/funes.yaml", []byte(test.cfgFile), 0o644)
