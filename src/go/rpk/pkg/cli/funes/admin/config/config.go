// Package config contains commands to talk to Funes's admin config
// endpoints.
//
...
func NewCommand(fs afero.Fs, p *config.Params) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
		Short: "View or modify Funes configuration through the admin listener",
...
func newPrintCommand(fs afero.Fs, p *config.Params) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:   "Display the current Funes configuration",
		Args:    cobra.ExactArgs(0),
...
// 'rpk funes admin config log-level set'.
...
This command temporarily changes a broker logger's log level. Each Funes
...
independently update your funes installations from rpk. The success or
...
`,
	...
	cmd.Flags().IntVarP(&expirySeconds, "expiry-seconds", "e", 300, "seconds to persist this log level override before funes reverts to its previous settings (if 0, persist until shutdown)")
...
// To generate this list, run 'funes --help-loggers'.
