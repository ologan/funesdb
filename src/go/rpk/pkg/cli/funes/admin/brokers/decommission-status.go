...
$ rpk funes admin brokers decommission-status 4
DECOMMISSION PROGRESS
=====================
NAMESPACE-TOPIC              PARTITION  MOVING-TO  COMPLETION-%  PARTITION-SIZE
sql/test                   0          3          9             1699470920
sql/test                   4          3          0             1614258779
sql/test2                  3          3          3             2722706514
sql/test2                  4          3          4             2945518089
sql_internal/id_allocator  0          3          0             3562

Using --detailed / -d, it additionally prints granular reports.

$ rpk funes admin brokers decommission-status 4 -d
DECOMMISSION PROGRESS
=====================
NAMESPACE-TOPIC  PARTITION  MOVING-TO  COMPLETION-%  PARTITION-SIZE  BYTES-MOVED  BYTES-REMAINING
sql/test       0          3          13            1731773243      228114727    1503658516
sql/test       4          3          1             1645952961      18752660     1627200301
sql/test2      3          3          5             2752632301      140975805    2611656496
sql/test2      4          3          6             2975443783      181581219    2793862564
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if dbs.Finished {
				if dbs.ReplicasLeft == 0 {
					...
					out.Exit("Node %d is decommissioned but there are %d replicas left, which may be an issue inside Funes. Please describe how you encountered this at https://github.com/redpanda-data/redpanda/issues/new?assignees=&labels=kind%2Fbug&template=01_bug_report.md", broker, dbs.ReplicasLeft)
				}
			}
