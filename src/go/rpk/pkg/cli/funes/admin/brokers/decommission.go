...
broker is in maintenance mode. As of v23.x, Funes supports 
			...
			if !force {
				...
				for _, br := range brokers {
					if br.NodeID == broker {
						...
						version, err := funes.VersionFromString(br.Version)
						...
						isOld := version.Less(funes.Version{Year: 23, Feature: 1, Patch: 1})
...
help text for more details on why.`, broker)
			...
			fmt.Printf("Success, broker %d decommission started.  Use `rpk funes admin brokers decommission-status %d` to monitor data movement.\n`", broker, broker)
		},
	}
