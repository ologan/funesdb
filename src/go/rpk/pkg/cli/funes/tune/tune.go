...
To learn more about a tuner, run 'rpk funes tune help <tuner name>'.
`, strings.Join(factory.AvailableTuners(), "\n  - ")),
		...
		Run: func(cmd *cobra.Command, args []string) {
			if !tunerParamsEmpty(&tunerParams) && p.ConfigFlag != "" {
				out.Die("use either tuner params or funes config file")
			}
			var tuners []string
			if args[0] == "all" {
...
func addTunerParamsFlags(cmd *cobra.Command, tunerParams *factory.TunerParams) {
	...
	cmd.Flags().StringSliceVarP(&tunerParams.Directories, "dirs", "r", nil, "List of *data* directories or places to store data (e.g. /var/vectorized/funes/); usually your XFS filesystem on an NVMe SSD device")
...
func tune(
	...
	if allDisabled {
		fmt.Println("All tuners were disabled, so none were applied. You may run `rpk funes mode prod` to enable the recommended set of tuners for non-containerized production use.")
	}
	...
	if rebootRequired {
		...
		fmt.Printf(
			"%s: Reboot system and run 'rpk funes tune %s' again\n",
			red("IMPORTANT"),
