...
func NewStopCommand(fs afero.Fs, p *config.Params) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "stop",
		Short: "Stop funes",
		Long: `Stop a local funes process. 'rpk stop'
first sends SIGINT, and waits for the specified timeout. Then, if funes
...
running.`,
	...
	cmd.Flags().DurationVar(&timeout, "timeout", 5*time.Second, "The maximum amount of time to wait for funes to stop after each signal is sent (e.g. 300ms, 1.5s, 2h45m)")
	return cmd
}

func executeStop(fs afero.Fs, y *config.FunesYaml, timeout time.Duration) error {
	pidFile := y.PIDFile()
	...
	if !isLocked {
		...
		// funes isn't running, so there's nothing to do.
		zap.L().Sugar().Debugf(
			"'%s' isn't locked, which means funes isn't running. Nothing to do.",
			pidFile,
		)
		return nil
	}
