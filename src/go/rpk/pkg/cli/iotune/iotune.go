...
func NewCommand(fs afero.Fs, p *config.Params) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(directories) != 0 {
				...
				evalDirectories = []string{y.Funes.Directory}
			}
	...
	cmd.Flags().StringVar(&outputFile, "out", filepath.Join(filepath.Dir(config.DefaultFunesYamlPath), "io-config.yaml"), "The file path where the IO config will be written")
