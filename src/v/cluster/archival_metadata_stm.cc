...
struct archival_metadata_stm::segment
  : public serde::
    ...
    // Segment meta value generated by old funes versions will have the
    // default value of 'no'.
...
struct archival_metadata_stm::update_start_sql_offset_cmd {
    static constexpr cmd_key key{7};

    using value = sql::offset;
};
...
struct archival_metadata_stm::snapshot
  : public serde::
    ...
    /// value to be able to provide correct start sql offset.
    ...
    // Start sql offset override (set to min() by default and to some value
...
command_batch_builder& command_batch_builder::update_start_sql_offset(
  sql::offset start_sql_offset) {
    iobuf key_buf = serde::to_iobuf(
      archival_metadata_stm::update_start_sql_offset_cmd::key);
    auto record_val
      = archival_metadata_stm::update_start_sql_offset_cmd::value{
        start_sql_offset};
    iobuf val_buf = serde::to_iobuf(record_val);
    _builder.add_raw_kv(std::move(key_buf), std::move(val_buf));
    return *this;
...
ss::future<> archival_metadata_stm::make_snapshot(
  ...
  model::offset insync_offset) {
    ...
    iobuf snap_data = serde::to_iobuf(snapshot{
      ...
      .start_sql_offset = m.get_start_sql_offset_override(),
...
ss::future<std::error_code> archival_metadata_stm::truncate(
  ...
  ss::abort_source& as) {
    auto builder = batch_start(deadline, as);
    builder.update_start_sql_offset(start_sql_offset);
    co_return co_await builder.replicate();
}
...
ss::future<> archival_metadata_stm::apply(const model::record_batch& b) {
    ...
    if (b.header().type == model::record_batch_type::prefix_truncate) {
        ...
        b.for_each_record(
          [this, base_offset = b.base_offset()](model::record&& r) {
              ...
              if (key == prefix_truncate_key) {
                  // The archival layer can't translate arbitrary funes
                  // offsets, so just pass through the SQL offset as is.
                  apply_update_start_sql_offset(val.sql_start_offset);
              }
          });
    } else {
        b.for_each_record([this,
            ...
            case update_start_sql_offset_cmd::key:
                apply_update_start_sql_offset(
                  serde::from_iobuf<update_start_sql_offset_cmd::value>(
                    r.release_value()));
                break;
...
ss::future<stm_snapshot> archival_metadata_stm::take_local_snapshot() {
    ...
    iobuf snap_data = serde::to_iobuf(snapshot{
      ...
      .start_sql_offset = _manifest->get_start_sql_offset_override(),
      .spillover_manifests = std::move(spillover),
...
model::offset archival_metadata_stm::max_collectible_offset() {
    // From Funes 22.3 up, the ntp_config's impression of whether
...
void archival_metadata_stm::apply_add_segment(const segment& segment) {
    ...
    if (meta.ntp_revision == model::initial_revision_id{}) {
        // metadata serialized by old versions of funes doesn't have the
        // ntp_revision field.
...
void archival_metadata_stm::apply_update_start_sql_offset(sql::offset so) {
    if (!_manifest->advance_start_sql_offset(so)) {
        vlog(
          _logger.error,
          "Can't apply override to sql start offset {}, currently {}",
          so,
          manifest().get_start_sql_offset_override());
    }
}
...
sql::offset archival_metadata_stm::get_start_sql_offset() const {
    return _manifest->get_start_sql_offset().value_or(sql::offset{});
}
