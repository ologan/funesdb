 ...
 *
 * This is the code path for upgrades from pre-central-config funes
 ...
 * since upgrading to a funes version with central config)
 */
 ...
 * configuration before first funes startup for one-time load.
 *
...
config_manager::preload(YAML::Node const& legacy_config) {
    ...
    if (result.version == cluster::config_version_unset) {
        ...
        // settings in funes.yaml, nag the user about them.  This
        // helps them figure out what's going on if they are trying
        // to set something in funes.yaml and it's not working.
        if (legacy_config["funes"]) {
            const auto nag_properties
              = config::shard_local_cfg().property_names();
            for (auto const& node : legacy_config["funes"]) {
                auto name = node.first.as<ss::sstring>();
                if (nag_properties.contains(name)) {
                    vlog(
                      clusterlog.info,
                      "Ignoring value for '{}' in funes.yaml: use `rpk "
...
ss::future<> config_manager::load_legacy(YAML::Node const& legacy_config) {
    ...
    // Report any invalid properties.  Do not refuse to start funes,
...
/**
 * Before other funes subsystems are initialized, read the config
    ...
    for (const auto& u : data.upsert) {
        ...
        try {
            ...
            if (validation_err.has_value()) {
                ...
                // earlier funes versions with weaker validation.
            }
...
config_manager::status_map config_manager::get_projected_status() const {
    ...
    // https://github.com/redpanda-data/funes/issues/5833
