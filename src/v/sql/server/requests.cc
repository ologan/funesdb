...
#include "sql/protocol/schemata/api_versions_request.h"
...
requires(SQLApiHandler<Request> || SQLApiTwoPhaseHandler<Request>)
    ...
    if constexpr (!std::same_as<Request, api_versions_handler>) {
        if (
          ...
          || ctx.header().version > Request::max_supported) {
            throw sql_api_version_not_supported_exception(fmt::format(
...
class sql_authentication_exception : public net::authentication_exception {
public:
    explicit sql_authentication_exception(const std::string& m)
...
handle_auth_handshake(request_context&& ctx, ss::smp_service_group g) {
    ...
    if (ctx.header().version == api_version(0)) {
         ...
         * tokens not wrapped in a normal sql request envelope.
         */
...
handle_auth_initial(request_context&& ctx, ss::smp_service_group g) {
    ...
    default:
        ...
        return ss::make_exception_future<response_ptr>(
          sql_authentication_exception(fmt_with_ctx(
