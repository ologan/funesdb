...
class replicated_partition final : public sql::partition_proxy::impl {
public:
    ...
    sync_effective_start(model::timeout_clock::duration timeout) final {
        auto synced_start_offset_override
          = co_await _partition->sync_sql_start_offset_override(timeout);
        ...
        co_return sql_start_offset_with_override(
          synced_start_offset_override.value());
    }

    model::offset start_offset() const final {
        const auto start_offset_override
          = _partition->sql_start_offset_override();
        if (!start_offset_override.has_value()) {
            return partition_sql_start_offset();
        }
        return sql_start_offset_with_override(start_offset_override.value());
    }
    ...
    /**
     * According to SQL protocol semantics a log_end_offset is an offset that
...
private:
    // Returns the SQL offset corresponding to the lowest offset in the
    // log, including local and cloud storage. Doesn't take into account any
    // start offset overrides (see start_offset()).
    model::offset partition_sql_start_offset() const {
        if (
        ...
        if (
          ...
          && _partition->cloud_data_available()
          && (_partition->start_cloud_offset() < local_sql_start_offset)) {
            return _partition->start_cloud_offset();
        }
        return local_sql_start_offset;
    }

    model::offset sql_start_offset_with_override(
      model::offset start_sql_offset_override) const {
        if (start_sql_offset_override == model::offset{}) {
            return partition_sql_start_offset();
        }
        if (_partition->is_read_replica_mode_enabled()) {
            ...
            if (hwm <= start_sql_offset_override) {
                return hwm;
            }
        }
        return std::max(
          partition_sql_start_offset(), start_sql_offset_override);
    }

    // Returns the highest offset in the given term, without considering
    // overrides of the starting offset.
    ss::future<std::optional<model::offset>>
      get_leader_epoch_last_offset_unbounded(sql::leader_epoch) const;
    ...
    bool may_read_from_cloud(sql::offset);
