...
#include "sql/server/protocol_utils.h"
...
connection_context::connection_context(
  ...
  config::conversion_binding<std::vector<bool>, std::vector<ss::sstring>>
    sql_throughput_controlled_api_keys) noexcept
  : _server(s)
  , conn(conn)
  , _as()
  , _sasl(std::move(sasl))
  ...
  , _sql_throughput_controlled_api_keys(
      std::move(sql_throughput_controlled_api_keys)) {}
...
ss::future<> connection_context::process_one_request() {
    ...
    } catch (const sql_api_version_not_supported_exception& e) {
        vlog(
          klog.warn,
 ...
 * authentication string. https://github.com/redpanda-data/funes/issues/1315.
...
ss::future<> connection_context::handle_auth_v0(const size_t size) {
    ...
    sasl_authenticate_response response;
    {
        ...
        auto resp = co_await sql::process_request(
...
connection_context::record_tp_and_calculate_throttle(
  const request_header& hdr, const size_t request_size) {
    ...
    if (_sql_throughput_controlled_api_keys().at(hdr.key)) {
...
ss::future<session_resources> connection_context::throttle_request(
  const request_header& hdr, size_t request_size) {
    ...
    // delayed. this is a similar strategy used by sql 2.0: the
 ...
 * may become ready out of order, but SQL clients expect responses exactly in
 * request order.
 *
...
ss::future<> connection_context::maybe_process_responses() {
    return ss::repeat([this]() mutable {
        ...
        // drastic changes to sql message processing framework - because
        ...
        if (_sql_throughput_controlled_api_keys().at(request_key)) {
