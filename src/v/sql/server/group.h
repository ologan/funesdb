...
#include "sql/group_probe.h"
#include "sql/protocol/fwd.h"
...
/**
 * \defgroup sql-groups SQL group membership API
 *
 * The SQL API defines a group membership protocol for distributing and
 ...
 * feature in SQL for distributing the work of consuming a topic across the
 * members in the group.
 *
 * The group membership API is generic and can be used by SQL clients to
 * build other group membership-based features. For instance, in addition to
 * consumer groups, the group membership API is used to implement SQL Connect
 * that aids in connecting SQL with external data sources.
 *
 ...
 * to their counterparts in the SQL implementation. This equivalence has
...
ss::sstring group_state_to_sql_name(group_state);
...
sql::error_code map_store_offset_error_code(std::error_code);
...
public:
    ...
    struct offset_metadata {
        ...
        model::offset offset;
        ss::sstring metadata;
        sql::leader_epoch committed_leader_epoch;
        model::timestamp commit_timestamp;
        std::optional<model::timestamp> expiry_timestamp;
        /*
         * this is an offset that was written prior to upgrading to funes
    ...
    /// Get the group id.
    const sql::group_id& id() const { return _id; }
    ...
    sql::generation_id generation() const { return _generation; }
     ...
     */
    member_ptr get_member(const sql::member_id& id) const {
        return _members.at(id);
    }

    /// Check if the group contains a member.
    bool contains_member(const sql::member_id& member_id) const {
    ...
    void add_pending_member(const sql::member_id&, duration_type);

    /// Check if the group contains a pending member.
    bool contains_pending_member(const sql::member_id& member) const {
    ...
    void remove_pending_member(sql::member_id member_id);

    /// Check if a member id refers to the group leader.
    bool is_leader(const sql::member_id& member_id) const {
    ...
    const std::optional<sql::protocol_type>& protocol_type() const {
        return _protocol_type;
    }

    /// Get the group's configured protocol (if any).
    const std::optional<sql::protocol_name>& protocol() const {
        return _protocol;
    }
     ...
     */
    sql::protocol_name select_protocol() const;
     ...
     */
    static sql::member_id generate_member_id(const join_group_request& r);
    ...
    join_group_stages add_member_and_rebalance(
      sql::member_id member_id, join_group_request&& request);
    ...
    sql::error_code member_leave_group(
    ...
    // helper for the sql api: describe groups
...
private:
    using member_map = absl::node_hash_map<sql::member_id, member_ptr>;
    using protocol_support = absl::node_hash_map<sql::protocol_name, int>;
    ...
    model::record_batch do_checkpoint(Func&& assignments_provider) {
        sql::group_metadata_key key;
        key.group_id = _id;
        sql::group_metadata_value metadata;
    ...
    ss::future<cluster::commit_group_tx_reply>
    do_commit(sql::group_id group_id, model::producer_identity pid);
    ...
    absl::node_hash_map<sql::member_id, ss::timer<clock_type>>
      _pending_members;
    std::optional<sql::protocol_type> _protocol_type;
    std::optional<sql::protocol_name> _protocol;
    std::optional<sql::member_id> _leader;
