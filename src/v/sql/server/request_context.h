...
skip_auditing(api_key key, const security::acl_principal& principal) {
    return security::audit::sql_api_to_event_type(key)
...
public:
    ...
    sql::usage_manager& usage_mgr() const {
    ...
    const std::unique_ptr<funesproxy::schema_registry::api>&
    ...
    sql::group_router& groups() { return _conn->server().group_router(); }
    ...
    ss::future<response_ptr> respond(ResponseType r) {
        ...
        if constexpr (std::is_same_v<ResponseType, api_versions_response>) {
            ...
            if (r.data.error_code == sql::error_code::unsupported_version) {
    ...
    security::auth_result do_authorized(
      ...
      authz_quiet quiet = authz_quiet{false}) {
        if constexpr (std::is_same_v<T, model::topic>) {
            if (name == model::sql_audit_logging_topic) [[unlikely]] {
