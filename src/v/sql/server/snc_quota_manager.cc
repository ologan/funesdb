...
void snc_quotas_probe::setup_metrics() {
    ...
    metric_defs.emplace_back(sm::make_counter(
      "traffic_intake",
      _traffic_in,
      sm::description("Amount of SQL traffic received from the clients "
                      "that is taken into processing, in bytes")));

    _metrics.add_group(
      prometheus_sanitize::metrics_name("sql:quotas"),
      metric_defs,
      {},
      {sm::shard_label});
}
...
snc_quota_manager::snc_quota_manager()
  : _max_sql_throttle_delay(
    config::shard_local_cfg().max_sql_throttle_delay_ms.bind())
  , _sql_throughput_limit_node_bps{
      config::shard_local_cfg().sql_throughput_limit_node_in_bps.bind(),
      config::shard_local_cfg().sql_throughput_limit_node_out_bps.bind()}
  , _sql_quota_balancer_window(
      config::shard_local_cfg().sql_quota_balancer_window.bind())
  , _sql_quota_balancer_node_period(
      config::shard_local_cfg().sql_quota_balancer_node_period.bind())
  , _sql_quota_balancer_min_shard_throughput_ratio(
      config::shard_local_cfg()
        .sql_quota_balancer_min_shard_throughput_ratio.bind())
  , _sql_quota_balancer_min_shard_throughput_bps(
      config::shard_local_cfg()
        .sql_quota_balancer_min_shard_throughput_bps.bind())
  , _sql_throughput_control(config::shard_local_cfg().sql_throughput_control.bind())
  ...
  , _probe(*this)
{
    ...
    _sql_throughput_limit_node_bps.in.watch(
      [this] { update_node_quota_default(); });
    _sql_throughput_limit_node_bps.eg.watch(
      [this] { update_node_quota_default(); });
    _sql_quota_balancer_window.watch([this] {
        const auto v = _sql_quota_balancer_window();
        vlog(klog.debug, "qm - Set shard TP token bucket window: {}", v);
        _shard_quota.in.set_width(v);
        _shard_quota.eg.set_width(v);
    });
    _sql_quota_balancer_node_period.watch([this] {
    ...
    _sql_quota_balancer_min_shard_throughput_ratio.watch(
      [this] { update_shard_quota_minimum(); });
    _sql_quota_balancer_min_shard_throughput_bps.watch(
      [this] { update_shard_quota_minimum(); });
...
snc_quota_manager::calc_node_quota_default() const {
    ...
    const ingress_egress_state<std::optional<quota_t>> default_quota{
      .in = _sql_throughput_limit_node_bps.in(),
      .eg = _sql_throughput_limit_node_bps.eg()};
...
void snc_quota_manager::get_or_create_quota_context(
  ...
  std::optional<std::string_view> client_id) {
    ...
    const auto tcgroup_it = config::find_throughput_control_group(
      _sql_throughput_control().cbegin(),
      _sql_throughput_control().cend(),
      client_id);
    if (tcgroup_it == _sql_throughput_control().cend()) {
        ctx->_exempt = false;
        ...
        if (tcgroup_it->is_noname()) {
            vlog(
              ...
              std::distance(_sql_throughput_control().cbegin(), tcgroup_it));
        } else {
            vlog(
              klog.debug,
              "qm - Assigned throughput control group: {}",
...
snc_quota_manager::get_quota_balancer_node_period() const {
    const auto v = _sql_quota_balancer_node_period();
...
void snc_quota_manager::update_node_quota_default() {
    ...
    if (ss::this_shard_id() == quota_balancer_shard) {
        ...
        if (_sql_quota_balancer_node_period() == 0ms) {
...
template<class T>
struct fmt::formatter<sql::ingress_egress_state<T>> : parseless_formatter {
    template<typename Ctx>
    auto format(const sql::ingress_egress_state<T>& p, Ctx& ctx) const {
        return fmt::format_to(ctx.out(), "(in:{}, eg:{})", p.in, p.eg);
    }
};

template<>
struct fmt::formatter<sql::borrower_t> : parseless_formatter {
    template<typename Ctx>
    auto format(const sql::borrower_t& v, Ctx& ctx) const {
