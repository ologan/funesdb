...
struct fetch_session_partition {
    ...
    sql::leader_epoch current_leader_epoch = invalid_leader_epoch;
};
/**
...
private:
    struct entry {
        explicit entry(sql::fetch_session_partition partition)
          : partition(std::move(partition)) {}

        sql::fetch_session_partition partition;
    ...
    static auto make_partition_iterator(io_list_t::const_iterator it) {
        return boost::iterators::make_transform_iterator(
          it, [](const entry& e) -> const sql::fetch_session_partition& {
              return e.partition;
          });
    }
...
public:
    ...
    void emplace(sql::fetch_session_partition v) {
