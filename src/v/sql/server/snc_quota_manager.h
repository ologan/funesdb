...
public:
    ...
    /// lifetime. However since the sql API allows changing client_id of a
...
private:
    // configuration
    config::binding<std::chrono::milliseconds> _max_sql_throttle_delay;
    ingress_egress_state<config::binding<std::optional<quota_t>>>
      _sql_throughput_limit_node_bps;
    config::binding<std::chrono::milliseconds> _sql_quota_balancer_window;
    config::binding<std::chrono::milliseconds>
      _sql_quota_balancer_node_period;
    config::binding<double> _sql_quota_balancer_min_shard_throughput_ratio;
    config::binding<quota_t> _sql_quota_balancer_min_shard_throughput_bps;
    config::binding<std::vector<config::throughput_control_group>>
      _sql_throughput_control;
