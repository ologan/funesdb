  ...
  std::optional<model::timeout_clock::time_point> debounce_deadline) {
    ...
    public:
        ...
        void print(std::ostream& os) final {
            fmt::print(os, "sql::partition reader for ");
...
replicated_partition::aborted_transactions_local(
  ...
  ss::lw_shared_ptr<const storage::offset_translator_state> ot_state) {
    ...
    // eviction point). See
    // https://github.com/redpanda-data/funes/issues/3001
 ...
 */
bool replicated_partition::may_read_from_cloud(sql::offset start_offset) {
  ...
  ss::lw_shared_ptr<const storage::offset_translator_state> ot_state) {
    ...
    if (may_read_from_cloud(model::offset_cast(base))) {
        ...
        if (!tx_remote.empty()) {
            ...
            // for segments which was uploaded by old funes version because
            ...
            // funes. In this case we will try to fetch the aborted
...
replicated_partition::timequery(storage::timequery_config cfg) {
    // cluster::partition::timequery returns a result in SQL offsets,
    // no further offset translation is required here.
    auto res = co_await _partition->timequery(cfg);
    if (!res.has_value()) {
        co_return std::nullopt;
    }
    const auto sql_start_override = _partition->sql_start_offset_override();
    if (
      !sql_start_override.has_value()
      || sql_start_override.value() <= res.value().offset) {
  ...
  model::record_batch_reader rdr, raft::replicate_options opts) {
    ...
    if (_partition->is_read_replica_mode_enabled()) {
        return ss::make_ready_future<ret_t>(
          sql::error_code::invalid_topic_exception);
    }
    return _partition->replicate(std::move(rdr), opts)
      .then([](result<cluster::sql_result> r) {
          if (!r) {
    ...
    if (_partition->is_read_replica_mode_enabled()) {
        return {
          ...
          ss::make_ready_future<result<raft::replicate_result>>(
            make_error_code(sql::error_code::invalid_topic_exception))};
    }
...
replicated_partition::get_leader_epoch_last_offset_unbounded(
  sql::leader_epoch epoch) const {
    ...
    // in the next term. This mirrors behavior in SQL, see
    // https://github.com/apache/sql/blob/97105a8e5812135515f5a0fa4d5ff554d80df2fe/storage/src/main/java/org/apache/sql/storage/internals/epoch/LeaderEpochFileCache.java#L255-L281
...
ss::future<error_code> replicated_partition::prefix_truncate(
  model::offset sql_truncation_offset,
  ss::lowres_clock::time_point deadline) {
    if (
      sql_truncation_offset <= start_offset()
      || sql_truncation_offset > high_watermark()) {
        co_return error_code::offset_out_of_range;
    }
    model::offset rp_truncate_offset{};
    auto local_sql_start_offset = _translator->from_log_offset(
      _partition->raft_start_offset());
    if (sql_truncation_offset > local_sql_start_offset) {
        rp_truncate_offset = _translator->to_log_offset(
          sql_truncation_offset);
    }
    auto errc = co_await _partition->prefix_truncate(
      rp_truncate_offset,
      model::offset_cast(sql_truncation_offset),
      deadline);

    if (errc.category() == raft::error_category()) {
        switch (raft::errc(errc.value())) {
        case raft::errc::success:
            co_return sql::error_code::none;
        case raft::errc::not_leader:
            co_return sql::error_code::not_leader_for_partition;
        case raft::errc::shutting_down:
            ...
            co_return sql::error_code::request_timed_out;
        default:
            vlog(
