...
group_metadata_key group_metadata_key::decode(protocol::decoder& reader) {
    ...
    vassert(
      ...
      version);
    ret.group_id = sql::group_id(reader.read_string());
    return ret;
}
...
member_state member_state::decode(protocol::decoder& reader) {
    ...
    ret.client_id = sql::client_id(reader.read_string());
    ret.client_host = sql::client_host(reader.read_string());
...
group_metadata_value group_metadata_value::decode(protocol::decoder& reader) {
    ...
    ret.protocol_type = sql::protocol_type(reader.read_string());
    ret.generation = sql::generation_id(reader.read_int32());
    auto protocol_opt = reader.read_nullable_string();
    if (protocol_opt) {
        ret.protocol = sql::protocol_name(std::move(protocol_opt.value()));
    }
    auto leader_opt = reader.read_nullable_string();
    if (leader_opt) {
        ret.leader = sql::member_id(std::move(leader_opt.value()));
    }
...
offset_metadata_value offset_metadata_value::decode(protocol::decoder& reader) {
    ...
    if (version >= group_metadata_version{3}) {
        ret.leader_epoch = sql::leader_epoch(reader.read_int32());
    }
...
iobuf maybe_unwrap_from_iobuf(iobuf buffer) {
    /*
     * Previously in funes we had an error leading to wrapping
     ...
     *
     *
     * In SQL the only allowed values for group metadata versions are
     ...
     *
     * When serialized with previous `sql_internal/group` serializer a key was
     * represented by
     *
    ...
    if (unlikely(
        ...
        vlog(sql::klog.debug, "Unwrapping group metadata key from iobuf");
        // unwrap from iobuf
