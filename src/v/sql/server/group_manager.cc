...
#include "sql/protocol/delete_groups.h"
...
#include "sql/server/group_recovery_consumer.h"
...
void group_manager::handle_topic_delta(
  cluster::topic_table::delta_range_t deltas) {
    // topic-partition deletions in the sql namespace are the only deltas that
    ...
    for (const auto& delta : deltas) {
        if (
          ...
          && delta.ntp().ns == model::sql_namespace) {
...
ss::future<> group_manager::handle_partition_leader_change(
  ...
  std::optional<model::node_id> leader_id) {
    ...
    auto timeout
      = ss::lowres_clock::now()
        + config::shard_local_cfg().sql_group_recovery_timeout_ms();
    /*
...
ss::future<> group_manager::do_recover_group(
  ...
  group_stm group_stm) {
    if (group_stm.has_data()) {
        ...
        if (!group) {
            group = ss::make_lw_shared<sql::group>(
              group_id,
...
group::join_group_stages group_manager::join_group(join_group_request&& r) {
    ...
    if (!group) {
        // <sql>only try to create the group if the group is UNKNOWN AND
        // the member id is UNKNOWN, if member is specified but group does
        // not exist we should reject the request.</sql>
...
group::sync_group_stages group_manager::sync_group(sync_group_request&& r) {
    ...
    if (error != error_code::none) {
        if (error == error_code::coordinator_load_in_progress) {
            // <sql>The coordinator is loading, which means we've lost the
...
ss::future<heartbeat_response> group_manager::heartbeat(heartbeat_request&& r) {
    ...
    if (error != error_code::none) {
        if (error == error_code::coordinator_load_in_progress) {
            // <sql>the group is still loading, so respond just
            // blindly</sql>
...
group_manager::txn_offset_commit(txn_offset_commit_request&& r) {
    ...
    return p->catchup_lock->hold_read_lock().then(
      [this, p, r = std::move(r)](ss::basic_rwlock<>::holder unit) mutable {
          ...
          auto group = get_group(r.data.group_id);
          if (!group) {
              // <sql>the group is not relying on SQL for group
              // management, so allow the commit</sql>
...
group_manager::offset_commit(offset_commit_request&& r) {
    ...
    if (!group) {
        if (r.data.generation_id < 0) {
            // <sql>the group is not relying on SQL for group management, so
            // allow the commit</sql>
            ...
            // <sql>or this is a request coming from an older generation.
            // either way, reject the commit</sql>
...
described_group
group_manager::describe_group(const model::ntp& ntp, const sql::group_id& g) {
...
bool group_manager::valid_group_id(const group_id& group, api_key api) {
    ...
    case offset_fetch_api::key:
        // <sql> For backwards compatibility, we support the offset commit
  ...
  const model::ntp& ntp, const group_id& group, api_key api) {
    ...
    if (const auto it = _partitions.find(ntp); it != _partitions.end()) {
        ...
        if (it->second->loading) {
             ...
             * condition, and is what sql brokers return. it should cause a
             * client to retry with backoff. however, it seems to be a rare
             * error condition in sql (not in funes) and the sarama client
