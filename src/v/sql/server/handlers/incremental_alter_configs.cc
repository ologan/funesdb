...
#include "sql/server//handlers/configs/config_utils.h"
 ...
 * the sql API (i.e. those in common with other sql implementations),
 * map their traditional sql name to the funes config property name.
 */
  ...
  request_context& ctx, std::vector<req_resource_t> resources) {
    ...
    for (const auto& resource : resources) {
        ...
        for (const auto& c : resource.configs) {
            // mapping looks sane for sql's properties
            ...
            // `log_compression_type`) but for funes's properties it returns
            //   funes.remote.read=false sensitive=false
            //   synonyms={DEFAULT_CONFIG:funes.remote.read=false}
