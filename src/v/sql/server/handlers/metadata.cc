...
#include "sql/protocol/schemata/metadata_response.h"
#include "sql/server/errors.h"
#include "sql/server/fwd.h"
#include "sql/server/handlers/details/isolated_node_utils.h"
 ...
 *
 * SQL clients request metadata refresh in case they receive error that is
 ...
 * broker to choose to refresh metadata from. (f.e. Java sql client uses the
 ...
 * heuristics:
 *
 * 1) when current leader is unknown, return former leader (SQL behavior)
 *
...
bool is_internal(model::topic_namespace_view tp_ns) {
    return tp_ns == model::sql_consumer_offsets_nt;
}
...
static ss::future<std::vector<metadata_response::topic>> get_topic_metadata(
  ...
  const is_node_isolated_or_decommissioned is_node_isolated) {
    ...
    if (request.list_all_topics) {
        ...
        // outside of sql namespace
        res.reserve(topics_md.size());
        for (const auto& [tp_ns, md] : topics_md) {
            // only serve topics from the sql namespace
            if (tp_ns.ns != model::sql_namespace) {
                continue;
            }
            /*
 ...
 * that is accessible to the client making this request.  A funes
 * cluster for which this is not true is in an invalid configuration
 * and cannot serve SQL on any listener that does not have an equivalent
 * listener on other nodes, because SQL clients have to be able to
 * connect to all brokers.
 *
 * @return pointer to the best guess at which listener on a peer should
 *         be used in sql metadata responses.
 */
...
guess_peer_listener(request_context& ctx, const cluster::node_metadata& nm) {
    ...
    // listener serving this SQL request.  This can happen during
    // configuration changes
    // (https://github.com/redpanda-data/funes/issues/3588)
    //
    ...
    const auto my_listeners = config::node().advertised_sql_api();
    int16_t my_port = 0;
    ...
    for (const auto& listener : nm.broker.sql_advertised_listeners()) {
    ...
    // peer.
    if (!nm.broker.sql_advertised_listeners().empty()) {
        return nm.broker.sql_advertised_listeners()[0];
    } else {
        // A broker with no sql listeners, there is no way to
        // include it in our response
        return std::nullopt;
    }
}

// If node isolated or decomissioned it can not handle sql requests from
...
ss::future<response_ptr> metadata_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group g) {
    ...
    if (cluster_id.has_value()) {
        reply.data.cluster_id = ssx::sformat("funes.{}", cluster_id.value());
    } else {
        // Include a "funes." cluster ID even if we didn't initialize
        // cluster_id yet, so that callers can identify which SQL
        // implementation they're talking to.
        reply.data.cluster_id = "funes.initializing";
    }
...
metadata_memory_estimator(size_t request_size, connection_context& conn_ctx) {
    ...
    for (auto& [tp_ns, topic_metadata] : md_cache.all_topics_metadata()) {
        ...
        size_estimate += sizeof(sql::metadata_response_topic);
        size_estimate += tp_ns.tp().size();

        using partition = sql::metadata_response_partition;
