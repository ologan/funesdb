...
#include "sql/protocol/describe_producers.h"
#include "sql/protocol/errors.h"
#include "sql/protocol/schemata/describe_producers_response.h"
...
using ret_res = checked<cluster::rm_stm::transaction_set, sql::error_code>;
...
ss::future<checked<cluster::rm_stm::transaction_set, sql::error_code>>
get_producers_for_partition(request_context& ctx, model::ntp ntp) {
    ...
    if (!shard) {
        ...
        return ss::make_ready_future<ret_res>(
          sql::error_code::not_leader_for_partition);
    }
...
describe_producers_handler::handle(request_context ctx, ss::smp_service_group) {
    ...
    for (const auto& topic : request.data.topics) {
        ...
        for (const auto& partition : topic.partition_indexes) {
            ...
            model::ntp ntp(model::sql_namespace, topic.name, partition);
