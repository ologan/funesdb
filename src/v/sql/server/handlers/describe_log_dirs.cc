  ...
  const std::optional<std::vector<describable_log_dir_topic>>& topics) {
    ...
    if (!topics) {
        for (const auto& partition : pm.partitions()) {
            if (partition.first.ns != model::sql_namespace) {
                continue;
            }
    ...
    for (const auto& topic : *topics) {
        for (auto p_id : topic.partition_index) {
            model::ntp ntp(model::sql_namespace, topic.topic, p_id);
...
ss::future<response_ptr> describe_log_dirs_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group ssg) {
    ...
    // funes only supports a single data directory right now
    ...
    if (!authz) {
        // in this case sql returns no authorization error
        co_return co_await ctx.respond(std::move(response));
    }
