...
#include "sql/protocol/schemata/delete_acls_response.h"
...
bindings_to_delete_result(const std::vector<security::acl_binding>& bindings) {
    ...
    for (auto& binding : bindings) {
        ...
        // resource pattern
        m.resource_type = details::to_sql_resource_type(
          binding.pattern().resource());
        m.resource_name = binding.pattern().name();
        m.pattern_type = details::to_sql_pattern_type(
          binding.pattern().pattern());

        // acl entry
        m.principal = details::to_sql_principal(binding.entry().principal());
        m.host = details::to_sql_host(binding.entry().host());
        m.operation = details::to_sql_operation(binding.entry().operation());
        m.permission_type = details::to_sql_permission(
