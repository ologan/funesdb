...
std::vector<R> authorize_alter_config_resources(
  request_context& ctx, groupped_resources<T>& to_authorize) {
    ...
    const auto& sql_nodelete_topics
    ...
    auto unauthorized_it = std::partition(
      ...
      [&ctx, &sql_nodelete_topics, &sql_noproduce_topics](const T& res) {
          auto topic = model::topic(res.resource_name);

          auto is_nodelete_topic = std::find(
                                     sql_nodelete_topics.cbegin(),
                                     sql_nodelete_topics.cend(),
                                     topic)
                                   != sql_nodelete_topics.cend();
          ...
          auto is_noproduce_topic = std::find(
                                      sql_noproduce_topics.cbegin(),
                                      sql_noproduce_topics.cend(),
                                      topic)
                                    != sql_noproduce_topics.cend();
...
public:
    ...
    template<typename C>
    bool operator()(C const& cfg, sql::config_resource_operation op) {
        ...
        case funesproxy::schema_registry::schema_id_validation_mode::compat:
            matcher
              .match(
            ...
            [[fallthrough]];
        case funesproxy::schema_registry::schema_id_validation_mode::funes:
            matcher
              .match(
...
private:
    ...
    static void apply(
      ...
      sql::config_resource_operation op) {
        sql::parse_and_set_optional_bool_alpha(prop, value, op);
    }
    ///\brief Parse and set the Subject Name Strategy
    static void apply(
      cluster::property_update<std::optional<
        funesproxy::schema_registry::subject_name_strategy>>& prop,
      std::optional<ss::sstring> const& value,
      sql::config_resource_operation op) {
        sql::parse_and_set_optional(prop, value, op);
    }
