...
// SQL protocol defines integral types by sizes. See
// https://sql.apache.org/protocol.html
...
// Funes so we do not include checks for those types. You may find a similar
// mapping in Apache SQL at
// https://github.com/apache/sql/blob/be032735b39360df1a6de1a7feea8b4336e5bcc0/core/src/main/scala/sql/server/ConfigHelper.scala
template<typename T>
consteval describe_configs_type property_config_type() {
    ...
    constexpr auto is_string_type = std::is_same_v<T, ss::sstring> ||
        ...
        std::is_same_v<T, funesproxy::schema_registry::subject_name_strategy>;
 ...
 * some SQL clients insist that after an AlterConfig RPC, anything
 * they didn't set should be DEFAULT_CONFIG.
 *
 * See https://github.com/redpanda-data/funes/issues/7451
 */
template<typename T>
std::optional<T>
...
static ss::sstring
sql_endpoint_format(const std::vector<model::broker_endpoint>& endpoints) {
...
static ss::sstring sql_authn_endpoint_format(
  ...
  bool include_documentation) {
    ...
    add_broker_config_if_requested(
      ...
      maybe_make_documentation(
        include_documentation, config::node().sql_api.desc()),
      &sql_authn_endpoint_format);

    add_broker_config_if_requested(
      resource,
      result,
      "advertised.listeners",
      config::node().advertised_sql_api_property(),
      include_synonyms,
      maybe_make_documentation(
        include_documentation,
        config::node().advertised_sql_api_property().desc()),
      &sql_endpoint_format);
    ...
    for (auto& resource : request.data.resources) {
        ...
        case config_resource_type::topic: {
            model::topic_namespace topic(
              model::sql_namespace, model::topic(resource.resource_name));

            auto err = model::validate_sql_topic_name(topic.tp);
            if (err) {
            ...
            add_topic_config_if_requested(
              resource,
              result,
              config::shard_local_cfg().sql_batch_max_bytes.name(),
              ...
              maybe_make_documentation(
                request.data.include_documentation,
                config::shard_local_cfg().sql_batch_max_bytes.desc()),
            ...
            case funesproxy::schema_registry::schema_id_validation_mode::
              compat: {
                add_topic_config_if_requested(
                  resource,
                  result,
            ...
            case funesproxy::schema_registry::schema_id_validation_mode::none: {
                break;
            }
            }
