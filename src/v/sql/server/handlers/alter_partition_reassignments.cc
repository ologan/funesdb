...
#include "sql/protocol/schemata/alter_partition_reassignments_response.h"
...
map_errc_to_response(const model::ntp& ntp, std::error_code ec) {
    ...
    if (ec) {
        ...
        if (ec.category() == cluster::error_category()) {
            ...
            response.error_code = sql::error_code::unknown_server_error;
        }
    }
...
static ss::future<response_ptr> do_handle(alter_op_context& octx) {
    if (!config::shard_local_cfg().sql_enable_partition_reassignment()) {
        vlog(
          klog.info,
          "Rejected alter partition reassignment request: API is disabled. See "
          "`sql_enable_partition_reassignment` configuration option");
