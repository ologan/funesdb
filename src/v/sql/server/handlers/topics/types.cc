...
// Special case for options where SQL allows -1
// In funes the mapping is following
//
...
to_cluster_type(const creatable_topic& t) {
    auto cfg = cluster::topic_configuration(
      model::sql_namespace, t.name, t.num_partitions, t.replication_factor);
    ...
    for (auto& p : t.configs) {
        schema_id_validation_config_parser(
          p, sql::config_resource_operation::set);
    }
