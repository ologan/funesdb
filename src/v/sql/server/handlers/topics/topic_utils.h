...
/// All of the SQL Topic-related APIs have the same structure of
...
// SQL protocol error message
void append_cluster_results(
  const std::vector<cluster::topic_result>&,
  std::vector<creatable_topic_result>&);

// Converts objects representing SQLAPI message to objects consumed
// by cluster::controller API
// clang-format off
template<typename SQLApiTypeIter>
requires TopicRequestItem<typename SQLApiTypeIter::value_type> &&
requires(SQLApiTypeIter it) {
    to_cluster_type(*it);
}
// clang-format on
auto to_cluster_type(SQLApiTypeIter begin, SQLApiTypeIter end)
  -> std::vector<decltype(to_cluster_type(*begin))> {
    ...
    std::transform(
      ...
      [](const typename SQLApiTypeIter::value_type& sql_type) {
          return to_cluster_type(sql_type);
      });
...
/// This pattern is used in every Admin request of SQL protocol.
// clang-format off
template<typename Iter, typename ErrIter>
requires TopicRequestItem<typename Iter::value_type> &&
         TopicResultIterator<ErrIter>
