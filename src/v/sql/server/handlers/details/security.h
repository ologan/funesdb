 ...
 * is generally what should be returned as the error message in sql responses.
 *
...
/*
 * convert sql acl message into funes internal acl representation
 */
...
/*
 * convert sql describe acl request into an internal acl filter
 */
...
inline int8_t to_sql_resource_type(security::resource_type type) {
    switch (type) {
...
inline int8_t to_sql_pattern_type(security::pattern_type type) {
    switch (type) {
...
inline int8_t to_sql_operation(security::acl_operation op) {
    switch (op) {
...
inline int8_t to_sql_permission(security::acl_permission perm) {
    switch (perm) {
...
inline ss::sstring to_sql_principal(const security::acl_principal& p) {
    switch (p.type()) {
...
inline ss::sstring to_sql_host(security::acl_host host) {
    if (host.address()) {
...
/**
 * In some SQL APIs, set of allowed operations is encoded as a int32 bit
...
inline int32_t to_bit_field(const std::vector<security::acl_operation>& ops) {
    ...
    for (auto o : ops) {
        auto sql_acl_operation = to_sql_operation(o);
        vassert(
          sql_acl_operation <= available_bits,
          "can not encode {} as a bit in {} bit integer",
          o,
          available_bits);

        bitfield.set(sql_acl_operation);
    }
    // cast to signed, SQL uses signed integers
