...
/// Returned in responses where sql::error_code is anything else then a value
/// of error_code::none
...
validate_at_topic_level(request_context& ctx, const delete_records_topic& t) {
    ...
    const auto cfg = ctx.metadata_cache().get_topic_cfg(
      model::topic_namespace_view(model::sql_namespace, t.name));
    if (!cfg) {
...
ss::future<result_t> prefix_truncate(
  ...
  std::chrono::milliseconds timeout_ms) {
    ...
    if (sql_truncation_offset == at_current_high_watermark) {
        /// User is requesting to truncate all data
        sql_truncation_offset = partition->high_watermark();
    }
    if (sql_truncation_offset < model::offset(0)) {
    ...
    auto errc = co_await partition->prefix_truncate(
      sql_truncation_offset, ss::lowres_clock::now() + timeout_ms);
    ...
    const auto sql_start_offset = partition->start_offset();
    vlog(
      klog.info,
    ...
    co_return make_partition_response(ktp, sql_start_offset);
}
