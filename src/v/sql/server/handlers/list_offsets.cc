...
void list_offsets_request::compute_duplicate_topics() {
    /*
     * compute a set of duplicate topic partitions. sql has special error
...
static ss::future<list_offset_partition_response> list_offsets_partition(
  ...
  sql::leader_epoch current_leader_epoch,
  cluster::partition_manager& mgr) {
    auto sql_partition = make_partition_proxy(ktp, mgr);
    if (!sql_partition) {
    ...
    auto err = co_await sql_partition->linearizable_barrier();
    if (err) {
        co_return list_offsets_response::make_partition(
    ...
    auto leader_epoch_err = details::check_leader_epoch(
      current_leader_epoch, *sql_partition);
    if (leader_epoch_err != error_code::none) {
        co_return list_offsets_response::make_partition(
          ktp.get_partition(), leader_epoch_err);
    }

    auto offset = sql_partition->high_watermark();
    ...
    if (timestamp == list_offsets_request::earliest_timestamp) {
        ...
        auto maybe_start_ofs = co_await sql_partition->sync_effective_start();
        if (!maybe_start_ofs) {
            co_return list_offsets_response::make_partition(
    ...
    auto res = co_await sql_partition->timequery(storage::timequery_config{
      timestamp,
      offset,
      sql_read_priority(),
    ...
    if (res) {
        co_return list_offsets_response::make_partition(
          id, res->time, res->offset, sql_partition->leader_epoch());
    }
