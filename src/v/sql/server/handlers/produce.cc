...
#include "funesproxy/schema_registry/validation.h"
#include "raft/errc.h"
...
static error_code map_produce_error_code(std::error_code ec) {
    ...
    if (ec.category() == sql::error_category()) {
...
/*
 * Caller is expected to catch errors that may be thrown while the sql
 * batch is being deserialized (see reader_from_sql_batch).
 */
...
static partition_produce_stages produce_topic_partition(
  ...
  produce_request::partition& part) {
    auto ntp = model::ntp(
      model::sql_namespace, topic.name, part.partition_index);
    ...
    auto validator
      = funesproxy::schema_registry::maybe_make_schema_id_validator(
            ...
            [reader = std::move(reader),
             ...
             source_shard = ss::this_shard_id()](
              cluster::partition_manager& mgr) mutable {
                ...
                return funesproxy::schema_registry::maybe_validate_schema_id(
...
produce_topic(produce_ctx& octx, produce_request::topic& topic) {
    ...
    for (auto& part : topic.partitions) {
        const auto& sql_noproduce_topics
          = config::shard_local_cfg().sql_noproduce_topics();
        const bool is_noproduce_topic = std::find(
                                          sql_noproduce_topics.begin(),
                                          sql_noproduce_topics.end(),
                                          topic.name)
                                        != sql_noproduce_topics.end();
        const bool audit_produce_restricted
          = !octx.rctx.authorized_auditor()
            && topic.name == model::sql_audit_logging_topic();
...
produce_handler::handle(request_context ctx, ss::smp_service_group ssg) {
    ...
    if (request.has_transactional) {
        ...
        // <sql>Note that authorization to a transactionalId implies
        // ProducerId authorization</sql>
        ...
        // from sql source: "if required.acks is outside accepted
