...
void handler_probe::setup_metrics(
  metrics::internal_metric_groups& metrics, api_key key) {
    ...
    metrics.add_group(
      prometheus_sanitize::metrics_name("sql_handler"),
      {
        sm::make_counter(
          "requests_completed_total",
          [this] { return _requests_completed; },
          sm::description("Number of sql requests completed"),
          labels),
        sm::make_counter(
          "requests_errored_total",
          [this] { return _requests_errored; },
          sm::description("Number of sql requests errored"),
          labels),
        sm::make_counter(
          "requests_in_progress_total",
          [this] { return _requests_in_progress_every_ns / 1'000'000'000; },
          sm::description("A running total of sql requests in progress"),
          labels),
        sm::make_counter(
          "received_bytes_total",
          [this] { return _bytes_received; },
          sm::description("Number of bytes received from sql requests"),
          labels),
        sm::make_counter(
          "sent_bytes_total",
          [this] { return _bytes_sent; },
          sm::description("Number of bytes sent in sql replies"),
          labels),
        sm::make_histogram(
          "latency_microseconds",
          sm::description("Latency histogram of sql requests"),
          labels,
