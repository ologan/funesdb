...
ss::future<> fill_info_about_tx(
  ...
  sql::transactional_id tx_id) {
    ...
    if (tx_info.has_value()) {
        ...
        tx_info_resp.producer_id = sql::producer_id(tx.pid.id);
        tx_info_resp.producer_epoch = tx.pid.get_epoch();
        tx_info_resp.transaction_state = ss::sstring(tx.get_sql_status());
        ...
        case cluster::tx_errc::stm_not_found:
            tx_info_resp.error_code = sql::error_code::not_coordinator;
            break;
        case cluster::tx_errc::tx_id_not_found:
            tx_info_resp.error_code
              = sql::error_code::transactional_id_not_found;
            break;
        default:
            vlog(
              klog.warn,
              ...
              tx_id,
              tx_info.error());
            tx_info_resp.error_code = sql::error_code::unknown_server_error;
            break;
        }
    }
...
ss::future<> fill_info_about_transactions(
  cluster::tx_gateway_frontend& tx_frontend,
  describe_transactions_response& response,
  std::vector<sql::transactional_id> tx_ids) {
...
ss::future<response_ptr> describe_transactions_handler::handle(
  request_context ctx, ss::smp_service_group) {
    ...
    auto unauthorized_it = std::partition(
      ...
      [&ctx](const sql::transactional_id& tx_id) {
    ...
    std::vector<sql::transactional_id> unauthorized(
