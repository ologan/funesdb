...
#include "sql/server/fwd.h"
#include "sql/server/handlers/details/leader_epoch.h"
#include "sql/server/handlers/fetch/fetch_plan_executor.h"
#include "sql/server/handlers/fetch/fetch_planner.h"
  ...
  std::optional<model::timeout_clock::time_point> deadline) {
    ...
    try {
        auto result = co_await rdr.reader.consume(
          sql_batch_serializer(), deadline ? *deadline : model::no_timeout);
...
read_result::memory_units_t::memory_units_t(
  ...
  : sql(ss::consume_units(memory_sem, 0))
...
read_result::memory_units_t::~memory_units_t() noexcept {
    ...
    auto f = ss::smp::submit_to(
      shard, [uk = std::move(sql), uf = std::move(fetch)]() mutable noexcept {
          uk.return_all();
          uf.return_all();
      });
    if (!f.available()) {
 ...
 *   is assumed that a batch size has already been consumed from sql
...
static read_result::memory_units_t reserve_memory_units(
  ...
  const bool obligatory_batch_read) {
    ...
    const size_t memory_sql_now = memory_sem.current();
    const size_t memory_fetch = memory_fetch_sem.current();
    const size_t batch_size_estimate
      = config::shard_local_cfg().sql_memory_batch_size_estimate_for_fetch();
    ...
    if (obligatory_batch_read) {
        ...
        const size_t fetch_size = std::max(
          batch_size_estimate,
          std::min({max_bytes, memory_sql_now, memory_fetch}));
        memory_units.fetch = ss::consume_units(memory_fetch_sem, fetch_size);
        memory_units.sql = ss::consume_units(memory_sem, fetch_size);
    } else {
        ...
        const size_t fetch_size = std::min(
          {requested_fetch_size, memory_sql_now, memory_fetch});
...
static void adjust_memory_units(
  ...
  const size_t read_bytes) {
    adjust_semaphore_units(memory_sem, memory_units.sql, read_bytes);
...
static ss::future<read_result> do_read_from_ntp(
  ...
  ssx::semaphore& memory_fetch_sem) {
     ...
     */
    auto sql_partition = make_partition_proxy(ntp_config.ktp(), cluster_pm);
    if (unlikely(!sql_partition)) {
        co_return read_result(error_code::unknown_topic_or_partition);
    }
    if (!ntp_config.cfg.read_from_follower && !sql_partition->is_leader()) {
    ...
    auto leader_epoch_err = details::check_leader_epoch(
      ntp_config.cfg.current_leader_epoch, *sql_partition);
    if (leader_epoch_err != error_code::none) {
        co_return read_result(leader_epoch_err);
    }
    auto offset_ec = co_await sql_partition->validate_fetch_offset(
    ...
    if (config::shard_local_cfg().enable_transactions.value()) {
        if (
          ...
          == model::isolation_level::read_committed) {
            auto maybe_lso = sql_partition->last_stable_offset();
    ...
    if (offset_ec != error_code::none) {
        vlog(
          ...
          ntp_config.ktp(),
          ntp_config.cfg.start_offset,
          sql_partition->start_offset(),
          sql_partition->high_watermark(),
          offset_ec);
    ...
    if (ntp_config.cfg.consumer_rack_id && sql_partition->is_leader()) {
        auto p_info_res = sql_partition->get_partition_info();
        ...
        auto lso = sql_partition->last_stable_offset();
        if (unlikely(!lso)) {
            co_return read_result(lso.error());
        }
    ...
    read_result result = co_await read_from_partition(
      std::move(*sql_partition), ntp_config.cfg, foreign_read, deadline);
...
read_result::memory_units_t reserve_memory_units(
  ...
  const size_t max_bytes,
  const bool obligatory_batch_read) {
    return sql::reserve_memory_units(
                ...
                std::transform(
                  ...
                  [](cluster::rm_stm::tx_range range) {
                      return fetch_response::aborted_transaction{
                        .producer_id = sql::producer_id(range.pid.id),
...
static ss::future<std::vector<read_result>> fetch_ntps_in_parallel(
  ...
  ssx::semaphore& memory_fetch_sem) {
    ...
    const size_t max_bytes_per_fetch = std::min<size_t>(
      config::shard_local_cfg().sql_max_bytes_per_fetch(), bytes_left);
 ...
 * into sql specific response codes. On failure or success the
 ...
 * parallel. First, SQL expects to some extent that the order of the
...
namespace testing {
sql::fetch_plan make_simple_fetch_plan(op_context& octx) {
    auto planner = make_fetch_planner<simple_fetch_planner>();
