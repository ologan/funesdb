...
#include "sql/protocol/schemata/alter_configs_response.h"
        ...
        try {
            if (cfg.name == topic_property_cleanup_policy) {
                parse_and_set_optional(
                  ...
                  cfg.value,
                  sql::config_resource_operation::set);
                continue;
            }
            ...
            if (
              ...
              != funesproxy::schema_registry::schema_id_validation_mode::none) {
                if (schema_id_validation_config_parser(
                      cfg, sql::config_resource_operation::set)) {
                    continue;
                }
            }
            if (
...
alter_broker_configuartion(std::vector<alter_configs_resource> resources) {
    return unsupported_broker_configuration<
      ...
      "API. Try using sql incremental config API or "
      "funes admin API.");
}
