...
static void fill_response(
  ...
  describe_acls_response_data& response) {
    ...
    for (const auto& entry : entries) {
        ...
        // resource pattern
        resource.type = details::to_sql_resource_type(entry.first.resource());
        resource.name = entry.first.name();
        resource.pattern_type = details::to_sql_pattern_type(
          entry.first.pattern());
        ...
        for (auto& acl : entry.second) {
            ...
            acl_description desc{
              .principal = details::to_sql_principal(acl.principal()),
              .host = details::to_sql_host(acl.host()),
              .operation = details::to_sql_operation(acl.operation()),
              .permission_type = details::to_sql_permission(acl.permission()),
            };
