...
public:
    ...
    /// Get the member id.
    const sql::member_id& id() const { return _state.id; }

    void replace_id(member_id new_id) { _state.id = std::move(new_id); }

    /// Get the id of the member's group.
    const sql::group_id& group_id() const { return _group_id; }

    /// Get the instance id of the member's group.
    const std::optional<sql::group_instance_id>& group_instance_id() const {
    ...
    const sql::protocol_type& protocol_type() const { return _protocol_type; }
     ...
     */
    const sql::protocol_name& vote_for_protocol(
     ...
     */
    const bytes&
    get_protocol_metadata(const sql::protocol_name& protocol) const;
    ...
    // helper for sql api: describe groups
    described_group_member describe(const sql::protocol_name&) const;
