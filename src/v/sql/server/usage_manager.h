...
public:
    ...
    ///
    /// Should be called at the sql layer to account for bytes sent via sql
    /// port
    void add_bytes_sent(size_t sent) { _current_bucket.bytes_sent += sent; }

    /// Adds bytes received to current open window
    ///
    /// Should be called at the sql layer to account for bytes received via
    /// sql port
...
/// Bytes accounted for by the sql::usage_manager will not take into
