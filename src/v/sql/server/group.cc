...
#include "sql/protocol/schemata/describe_groups_response.h"
#include "sql/protocol/sync_group.h"
#include "sql/protocol/wire.h"
#include "sql/server/group_manager.h"
#include "sql/server/group_metadata.h"
#include "sql/server/logger.h"
#include "sql/server/member.h"
#include "sql/types.h"
#include "likely.h"
  ...
  , _feature_table(feature_table) {
    for (auto& m : md.members) {
        auto member = ss::make_lw_shared<group_member>(
          ...
          std::vector<sql::member_protocol>{member_protocol{
...
bool group::supports_protocols(const join_group_request& r) const {
    ...
    return std::any_of(
      ...
      [this](const sql::join_group_request_protocol& p) {
 ...
 */
sql::protocol_name group::select_protocol() const {
    // index of protocols supported by all members
    absl::flat_hash_set<sql::protocol_name> candidates;
    std::for_each(
    ...
    std::for_each(
      ...
      [this, error](const member_map::value_type& m) {
          ...
          if (member->is_syncing()) {
              ...
              // <sql>reset the session timeout for members after propagating
...
group::join_group_stages group::update_static_member_and_rebalance(
  member_id old_member_id, member_id new_member_id, join_group_request&& r) {
    ...
    /*
     * <sql> Heartbeat of old member id will expire without effect since the
     * group no longer contains that member id. New heartbeat shall be scheduled
     * with new member id.</sql>
     */
    schedule_next_heartbeat_expiration(member);
                  ...
                  .then([this,
                      ...
                      if (!result) {
                          vlog(
                            ...
                            result.error());
                          // <sql>Failed to persist member.id of the
                          // given static member, revert the update of
                          // the static member in the group.</sql>
                          auto member = replace_static_member(
                      ...
                      auto reply = join_group_response(
                        error_code::none,
                        generation(),
                        protocol().value_or(sql::protocol_name()),
                        leader().value_or(sql::member_id()),
                        new_member_id,
                        std::move(md));
...
group::add_new_dynamic_member(member_id new_member_id, join_group_request&& r) {
    vassert(
      ...
      r.data);
    // <sql>Only return MEMBER_ID_REQUIRED error if joinGroupRequest version
    // is >= 4 and groupInstanceId is configured to unknown.</sql>
    if (r.version >= api_version(4)) {
        // <sql>If member id required (dynamic membership), register the
...
group::join_group_known_member(join_group_request&& r) {
    if (in_state(group_state::dead)) {
        ...
        sql::member_id new_member_id = std::move(r.data.member_id);
        return add_member_and_rebalance(std::move(new_member_id), std::move(r));
    ...
    case group_state::completing_rebalance:
        if (r.data.protocols == member->protocols()) {
            // <sql>member is joining with the same metadata (which could be
            ...
            // <sql>member has changed metadata, so force a rebalance</sql>
    ...
    case group_state::stable:
        if (
          ...
          || r.data.protocols != member->protocols()) {
            // <sql>force a rebalance if a member has changed metadata or if
            ...
            // <sql>for followers with no actual change to their metadata,
            // just return group information for the current generation which
            // will allow them to issue SyncGroup</sql>
            join_group_response response(
              error_code::none,
              generation(),
              protocol().value_or(protocol_name("")),
...
group::join_group_stages group::add_member_and_rebalance(
  sql::member_id member_id, join_group_request&& r) {
    auto member = ss::make_lw_shared<group_member>(
      std::move(member_id),
      id(),
    ...
    // <sql>update the newMemberAdded flag to indicate that the join group can
    // be further delayed</sql>
    if (
      in_state(group_state::preparing_rebalance)
      && generation() == sql::generation_id(0)) {
    ...
    // <sql>The session timeout does not affect new members since they do not
    ...
    // member is still there, we expect it to retry.</sql>
...
void group::try_prepare_rebalance() {
    ...
    // <sql>if any members are awaiting sync, cancel their request and have
    // them rejoin.</sql>
...
void group::complete_join() {
    ...
    // <sql>remove dynamic members who haven't joined the group yet</sql>
...
void group::heartbeat_expire(
  sql::member_id member_id, clock_type::time_point deadline) {
    if (in_state(group_state::dead)) {
        vlog(
          _ctxlog.trace,
...
void group::remove_pending_member(sql::member_id member_id) {
...
void group::remove_member(member_ptr member) {
    ...
    // <sql>New members may timeout with a pending JoinGroup while the group
    ...
    // the JoinGroup request if is still active.</sql>
    try_finish_joining_member(
...
group::sync_group_stages group::handle_sync_group(sync_group_request&& r) {
    ...
    case group_state::stable: {
        // <sql>if the group is stable, we just return the current
        // assignment</sql>
        auto member = get_member(r.data.member_id);
        schedule_next_heartbeat_expiration(member);
...
sql::error_code group::member_leave_group(
  const member_id& member_id,
  const std::optional<group_instance_id>& group_instance_id) {
    // <sql>The LeaveGroup API allows administrative removal of members by
    ...
    if (contains_pending_member(member_id)) {
        // <sql>if a pending member is leaving, it needs to be removed
        // from the pending list, heartbeat cancelled and if necessary,
        // prompt a JoinGroup completion.</sql>
        remove_pending_member(member_id);
...
group::store_txn_offsets(txn_offset_commit_request r) {
    ...
    for (const auto& [tp, offset] : offsets) {
        offset_metadata md{
          ...
          .committed_leader_epoch = sql::leader_epoch(offset.leader_epoch),
...
sql::error_code map_store_offset_error_code(std::error_code ec) {
...
group::handle_offset_commit(offset_commit_request&& r) {
    if (in_state(group_state::dead)) {
        ...
        // <sql>The group is only using SQL to store offsets.</sql>
      ...
      || in_state(group_state::preparing_rebalance)) {
        // <sql>During PreparingRebalance phase, we still allow a commit
        // request since we rely on heartbeat response to eventually notify the
        // rebalance in progress signal to the consumer</sql>
        auto member = get_member(r.data.member_id);
        schedule_next_heartbeat_expiration(member);
        return store_offsets(std::move(r));
    } else if (in_state(group_state::completing_rebalance)) {
...
sql::member_id group::generate_member_id(const join_group_request& r) {
    auto cid = r.client_id ? *r.client_id : sql::client_id("");
    auto id = r.data.group_instance_id ? (*r.data.group_instance_id)() : cid();
    boost::uuids::uuid uuid = boost::uuids::random_generator()();
    return sql::member_id(ssx::sformat("{}-{}", id, to_string(uuid)));
}
...
described_group group::describe() const {
    described_group desc{
      .error_code = error_code::none,
      .group_id = id(),
      .group_state = group_state_to_sql_name(state()),
      .protocol_type = protocol_type().value_or(sql::protocol_type("")),
    };
...
void group::add_group_tombstone_record(
  const sql::group_id& group, storage::record_batch_builder& builder) {
...
ss::future<error_code> group::remove() {
    ...
    // sql chooses to report no error even if replication fails. the in-memory
...
std::ostream& operator<<(std::ostream& o, group_state gs) {
    return o << group_state_to_sql_name(gs);
}

ss::sstring group_state_to_sql_name(group_state gs) {
    // State names are written in camel case to match the formatting of sql
    // since these states are returned through the sql describe groups api.
    switch (gs) {
    case group_state::empty:
        return "Empty";
    case group_state::preparing_rebalance:
...
void group::add_pending_member(
  const sql::member_id& member_id, duration_type timeout) {
...
ss::future<cluster::commit_group_tx_reply>
group::do_commit(sql::group_id group_id, model::producer_identity pid) {
    auto prepare_it = _prepared_txs.find(pid);
    if (prepare_it == _prepared_txs.end()) {
    ...
    // It is fix for https://github.com/redpanda-data/funes/issues/5163.
    ...
    // tx_gateway_frontend). So funes will eventually finish commit and
    ...
    if (_protocol_type.has_value()) {
        if (
          ...
          && _subscriptions.has_value() && in_state(group_state::stable)) {
            /*
             * policy description from sql source:
             *
             * <sql>
         ...
         * consumer that SQL uses for offset storage only.
         *
