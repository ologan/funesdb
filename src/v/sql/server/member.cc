...
const sql::protocol_name& group_member::vote_for_protocol(
...
const bytes& group_member::get_protocol_metadata(
  const sql::protocol_name& protocol) const {
    auto it = std::find_if(
      _protocols.cbegin(),
      _protocols.cend(),
...
group_member::describe(const sql::protocol_name& protocol) const {
