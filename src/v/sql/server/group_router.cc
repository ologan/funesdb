...
auto group_router::route_tx(Request&& r, FwdFunc func) {
    ...
    if (!m) {
        ...
        // cluster::tx_errc instead of sql::error_code
        // because the latter is part of the sql protocol
        // we can't extend it
       ...
       func]() mutable {
          return get_group_manager()
            ...
            .then([this,
                ...
                // is under the influence of the sql smp_service_group (_ssg),
...
ss::future<described_group> group_router::describe_group(sql::group_id g) {
    auto m = shard_for(g);
    if (!m) {
