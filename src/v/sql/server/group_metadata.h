...
/**
 * Group log metadata consistent with SQL internal format
 */
...
struct group_metadata_value {
    ...
    sql::generation_id generation;
    std::optional<sql::protocol_name> protocol;
    std::optional<sql::member_id> leader;
...
/**
 * The value type for offset commit records, consistent with SQL format
 */
struct offset_metadata_value {
    static constexpr group_metadata_version latest_version{3};
    model::offset offset;
    // present only in version >= 3
    sql::leader_epoch leader_epoch = invalid_leader_epoch;
    ss::sstring metadata;
...
namespace std {
template<>
struct hash<sql::offset_metadata_key> {
    size_t operator()(const sql::offset_metadata_key& key) const {
        size_t h = 0;
