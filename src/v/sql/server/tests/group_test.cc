...
static const std::vector<member_protocol> test_group_protos = {
  {sql::protocol_name("n0"), "d0"}, {sql::protocol_name("n1"), "d1"}};
...
static member_ptr get_group_member(
  ...
  std::vector<member_protocol> protos = test_group_protos) {
    return ss::make_lw_shared<group_member>(
      sql::member_id(id),
      sql::group_id("g"),
      sql::group_instance_id(fmt::format("i-{}", id)),
      sql::client_id("client-id"),
      sql::client_host("client-host"),
...
SEASTAR_THREAD_TEST_CASE(get_member_throws_on_empty) {
    auto g = get();
    BOOST_CHECK_THROW(g.get_member(sql::member_id("m")), std::out_of_range);
}
...
SEASTAR_THREAD_TEST_CASE(get_member_returns_member) {
    ...
    (void)g.add_member(m);
    BOOST_TEST(g.get_member(sql::member_id("m")) == m);
}

SEASTAR_THREAD_TEST_CASE(contains_member) {
    auto g = get();
    BOOST_TEST(!g.contains_member(sql::member_id("m")));
    auto m = get_group_member();
    (void)g.add_member(m);
    BOOST_TEST(g.contains_member(sql::member_id("m")));
    BOOST_TEST(!g.contains_member(sql::member_id("n")));
}
...
SEASTAR_THREAD_TEST_CASE(pending_members) {
    auto g = get();
    BOOST_TEST(!g.contains_pending_member(sql::member_id("m")));
    g.add_pending_member(sql::member_id("m"), 5s);
    BOOST_TEST(g.contains_pending_member(sql::member_id("m")));
    g.remove_pending_member(sql::member_id("m"));
    BOOST_TEST(!g.contains_pending_member(sql::member_id("m")));
}
...
SEASTAR_THREAD_TEST_CASE(pending_members_expire) {
    auto g = get();
    BOOST_TEST(!g.contains_pending_member(sql::member_id("m")));
    g.add_pending_member(sql::member_id("m"), 1s);
    BOOST_TEST(g.contains_pending_member(sql::member_id("m")));
    ss::sleep(2s).get();
    BOOST_TEST(!g.contains_pending_member(sql::member_id("m")));
}
...
SEASTAR_THREAD_TEST_CASE(add_member_sets_leader) {
    auto g = get();
    BOOST_TEST(!g.is_leader(sql::member_id("m")));
    BOOST_TEST(!g.leader());
...
SEASTAR_THREAD_TEST_CASE(add_missing_assignments) {
    ...
    BOOST_TEST(a.size() == 2);
    BOOST_TEST(a[sql::member_id("m")] == bytes());
    BOOST_TEST(a[sql::member_id("n")] == bytes());

    a.clear();
    a[sql::member_id("m")] = bytes("d1");
    a[sql::member_id("o")] = bytes("d2");
    g.add_missing_assignments(a);
    BOOST_TEST(a.size() == 3);
    BOOST_TEST(a[sql::member_id("m")] == bytes("d1"));
    BOOST_TEST(a[sql::member_id("n")] == bytes());
    BOOST_TEST(a[sql::member_id("o")] == bytes("d2"));
}
...
SEASTAR_THREAD_TEST_CASE(all_members_joined) {
    ...
    g.add_pending_member(sql::member_id("x"), 5s);
    BOOST_TEST(!g.all_members_joined());
}
...
SEASTAR_THREAD_TEST_CASE(member_metadata) {
    ...
    auto protos = std::vector<member_protocol>{
      {sql::protocol_name("p0"), bytes()},
      {sql::protocol_name("p1"), bytes("foo")},
      {sql::protocol_name("p2"), bytes()}};
    auto m0 = get_group_member("m", protos);

    protos = std::vector<member_protocol>{
      {sql::protocol_name("p1"), bytes("bar")},
      {sql::protocol_name("p2"), bytes()},
      {sql::protocol_name("p3"), bytes()}};
    ...
    std::unordered_map<sql::member_id, join_group_response_member> conf;
    for (auto& m : md) {
        conf[m.member_id] = m;
    }
    BOOST_TEST(conf[sql::member_id("m")].metadata == bytes("foo"));
    BOOST_TEST(conf[sql::member_id("n")].metadata == bytes("bar"));
}
...
SEASTAR_THREAD_TEST_CASE(supports_protocols) {
    ...
    r.data.protocol_type = sql::protocol_type("x");
    r.data.protocols = std::vector<join_group_request_protocol>{
      {sql::protocol_name(""), bytes()}};
    BOOST_TEST(!g.supports_protocols(r));
    ...
    auto m2 = ss::make_lw_shared<group_member>(
      ...
      std::chrono::seconds(3),
      sql::protocol_type("p"),
      std::vector<member_protocol>{{sql::protocol_name("n2"), "d0"}});
    (void)g.add_member(m2);

    // n2 is not supported bc the first member doesn't support it
    r.data.protocols = std::vector<join_group_request_protocol>{
      {sql::protocol_name("n2"), bytes()}};
    BOOST_TEST(!g.supports_protocols(r));
}
...
SEASTAR_THREAD_TEST_CASE(generate_member_id) {
    ...
    r.client_id = sql::client_id(ss::sstring("dog"));
    ...
    r.client_id = sql::client_id(ss::sstring("dog"));
    r.data.group_instance_id = sql::group_instance_id("cat");
