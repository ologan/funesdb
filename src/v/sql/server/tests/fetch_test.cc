...
SEASTAR_THREAD_TEST_CASE(partition_iterator) {
     ...
     */
    auto transform = [](const sql::fetch_request& request) {
        std::vector<model::topic_partition> res;
        std::transform(
          request.cbegin(),
          request.cend(),
          std::back_inserter(res),
          [&res](const sql::fetch_request::const_iterator::value_type& v) {
              if (v.new_topic) {
                  if (!res.empty()) {
...
// builder.
FIXTURE_TEST(read_from_ntp_max_bytes, funes_thread_fixture) {
    auto do_read = [this](model::ktp ktp, size_t max_bytes) {
        sql::fetch_config config{
          ...
          = octx.rctx.partition_manager()
              .invoke_on(
                ...
                [&octx, ktp, config](cluster::partition_manager& pm) {
                    return sql::testing::read_from_ntp(
                      pm,
...
FIXTURE_TEST(fetch_one, funes_thread_fixture) {
    wait_for_controller_leadership().get0();
    ...
    for (auto version : boost::irange<uint16_t>(
           sql::fetch_handler::min_supported,
           sql::fetch_handler::max_supported + int16_t(1))) {
        ...
        client.connect().get();
        auto resp = client.dispatch(req, sql::api_version(version)).get0();
...
FIXTURE_TEST(fetch_response_iterator_test, funes_thread_fixture) {
    static auto make_partition = [](ss::sstring topic) {
        return sql::fetch_response::partition{
          .name = model::topic(std::move(topic))};
    };

    static auto make_partition_response = [](int id) {
        sql::fetch_response::partition_response resp;
        resp.error_code = sql::error_code::none;
...
FIXTURE_TEST(fetch_empty, funes_thread_fixture) {
    // create a topic partition with some data
    model::topic topic("foo");
    model::partition_id pid(0);
    auto ntp = make_default_ntp(topic, pid);
    ...
    auto client = make_sql_client().get0();
    client.connect().get();
    auto resp_1 = client.dispatch(no_topics, sql::api_version(6)).get0();

    BOOST_REQUIRE(resp_1.data.topics.empty());

    sql::fetch_request no_partitions;
    ...
    auto resp_2 = client.dispatch(no_topics, sql::api_version(6)).get0();
    client.stop().then([&client] { client.shutdown(); }).get();

    BOOST_REQUIRE(resp_2.data.topics.empty());
}

FIXTURE_TEST(fetch_multi_partitions_debounce, funes_thread_fixture) {
    // create a topic partition with some data
    model::topic topic("foo");
    model::offset offset(0);
    ...
    add_topic(model::topic_namespace(model::ns("sql"), topic), 6).get();
    ...
    auto client = make_sql_client().get0();
    client.connect().get();
    auto fresp = client.dispatch(req, sql::api_version(4));
...
FIXTURE_TEST(fetch_one_debounce, funes_thread_fixture) {
    model::topic topic("foo");
    model::partition_id pid(0);
    model::offset offset(0);
    auto ntp = make_default_ntp(topic, pid);
    ...
    BOOST_REQUIRE(resp.data.topics[0].partitions.size() == 1);
    BOOST_REQUIRE(
      resp.data.topics[0].partitions[0].error_code == sql::error_code::none);
    BOOST_REQUIRE(resp.data.topics[0].partitions[0].partition_index == pid);
    BOOST_REQUIRE(resp.data.topics[0].partitions[0].records);
    BOOST_REQUIRE(resp.data.topics[0].partitions[0].records->size_bytes() > 0);
}

FIXTURE_TEST(fetch_multi_topics, funes_thread_fixture) {
    // create a topic partition with some data
    ...
    add_topic(model::topic_namespace(model::ns("sql"), topic_1), 6).get();
    add_topic(model::topic_namespace(model::ns("sql"), topic_2), 1).get();
    ...
    auto resp = client.dispatch(req, sql::api_version(4)).get0();
    client.stop().then([&client] { client.shutdown(); }).get();
...
FIXTURE_TEST(fetch_request_max_bytes, funes_thread_fixture) {
    model::topic topic("foo");
    model::partition_id pid(0);
    auto ntp = make_default_ntp(topic, pid);
    ...
    auto fetch = [this, &topic, &pid](int32_t max_bytes) {
        ...
        auto client = make_sql_client().get();
        client.connect().get();
        auto fresp = client.dispatch(req, sql::api_version(4));
