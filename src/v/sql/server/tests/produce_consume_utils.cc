...
ss::future<sql_produce_transport::pid_to_offset_map_t>
sql_produce_transport::produce(
  ...
  std::optional<model::timestamp> ts) {
    ...
    for (auto& data_resp : resp.data.responses) {
        for (auto& prt_resp : data_resp.partitions) {
            if (prt_resp.error_code != sql::error_code::none) {
...
std::vector<sql::partition_produce_data>
sql_produce_transport::produce_partition_requests(
  const pid_to_kvs_map_t& records_per_partition,
  std::optional<model::timestamp> ts) {
    std::vector<sql::partition_produce_data> ret;
...
ss::future<pid_to_kvs_map_t> sql_consume_transport::consume(
  model::topic topic_name,
  ...
  model::offset offset_inclusive) {
    ...
    auto fetch_resp = co_await _transport.dispatch(
      std::move(req), sql::api_version(4));
    if (fetch_resp.data.error_code != sql::error_code::none) {
        throw std::runtime_error(
          fmt::format("fetch error: {}", fetch_resp.data.error_code));
    }
    vlog(test_log.debug, "Received response from the sql api");
    ...
    for (auto& topic : data.topics) {
        ...
        for (auto& partition : topic.partitions) {
            if (partition.error_code != sql::error_code::none) {
                throw std::runtime_error(fmt::format(
