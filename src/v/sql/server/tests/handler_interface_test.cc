...
void check_any_vs_static() {
    ...
    auto hopt = sql::handler_for_key(H::api::key);
...
void check_all_types(sql::type_list<Ts...>) {
    (check_any_vs_static<Ts>(), ...);
}

BOOST_AUTO_TEST_CASE(handler_all_types) {
    check_all_types(sql::request_types{});
}

BOOST_AUTO_TEST_CASE(handler_handler_for_key) {
    // key too low
    BOOST_CHECK(!sql::handler_for_key(sql::api_key(-1)).has_value());
    // key too high
    const auto max_key = sql::max_api_key(sql::request_types{});
    BOOST_CHECK(
      !sql::handler_for_key(sql::api_key(max_key + 1)).has_value());
    // last key should be present
    BOOST_CHECK(sql::handler_for_key(sql::api_key(max_key)).has_value());
    ...
    BOOST_CHECK(!sql::handler_for_key(sql::api_key(34)).has_value());
}
