...
// Wrapper around a SQL transport that encapsulates producing to a node.
//
// The primary goal of this class is to allow tests to produce without dealing
// with the SQL protocol. To that end, it exposes a protocol-agnostic API.
class sql_produce_transport {
public:
    using pid_to_offset_map_t
      = absl::flat_hash_map<model::partition_id, model::offset>;

    explicit sql_produce_transport(sql::client::transport&& t)
...
private:
    ...
    static std::vector<sql::partition_produce_data>
...
class sql_consume_transport {
public:
    explicit sql_consume_transport(sql::client::transport&& t)
      : _transport(std::move(t)) {}
    ...
    ss::future<pid_to_kvs_map_t> consume(
      ...
      model::offset sql_offset_inclusive);

    ss::future<std::vector<kv_t>> consume_from_partition(
      model::topic topic_name,
      model::partition_id pid,
      model::offset sql_offset_inclusive) {
        auto m = co_await consume(topic_name, {pid}, sql_offset_inclusive);
        if (m.empty()) {
