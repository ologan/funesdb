...
struct fixture {
    ...
    void roundtrip_test(const T& value) {
        ...
        sql::protocol::encoder writer(buffer);
        T::encode(writer, value);

        sql::protocol::decoder reader(std::move(buffer));
...
sql::member_state random_member_state() {
    sql::member_state state;
    state.id = random_named_string<sql::member_id>();
    state.instance_id = random_optional<sql::group_instance_id>();
    state.client_id = random_named_string<sql::client_id>();
    state.client_host = random_named_string<sql::client_host>();
...
FIXTURE_TEST(metadata_rt_test, fixture) {
    sql::group_metadata_key group_md_key;
    group_md_key.group_id = random_named_string<sql::group_id>();

    roundtrip_test(group_md_key);
    auto state = random_member_state();
    roundtrip_test(state);

    sql::group_metadata_value group_md;
    group_md.protocol_type = random_named_string<sql::protocol_type>();
    group_md.generation = random_named_int<sql::generation_id>();
    group_md.leader = random_optional<sql::member_id>();
    group_md.protocol = random_optional<sql::protocol_name>();
    group_md.leader = random_optional<sql::member_id>();
    ...
    sql::offset_metadata_key offset_key;
    offset_key.group_id = random_named_string<sql::group_id>();
    ...
    // version 1
    sql::offset_metadata_value offset_md_v1;
    ...
    // version 3
    sql::offset_metadata_value offset_md;

    offset_md.offset = random_named_int<model::offset>();
    offset_md.leader_epoch = random_named_int<sql::leader_epoch>();
...
FIXTURE_TEST(test_consumer_offsets_serializer, fixture) {
    auto serializer = sql::make_consumer_offsets_serializer();
    ...
    auto group_kv = serializer.to_kv(sql::group_metadata_kv{
      .key = group_md_key,
      .value = group_md.copy(),
    });
    ...
    auto offset_kv = serializer.to_kv(sql::offset_metadata_kv{
      .key = offset_key,
      .value = offset_md,
    });
...
FIXTURE_TEST(test_unwrapping_tombstones_from_iobuf, fixture) {
    std::vector<sql::group_metadata_serializer> serializers;
    serializers.reserve(1);
    serializers.push_back(sql::make_consumer_offsets_serializer());
