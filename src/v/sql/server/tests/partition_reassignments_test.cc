...
#include "sql/protocol/alter_partition_reassignments.h"
#include "sql/protocol/list_partition_reassignments.h"
...
class partition_reassignments_test_fixture : public funes_thread_fixture {
public:
    ...
    sql::alter_partition_reassignments_response alter_partition_reassignments(
      sql::client::transport& client,
      std::optional<model::topic> topic_name,
      std::vector<sql::reassignable_partition> reassignable_partitions) {
        sql::alter_partition_reassignments_request req;

        if (topic_name.has_value()) {
            req.data.topics = std::vector<sql::reassignable_topic>{
              sql::reassignable_topic{
                .name = *topic_name,
                .partitions = std::move(reassignable_partitions)}};
        }
        return client.dispatch(std::move(req), sql::api_version(0)).get();
    }

    sql::list_partition_reassignments_response list_partition_reassignments(
      sql::client::transport& client,
      std::optional<std::vector<sql::list_partition_reassignments_topics>>
        topics
      = std::nullopt) {
        sql::list_partition_reassignments_request req;
  ...
  test_alter_partition_reassignments, partition_reassignments_test_fixture) {
...

    {
        ...
        auto resp = alter_partition_reassignments(
          client, std::nullopt, std::vector<sql::reassignable_partition>{});
...

    {
        ...
        auto resp = alter_partition_reassignments(
          client, test_tp, std::vector<sql::reassignable_partition>{});
        BOOST_CHECK_EQUAL(resp.data.responses.size(), 0);
    }

    {
        test_log.info("Empty replicas (!= undefined replicas)");
        sql::reassignable_partition new_partition{
          .partition_index = pid0, .replicas = std::vector<model::node_id>{}};
        auto resp = alter_partition_reassignments(
          client,
          test_tp,
          std::vector<sql::reassignable_partition>{new_partition});
        ...
        for (auto& tp_resp : resp.data.responses) {
            ...
            for (auto& p_resp : tp_resp.partitions) {
                ...
                BOOST_CHECK_EQUAL(
                  p_resp.error_code,
                  sql::error_code::invalid_replica_assignment);
                BOOST_CHECK(
...

    {
        ...
        auto resp = alter_partition_reassignments(
          client,
          test_tp,
          std::vector<sql::reassignable_partition>{
            sql::reassignable_partition{
        ...
        for (auto& tp_resp : resp.data.responses) {
            ...
            for (auto& p_resp : tp_resp.partitions) {
                ...
                BOOST_CHECK_EQUAL(
                  p_resp.error_code,
                  sql::error_code::no_reassignment_in_progress);
                BOOST_CHECK(
                  *p_resp.error_message
  ...
  test_list_partition_reassignments, partition_reassignments_test_fixture) {
...

    {
        ...
        auto resp = list_partition_reassignments(
          client,
          std::vector<sql::list_partition_reassignments_topics>{
            sql::list_partition_reassignments_topics{
