...
#include "sql/server/token_bucket_rate_tracker.h"
...
SEASTAR_THREAD_TEST_CASE(token_bucket_algorithm_test) {
    ...
    const auto now = sql::token_bucket_rate_tracker::clock::now();

    /// Bucket should start with burst() credits as max, so 100 units
    sql::token_bucket_rate_tracker rate(quota, samples, window_sec);
...
SEASTAR_THREAD_TEST_CASE(token_bucket_edge_case_test) {
    ...
    sql::token_bucket_rate_tracker rate(max_val, max_val, 2s);
