...
FIXTURE_TEST(list_offsets, funes_thread_fixture) {
...
FIXTURE_TEST(list_offsets_earliest, funes_thread_fixture) {
    wait_for_controller_leadership().get0();
    auto ntp = make_data();
    auto shard = app.shard_table.local().shard_for(ntp);
    tests::cooperative_spin_wait_with_timeout(10s, [this, shard, ntp = ntp] {
    ...
    req.data.topics = {{
      ...
      .partitions = {{
        .partition_index = ntp.get_partition(),
        .timestamp = sql::list_offsets_request::earliest_timestamp,
      }},
    }};
...
FIXTURE_TEST(list_offsets_latest, funes_thread_fixture) {
    wait_for_controller_leadership().get0();
    auto ntp = make_data();
    auto shard = app.shard_table.local().shard_for(ntp);
    tests::cooperative_spin_wait_with_timeout(10s, [this, shard, ntp = ntp] {
    ...
    req.data.topics = {{
      ...
      .partitions = {{
        .partition_index = ntp.get_partition(),
        .timestamp = sql::list_offsets_request::latest_timestamp,
      }},
    }};
...
FIXTURE_TEST(list_offsets_not_found, funes_thread_fixture) {
    ...
    auto resp = client.dispatch(req, sql::api_version(4)).get();
    client.stop().then([&client] { client.shutdown(); }).get();
...
make_produce_request(model::topic_partition tp, model::record_batch&& batch) {
    std::vector<sql::produce_request::partition> partitions;
    partitions.emplace_back(sql::produce_request::partition{
      .partition_index{tp.partition},
      .records = sql::produce_request_record_data(std::move(batch))});

    std::vector<sql::produce_request::topic> topics;
    topics.emplace_back(sql::produce_request::topic{
      .name{std::move(tp.topic)}, .partitions{std::move(partitions)}});
    std::optional<ss::sstring> t_id;
    int16_t acks = -1;
    return sql::produce_request(t_id, acks, std::move(topics));
}

FIXTURE_TEST(list_offsets_by_time, funes_thread_fixture) {
    wait_for_controller_leadership().get0();
    model::ntp ntp(
      model::sql_namespace,
    ...
    for (long i = 0; i < batch_count; ++i) {
        ...
        auto res = client.dispatch(std::move(req), sql::api_version(7)).get();
        ...
        BOOST_REQUIRE_EQUAL(parts[0].error_code, sql::error_code::none);
    }
    ...
    for (long i = 0; i < batch_count; ++i) {
        ...
        auto resp_midbatch = client.dispatch(req, sql::api_version(1)).get0();
