...
SEASTAR_THREAD_TEST_CASE(get_protocol_metadata) {
    ...
    BOOST_CHECK_THROW(
      m.get_protocol_metadata(sql::protocol_name("dne")), std::out_of_range);
}
...
SEASTAR_THREAD_TEST_CASE(protocols) {
    ...
    protos[0].name = sql::protocol_name("x");
...
SEASTAR_THREAD_TEST_CASE(member_serde) {
    ...
    iobuf m0_iobuf;
    auto writer = sql::protocol::encoder(m0_iobuf);
    member_state::encode(writer, m0_state);
    sql::protocol::decoder reader(m0_iobuf.copy());
    auto m1_state = member_state::decode(reader);
    auto m1 = sql::group_member(
      std::move(m1_state),
      m0.group_id(),
      sql::protocol_type("p"),
      test_protos);
