...
#include "sql/server/usage_aggregator.h"
...
class test_accounting_fiber final
  : public sql::usage_aggregator<ss::manual_clock> {
public:
    test_accounting_fiber(
      ...
      : sql::usage_aggregator<ss::manual_clock>(
        kvstore,
        usage_num_windows,
...
protected:
    ss::future<sql::usage> close_current_window() final {
...
namespace {
std::vector<sql::usage>
strip_window_data(const std::vector<sql::usage_window>& v) {
    std::vector<sql::usage> vv;
    std::transform(
      v.begin(),
      v.end(),
      std::back_inserter(vv),
      [](const sql::usage_window& uw) { return uw.u; });
    return vv;
}

ss::sstring print_window_data(const std::vector<sql::usage_window>& v) {
    std::stringstream ss;
    ss << "\n[\n";
    for (const auto& vs : v) {
        ss << "\t" << vs.u << ",\n";
...
FIXTURE_TEST(test_usage, kvstore_test_fixture) {
    ...
    data.emplace_back(sql::usage{.bytes_sent = 0, .bytes_received = 0});
...
SEASTAR_THREAD_TEST_CASE(test_round_to_interval_method) {
    ...
    for (const auto& ival : intervals) {
        ...
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts));
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts + 10s));
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts - 10s));
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts + 1min));
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts - 1min));
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts + 2min));
        BOOST_CHECK(ts == sql::detail::round_to_interval(ival, ts - 2min));
        /// Past the 2min threshold the method returns the input unmodified
        BOOST_CHECK(ts != sql::detail::round_to_interval(ival, ts + 3min));
        BOOST_CHECK(ts != sql::detail::round_to_interval(ival, ts - 3min));
        BOOST_CHECK(ts != sql::detail::round_to_interval(ival, ts + 10min));
        BOOST_CHECK(ts != sql::detail::round_to_interval(ival, ts - 10min));
    }
}
