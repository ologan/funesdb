...
struct fetch_plan_fixture : funes_thread_fixture {
...
PERF_TEST_F(fetch_plan_fixture, test_fetch_plan) {
    ...
    // create a request
    sql::fetch_request_data frq_data;
    frq_data.replica_id = sql::client::consumer_replica_id;
    ...
    frq_data.session_id = sql::invalid_fetch_session_id;
    frq_data.session_epoch = sql::initial_fetch_session_epoch;
    frq_data.topics.push_back(std::move(ft));

    sql::fetch_request fetch_req{frq_data};
    ...
    for (size_t i = 0; i < iters; i++) {
        auto plan = sql::testing::make_simple_fetch_plan(octx);
    ...
    // auto plan = sql::make_simple_fetch_plan(octx);
