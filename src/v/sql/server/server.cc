...
#include "sql/protocol/schemata/list_groups_response.h"
#include "sql/server/connection_context.h"
...
#include "sql/server/handlers/details/security.h"
#include "sql/server/handlers/end_txn.h"
#include "sql/server/handlers/fetch/replica_selector.h"
#include "sql/server/handlers/handler_interface.h"
...
server::server(
  ...
  ss::sharded<sql::group_router>& router,
  ...
  const std::unique_ptr<funesproxy::schema_registry::api>& sr) noexcept
  : net::server(cfg, klog)
  , _smp_group(smp)
  ...
  , _mtls_principal_mapper(
      config::shard_local_cfg().sql_mtls_principal_mapping_rules.bind())
  ...
  , _memory_fetch_sem(
      static_cast<size_t>(
        ...
        * config::shard_local_cfg().sql_memory_share_for_fetch()),
      "sql/server-mem-fetch")
  ...
  , _schema_registry(sr) {
    vlog(
      klog.debug,
      "Starting sql server with {} byte limit on fetch requests",
...
config::broker_authn_method get_authn_method(const net::connection& conn) {
    ...
    //    Use it
    // Else if sql_enable_authorization is not set
    ...
    // if sql_enable_authorization is not set, use sasl iff enable_sasl
    if (
      !config.sql_enable_authorization().has_value()
      && config.enable_sasl()) {
...
/*static*/ std::vector<bool> server::convert_api_names_to_key_bitmap(
  const std::vector<ss::sstring>& api_names) {
    ...
    for (const ss::sstring& api_name : api_names) {
        ...
        vlog(klog.warn, "Unrecognized SQL API name: {}", api_name);
    }
    return res;
}
...
ss::future<> server::apply(ss::lw_shared_ptr<net::connection> conn) {
    ...
    auto ctx = ss::make_lw_shared<connection_context>(
      *this,
      ...
      authz_enabled,
      mtls_state,
      config::shard_local_cfg().sql_request_max_bytes.bind(),
      config::shard_local_cfg()
        .sql_throughput_controlled_api_keys.bind<std::vector<bool>>(
...
ss::future<response_ptr> list_groups_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group g) {
    ...
    auto additional_resources_func = [&resp]() {
        std::vector<sql::group_id> groups;
...
ss::future<response_ptr> sasl_handshake_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group g) {
    ...
    const bool has_sql_gssapi = ctx.feature_table().local().is_active(
      features::feature::sql_gssapi);
    if (has_sql_gssapi && supports("GSSAPI")) {
...
process_result_stages join_group_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group g) {
    ...
    if (ctx.header().client_id) {
        request.client_id = sql::client_id(
          ss::sstring(*ctx.header().client_id));
    }
    request.client_host = sql::client_host(
...
ss::future<response_ptr> delete_groups_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group g) {
    ...
    std::vector<sql::group_id> unauthorized(
        ...
        auto unknowns_it = std::partition(
          ...
          [&topic, &ctx](const offset_delete_request_partition& partition) {
              return ctx.metadata_cache().contains(
                model::topic_namespace_view(model::sql_namespace, topic.name),
...
delete_topics_handler::handle(request_context ctx, ss::smp_service_group) {
    ...
    auto sql_nodelete_topics
      = config::shard_local_cfg().sql_nodelete_topics();
    if (config::shard_local_cfg().audit_enabled()) {
        sql_nodelete_topics.push_back(model::sql_audit_logging_topic());
    }
    auto nodelete_it = std::partition(
      request.data.topic_names.begin(),
      request.data.topic_names.end(),
      [&sql_nodelete_topics](const model::topic& topic) {
          return std::find(
                   sql_nodelete_topics.begin(),
                   sql_nodelete_topics.end(),
                   topic)
                 == sql_nodelete_topics.end();
      });
    ...
    auto quota_exceeded_it = co_await ssx::partition(
      ...
      [&ctx, &resp_delay](const model::topic& t) {
          const auto cfg = ctx.metadata_cache().get_topic_cfg(
            model::topic_namespace_view(model::sql_namespace, t));
        ...
        std::transform(
          ...
          [](model::topic& tp) {
              return model::topic_namespace(
                model::sql_namespace, std::move(tp));
          });
        ...
        if (request.data.transactional_id) {
            ...
            return ctx.tx_gateway_frontend()
              ...
              .then([&ctx](cluster::init_tm_tx_reply r) {
                  ...
                  switch (r.ec) {
                  case cluster::tx_errc::none:
                      reply.data.producer_id = sql::producer_id(r.pid.id);
        ...
        return ctx.id_allocator_frontend()
          ...
          .then([&ctx](cluster::allocate_id_reply r) {
              ...
              if (r.ec == cluster::errc::success) {
                  reply.data.producer_id = sql::producer_id(r.id);
...
ss::future<response_ptr> create_acls_handler::handle(
  request_context ctx, [[maybe_unused]] ss::smp_service_group ssg) {
    ...
    // sql: put things back in the same order :(
...
offset_commit_handler::handle(request_context ctx, ss::smp_service_group ssg) {
    ...
    for (auto it = octx.request.data.topics.begin();
        ...
        model::topic_namespace_view tn(model::sql_namespace, topic_name);
...
list_transactions_handler::handle(request_context ctx, ss::smp_service_group) {
    ...
    if (!co_await tx_registry.ensure_tx_topic_exists()) {
        vlog(
          ...
          model::tx_manager_nt);
        response.data.error_code = sql::error_code::unknown_server_error;
        co_return co_await ctx.respond(std::move(response));
    }
    ...
    if (txs.has_value()) {
        for (const auto& tx : txs.value()) {
            ...
            if (filter_tx(request, tx)) {
                ...
                tx_state.producer_id = sql::producer_id(tx.pid.id);
                tx_state.transaction_state = ss::sstring(tx.get_sql_status());
