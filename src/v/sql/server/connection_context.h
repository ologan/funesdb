...
class sql_api_version_not_supported_exception : public std::runtime_error {
public:
    explicit sql_api_version_not_supported_exception(const std::string& m)
      : std::runtime_error(m) {}
};
...
public:
    ...
    /// The instance of \ref sql::server on the shard serving the connection
...
private:
    ...
    /// In SQL 2.0 compliant behaviour, it is only reported to the clients in
...
private:
    ...
    using sequence_id = named_type<uint64_t, struct sql_protocol_sequence>;
