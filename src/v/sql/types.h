...
/// SQL API request correlation.
using correlation_id = named_type<int32_t, struct sql_correlation_type>;

using client_id = named_type<ss::sstring, struct sql_client_id_type>;
using client_host = named_type<ss::sstring, struct sql_client_host_type>;

/// An unknown / missing generation id (SQL protocol specific)
...
// Unknown/missing/not initialized session (SQL protocol specific)
static constexpr fetch_session_id invalid_fetch_session_id(0);
/**
 * Used by the client to start new fetch session. (SQL protocol specific)
 */
static constexpr fetch_session_epoch initial_fetch_session_epoch(0);
/**
 * Used by the client to close existing fetch session. (SQL protocol specific)
 */
static constexpr fetch_session_epoch final_fetch_session_epoch(-1);

// TODO Ben: Why is this an undefined reference for funesproxy when defined in
// sql/requests.cc
...
///
/// NOTE: for efficiency this structure is shared between sql request
...
inline sql::leader_epoch leader_epoch_from_term(model::term_id term) {
    try {
        return sql::leader_epoch(
          boost::numeric_cast<sql::leader_epoch::type>(term()));
    } catch (boost::bad_numeric_cast&) {
        return sql::invalid_leader_epoch;
    }
}
/**
...
template<>
struct adl<sql::member_protocol> {
    void to(iobuf& out, sql::member_protocol p) {
        iobuf md = bytes_to_iobuf(p.metadata);
        reflection::serialize(out, p.name, md);
    }

    sql::member_protocol from(iobuf_parser& in) {
        return sql::member_protocol{
          .name = adl<sql::protocol_name>{}.from(in),
