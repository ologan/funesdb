...
/// SQL API key.
using api_key = named_type<int16_t, struct sql_api_key>;

/// SQL API version.
using api_version = named_type<int16_t, struct sql_api_version>;

/// SQL group identifier.
using group_id = named_type<ss::sstring, struct sql_group_id>;

/// SQL group member identifier.
using member_id = named_type<ss::sstring, struct sql_member_id>;

/// An unknown / missing member id (SQL protocol specific)
inline const member_id unknown_member_id("");

/// SQL group instance identifier.
using group_instance_id
  = named_type<ss::sstring, struct sql_group_instance_id>;

using leader_epoch = named_type<int32_t, struct leader_epoch_tag>;
/**
 * Used to mark that leader epoch is not intended to be used (SQL protocol
 * specific)
 */
inline constexpr leader_epoch invalid_leader_epoch(-1);

/// SQL group generation identifier.
using generation_id = named_type<int32_t, struct sql_generation_id>;

/// SQL group protocol type.
using protocol_type = named_type<ss::sstring, struct sql_protocol_type>;

/// SQL group protocol name.
using protocol_name = named_type<ss::sstring, struct sql_protocol>;

/// SQL transactional id identifier.
using transactional_id = named_type<ss::sstring, struct sql_transactional_id>;

/// SQL producer id identifier.
using producer_id = named_type<int64_t, struct sql_producer_id>;
...
/// Used to signify if a sql request will never be interpreted as flexible.
...
template<typename T>
concept SQLApi = requires(T request) {
...
inline const sql::protocol_type consumer_group_protocol_type("consumer");
