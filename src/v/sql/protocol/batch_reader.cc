...
record_batch_info read_record_batch_info(iobuf_const_parser& in) {
    if (unlikely(in.bytes_left() < internal::sql_header_size)) {
        throw exception(
          error_code::corrupt_message,
          fmt_with_ctx(
            fmt::format,
            "Invalid sql header parsing: Only {} bytes remain",
            in.bytes_left()));
    }
    ...
    // size_bytes are  the normal sql batch length minus the `IGNORED`
    const int32_t size_bytes
      = batch_length - internal::sql_header_size
    ...
    if (unlikely(total_bytes_consumed != internal::sql_header_size)) {
        throw exception(
          ...
          fmt_with_ctx(
            fmt::format,
            "Invalid sql header parsing. Must consume exactly: {} bytes, but "
            "consumed: {} bytes",
        ...
        const auto consume_one = [this, &batches]() {
            ...
            if (likely(kba.v2_format && kba.valid_crc && kba.batch)) {
                ...
                return ss::make_exception_future<>(exception(
                  sql::error_code::corrupt_message,
                  fmt_with_ctx(
                    fmt::format, "Invalid sql record parsing: {}", msg)));
            }
        };
