...
std::string_view error_code_to_str(error_code error) {
    ...
    case error_code::sql_storage_error:
        return "sql_storage_error";
...
std::error_code make_error_code(sql::error_code ec) {
...
const std::error_category& error_category() noexcept {
    struct error_category final : std::error_category {
        ...
        std::string message(int ec) const override {
            return std::string(
              sql::error_code_to_str(static_cast<sql::error_code>(ec)));
        }
    };
