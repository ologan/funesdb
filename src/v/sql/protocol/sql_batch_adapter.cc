...
#include "sql/protocol/sql_batch_adapter.h"

#include "bytes/iobuf.h"
#include "compression/compression.h"
#include "hashing/crc32c.h"
#include "sql/protocol/legacy_message.h"
...
model::record_batch_header sql_batch_adapter::read_header(iobuf_parser& in) {
    ...
    // size_bytes are  the normal sql batch length minus the `IGNORED` fields
    // note that we are wire compatible with SQL with everything below the CRC
    // Note:
    //   if you change this, please remember to change batch_consumer.h
    int32_t size_bytes
      = batch_length - static_cast<int32_t>(internal::sql_header_size)
        + static_cast<int32_t>(
          model::packed_record_batch_header_size
          // SQL *does not* include the first 2 fields in the size calculation
    ...
    if (unlikely(total_bytes_consumed != internal::sql_header_size)) {
        throw std::runtime_error(fmt::format(
          "Invalid sql header parsing. Must consume exactly:{}, but "
          "consumed:{}",
          internal::sql_header_size,
...
void sql_batch_adapter::verify_crc(int32_t expected_crc, iobuf_parser in) {
    auto crc = crc::crc32c();
    ...
    if (unlikely((uint32_t)expected_crc != crc.value())) {
        ...
        vlog(
          klog.warn,
          "Cannot validate SQL record batch. Missmatching CRC. Expected:{}, "
          "Got:{}",
          expected_crc,
          crc.value());
    } else {
...
iobuf sql_batch_adapter::adapt(iobuf&& kbatch) {
    // The batch size given in the sql header does not include the offset
    // preceeding the length field nor the size of the length field itself.
    constexpr size_t sql_length_diff
      = sizeof(model::record_batch_header::base_offset)
        + sizeof(model::record_batch_header::size_bytes);

    if (unlikely(kbatch.size_bytes() < sql_length_diff)) {
        vlog(klog.error, "kbatch is unexpectedly small");
        return iobuf{};
    }

    auto batch_length =
      [peeker{iobuf_parser(kbatch.share(0, sql_length_diff))}]() mutable {
          peeker.skip(sizeof(model::record_batch_header::base_offset));
          return peeker.consume_be_type<int32_t>() + sql_length_diff;
      }();
 ...
 * accumulated in the new-style funes batch format. Compressed messages
 ...
 *
 * TODO
 *   - https://github.com/redpanda-data/funes/issues/1396
 */
void sql_batch_adapter::convert_message_set(
  ...
  bool expect_uncompressed) {
    ...
    while (parser.bytes_left()) {
        ...
        /*
         * lz4 with magic_0 was implemented in sql with a bug and requires
...
void sql_batch_adapter::adapt_with_version(
