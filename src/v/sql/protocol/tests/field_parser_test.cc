...
sql::tagged_fields make_random_tags(size_t n) {
    sql::tagged_fields::type tags;
    for (uint32_t i = 0; i < n; ++i) {
        tags.emplace(n, random_generators::get_bytes());
    }
    return sql::tagged_fields(std::move(tags));
}

sql::tagged_fields copy_tags(const sql::tagged_fields& otags) {
    sql::tagged_fields::type tags;
...
SEASTAR_THREAD_TEST_CASE(serde_tags) {
    ...
    /// Deserialize the tags with the sql::protocol::decoder
    sql::protocol::decoder reader(std::move(buf));
    ...
    iobuf result;
    sql::protocol::encoder end_writer(result);
...
void write_flex(T& type, iobuf& buf) {
    ...
    if constexpr (std::is_same_v<T, std::vector<test_struct>>) {
        writer.write_flex_array(
          type, [](test_struct& ts, sql::protocol::encoder& writer) {
              writer.write_flex(ts.field_a);
              writer.write(ts.field_b);
              writer.write_tags(sql::tagged_fields{});
          });
    ...
    } else if constexpr (std::is_same_v<T, sql::uuid>) {
        writer.write(type);
    } else {
...
T read_flex(iobuf buf) {
    ...
    if constexpr (std::is_same_v<T, ss::sstring>) {
        ...
        return reader.read_flex_array([](sql::protocol::decoder& reader) {
            test_struct v;
...
SEASTAR_THREAD_TEST_CASE(serde_flex_types) {
    ...
    };
    {
        ...
        auto uuid = sql::uuid::from_string(encoded);
    ...
    }
    {
        ...
        sql::protocol::encoder writer(writers_buf);
        writer.write_flex(std::move(data));

        sql::protocol::decoder reader(std::move(writers_buf));
