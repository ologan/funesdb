...
#include "sql/server/handlers/handlers.h"
...
/// Signifies a request if true, otherwise is_a sql response
using is_sql_request = ss::bool_class<struct is_sql_request_tag>;

bytes invoke_franz_harness(
  api_key key, api_version v, is_sql_request is_request) {
    ...
    ss::sstring stdout;
    {
        ...
        boost::process::child c(
          ...
          boost::process::args({
            ...
            "-version",
            std::to_string(v()),
            (is_request == is_sql_request::yes ? "-is-request=true"
                                                 : "-is-request=false"),
          }),
        ...
        vassert(
          c.exit_code() == 0,
          "sql-request-generator exited with non-zero status");
...
/// sql_batch_adapter for every partition in a request must be queried for its
/// legacy success status
bool legacy_batch_failures(
  const sql::produce_request_data& req, api_version) {
    return std::any_of(
...
/// Comprehensive sql protocol tests
///
/// This test is performed for each sql api, at all supported levels, for
/// request and response types
//
/// A small go program that depends on franz-go generates a sample request or
/// response for a given api/version combination. Funes attempts to
/// deserialize and reserialize the data, comparing the binary representation
/// with the initial.
template<typename T>
void check_sql_binary_format(
  api_key key, api_version version, is_sql_request is_request) {
    decltype(T::data) r;
    auto result = invoke_franz_harness(key, version, is_request);
    {
        if constexpr (HasPrimitiveDecode<decltype(r)>) {
            ...
            sql::protocol::decoder rdr(bytes_to_iobuf(result));
    ...
    bytes b;
    {
        if constexpr (std::is_same_v<T, produce_request>) {
            /// Funes currently does not support encoding legacy batches,
...
template<sql::SQLApiHandlerAny H>
void check_proto_compat() {
    for (auto version = H::min_supported; version <= H::max_supported();
         ++version) {
        TEST_COMPAT_CHECK_NO_THROW(
          check_sql_binary_format<typename H::api::request_type>(
            H::api::key, version, is_sql_request::yes),
          H::api::key,
          version,
          true);
        TEST_COMPAT_CHECK_NO_THROW(
          check_sql_binary_format<typename H::api::response_type>(
            H::api::key, version, is_sql_request::no),
          H::api::key,
          version,
          false);
    }
...
// When a new sql message is added that contains optional tag fields,
...
using tag_field_entries = sql::type_list<
...
long create_default_and_non_default_data(
  ...
  decltype(create_topics_response::data)& default_data) {
    ...
    default_data.topics.at(0).topic_config_error_code = sql::error_code{0};
...
bool validate_buffer_against_data(
  const T& check_data, api_version version, const bytes& buffer) {
    ...
    if constexpr (HasPrimitiveDecode<decltype(r)>) {
        ...
        sql::protocol::decoder rdr(bytes_to_iobuf(buffer));
        r.decode(rdr, version);
    }
...
template<typename T>
void check_sql_tag_format(api_version version, bool is_request) {
...
template<typename T>
concept FieldEntry = sql::SQLApi<typename T::api> && requires(T t) {
...
void check_tag_request() {
    if constexpr (T::request) {
        check_sql_tag_format<typename T::api::request_type>(
          T::test_version, T::request);
    } else {
        check_sql_tag_format<typename T::api::response_type>(
          T::test_version, T::request);
    }
}

template<typename... Ts>
void check_all_tag_requests(sql::type_list<Ts...>) {
    (check_tag_request<Ts>(), ...);
}

BOOST_AUTO_TEST_CASE(test_sql_protocol_compat) {
    check_all_requests(sql::request_types{});
}
