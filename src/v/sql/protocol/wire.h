...
public:
    ...
    // wrap a writer in a sql bytes array object. the writer should return
...
inline void writer_serialize_batch(encoder& w, model::record_batch&& batch) {
    /*
     * calculate batch size expected by sql client.
     *
     * 1. records_size = batch.size_bytes() - RP header size;
     * 2. sql_total = records_size + sql header size;
     * 3. batch_size = sql_total - sizeof(offset) - sizeof(length);
     *
     ...
     * the sql batch size described below, RP batch size includes the size
     * of the length field itself.
     *
     * The adjustment in (3) is because the batch size given in the sql
    ...
    auto size = batch.size_bytes() - model::packed_record_batch_header_size
                + internal::sql_header_size - sizeof(int64_t)
                - sizeof(int32_t);
