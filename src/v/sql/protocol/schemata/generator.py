...
#
# Code generator for sql messages
# =================================
#
# Message schemas are taken from the 2.4 branch.
#
# SQL reference on schema:
#     https://github.com/apache/sql/blob/2.4/clients/src/main/resources/common/message/README.md
#
# TODO:
...
#    - basic_type_map
#
# control how the types in a sql message schema translate to funes types.
...
path_type_map = {
    ...
    "OffsetFetchResponseData": {
        "Topics": {
            "Partitions": {
                ...
                "CommittedLeaderEpoch": ("sql::leader_epoch", "int32"),
            },
        }
    },
    "OffsetCommitRequestData": {
        ...
        "MemberId": ("sql::member_id", "string"),
        "GroupInstanceId": ("sql::group_instance_id", "string"),
    },
    ...
    "OffsetDeleteResponseData": {
        "ErrorCode": ("sql::error_code", "int16"),
        "Topics": {
            "Partitions": {
                "PartitionIndex": ("model::partition_id", "int32"),
                "ErrorCode": ("sql::error_code", "int16"),
    ...
    "JoinGroupRequestData": {
        "MemberId": ("sql::member_id", "string"),
        "GroupInstanceId": ("sql::group_instance_id", "string"),
        "ProtocolType": ("sql::protocol_type", "string"),
        "Protocols": {
            "Name": ("sql::protocol_name", "string"),
        },
    },
    "JoinGroupResponseData": {
        "GenerationId": ("sql::generation_id", "int32"),
        "ProtocolName": ("sql::protocol_name", "string"),
        "Leader": ("sql::member_id", "string"),
        "MemberId": ("sql::member_id", "string"),
        "Members": {
            "MemberId": ("sql::member_id", "string"),
            "GroupInstanceId": ("sql::group_instance_id", "string"),
    ...
    "CreateTopicsResponseData": {
        "Topics": {
            "Configs": {
                "ConfigSource": ("sql::describe_configs_source", "int8"),
            },
            "TopicConfigErrorCode": ("sql::error_code", "int16"),
        },
    },
    "FindCoordinatorRequestData": {
        "KeyType": ("sql::coordinator_type", "int8"),
    },
    "ListOffsetRequestData": {
        "Topics": {
            "Partitions": {
                ...
                "CurrentLeaderEpoch": ("sql::leader_epoch", "int32"),
            },
        },
    },
    "ListOffsetResponseData": {
        "Topics": {
            "Partitions": {
                ...
                "LeaderEpoch": ("sql::leader_epoch", "int32"),
            },
        },
    },
    ...
    "DescribeConfigsRequestData": {
        "Resources": {
            "ResourceType": ("sql::config_resource_type", "int8"),
        },
    },
    "DescribeConfigsResponseData": {
        "Results": {
            ...
            "Configs": {
                "ConfigSource": ("sql::describe_configs_source", "int8"),
                "ConfigType": ("sql::describe_configs_type", "int8"),
            },
        },
    },
    "ProduceRequestData": {
        ...
        "Topics": {
            "Partitions": {
                "PartitionIndex": ("model::partition_id", "int32"),
                "Records": ("sql::produce_request_record_data", "iobuf"),
            },
        },
    },
    "ProduceResponseData": {
    ...
    "FetchResponseData": {
        "Topics": {
            "Partitions": {
                ...
                "LogStartOffset": ("model::offset", "int64"),
                "Records": ("sql::batch_reader", "fetch_record_set"),
...
# a few sql field types specify an entity type
entity_type_map = dict(
    groupId=("sql::group_id", "string"),
    transactionalId=("sql::transactional_id", "string"),
    topicName=("model::topic", "string"),
    uuid=("sql::uuid", "uuid"),
    brokerId=("model::node_id", "int32"),
    producerId=("sql::producer_id", "int64"),
)

# mapping specified as a combination of native type and field name
field_name_type_map = {
    ("int16", "ErrorCode"): ("sql::error_code", None),
...
extra_headers = {
    ...
    "sql::produce_request_record_data": dict(
        header = "sql/protocol/sql_batch_adapter.h",
    ),
    "sql::batch_reader": dict(
        header = "sql/protocol/batch_reader.h",
    ),
...
WITHOUT_DEFAULT_EQUALITY_OPERATOR = {
    'sql::batch_reader', 'sql::produce_request_record_data'
}
...
class VersionRange:
    ...
    different ways in the sql schema format to specify the bounds.
    """
...
class FieldType:
    ...
    def create(field, path):
        """
        FieldType factory based on SQL field type name:
...
class Field:
    ...
    def _funes_path_type(self):
        """
        Resolve a funes field path type override.
        """
        d = path_type_map
        for p in self._path:
            d = d.get(p, None)
    ...
    def _funes_type(self):
        """
        Resolve a funes type override.
        Lookup occurs from most to least specific.
        """
        # path overrides
        path_type = self._funes_path_type()
        if path_type:
    ...
    def decoder(self, flex):
        ...
        """
        plain_decoder, named_type = self._funes_decoder()
        if self.is_array:
    ...
    def type_name_parts(self):
        ...
        """
        name, default_value = self._funes_type()
        yield name
        if isinstance(self._type, ArrayType):
...
{%- if struct.context_field %}
    // extra context not part of sql protocol.
    // added by funes. see generator.py:make_context_field.
...
#include "sql/protocol/schemata/{{ header }}"
{%- endfor %}
...
{%- elif field.nullable() %}
{
    ...
    if (tmp) {
{%- if named_type == "sql::produce_request_record_data" %}
...
bool {{ struct.name }}::errored() const {
    ...
    if ({{ field.name }} != sql::error_code::none) {
        return true;
    }
...
# This is the schema of the json files from the sql tree. This isn't strictly
# necessary for the code generator, but it is useful. The schema verification
# performed on our input files from sql is _very_ strict. Since the json files
# in sql do not seem to have any sort of formalized structure, verification
...
# type is not a native sql type, but is still represented in the code
...
def codegen(schema_path):
    ...
    # the root struct in the schema corresponds to the root type in funes.
