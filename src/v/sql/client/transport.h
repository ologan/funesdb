...
#include "sql/protocol/flex_versions.h"
...
class sql_request_disconnected_exception : public std::runtime_error {
public:
    explicit sql_request_disconnected_exception(const std::string& msg)
...
private:
    /*
     * send a request message and process the reply. note that the sql
              ...
              return protocol::parse_size(_in).then([this, is_flexible](
                  ...
                  if (!sz) {
                      return ss::make_exception_future<iobuf>(
                        sql_request_disconnected_exception(
...
public:
     ...
     */
    template<typename T>
    requires(SQLApi<typename T::api_type>)
     ...
     * of the funes sql server.
     *
     * TODO: this will go away once the sql client implements version
