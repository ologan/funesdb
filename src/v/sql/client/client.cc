...
#include "sql/client/partitioners.h"
#include "sql/client/sasl_client.h"
  ...
  model::topic topic, std::vector<record_essence> records) {
    ...
    for (auto& pb : partition_builders) {
        partitions.emplace_back(sql::produce_request::partition{
          ...
          .records = sql::produce_request_record_data{
    ...
    auto responses = co_await ssx::parallel_transform(
      std::move(partitions),
      [this, topic](sql::produce_request::partition p) mutable
...
client::create_topic(sql::creatable_topic req) {
        ...
        return _brokers.find(controller)
          .then([req](auto broker) mutable {
              return broker->dispatch(
                sql::create_topics_request{.data{.topics{std::move(req)}}});
          })
...
client::do_list_offsets(model::topic_partition tp) {
    ...
    auto res = co_await broker->dispatch(sql::list_offsets_request{
      .data = {.topics{
        {{.name{tp.topic},
          .partitions{
...
ss::future<sql::fetch_response> client::consumer_fetch(
  const group_id& g_id,
  const member_id& name,
  ...
  std::optional<int32_t> max_bytes) {
    ...
    return gated_retry_with_mitigation([this, g_id, name, end, max_bytes]() {
        ...
        return get_consumer(g_id, name)
          ...
          .then([this](sql::fetch_response res) {
