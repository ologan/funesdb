...
FIXTURE_TEST(produce_reconnect, sql_client_fixture) {
    ...
    BOOST_REQUIRE_EQUAL(req0.error_code, sql::error_code::none);
    BOOST_REQUIRE_EQUAL(req0.base_offset, model::offset(0));

    client.stop().get();
}

FIXTURE_TEST(produce_clean_shutdown, sql_client_fixture) {
    using namespace std::chrono_literals;
    ...
    auto produce_to_unknown_topic = [&client, tp]() {
        ...
        return client.produce_record_batch(tp, std::move(bat))
          .then([](auto res) {
              BOOST_REQUIRE_EQUAL(
                res.error_code, sql::error_code::operation_not_attempted);
              BOOST_REQUIRE_EQUAL(res.base_offset, model::offset(-1));
          });
    };
