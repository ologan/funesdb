...
FIXTURE_TEST(test_config_utils, funes_thread_fixture) {
    ...
    const auto create_credentials = [&, this]() {
        return sql::client::create_client_credentials(
    ...
    {
        std::vector<config::broker_authn_endpoint> sql_api;
        sql_api.push_back(sasl_ep);
        node_cfg.sql_api.property::set_value(sql_api);
    }
    ...
    // Enable authz
    cluster_cfg.sql_enable_authorization.set_value(std::optional<bool>(true));
