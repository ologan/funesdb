...
#include "sql/client/produce_batcher.h"
...
struct produce_batcher_context {
    ...
    std::vector<ss::future<sql::produce_response::partition>> produce_futs{};
    ...
    auto handle_response(sql::error_code error = sql::error_code::none) {
        auto batch = kc::consume_front(broker_batches);
        batcher.handle_response(sql::produce_response::partition{
          .partition_index{partition_id},
          .error_code = error,
          .base_offset{
            error == sql::error_code::none ? batch.first : model::offset{-1}},
          .log_append_time_ms{model::timestamp{0}},
          .log_start_offset{model::offset{-1}},
        });
        return batch.second.record_count();
    }
    ss::future<std::vector<sql::produce_response::partition>>
    get_responses() {
    ...
    ss::future<std::vector<model::offset>> get_response_offsets() {
        return get_responses().then(
          [](std::vector<sql::produce_response::partition> results) {
              ...
              std::transform(
                ...
                [](sql::produce_response::partition p) {
...
SEASTAR_THREAD_TEST_CASE(test_partition_producer_error) {
    ...
    BOOST_REQUIRE(ctx.handle_response() == 4);
    BOOST_REQUIRE(
      ctx.handle_response(sql::error_code::not_leader_for_partition) == 4);

    auto responses = ctx.get_responses().get0();
    BOOST_REQUIRE(responses.size() == 4);
    // successes
    BOOST_REQUIRE(responses[0].base_offset == model::offset(42));
    BOOST_REQUIRE(responses[0].error_code == sql::error_code::none);
    BOOST_REQUIRE(responses[1].base_offset == model::offset(44));
    BOOST_REQUIRE(responses[1].error_code == sql::error_code::none);
    // failures
    BOOST_REQUIRE(responses[2].base_offset == model::offset(-1));
    BOOST_REQUIRE(
      responses[2].error_code == sql::error_code::not_leader_for_partition);
    BOOST_REQUIRE(responses[3].base_offset == model::offset(-1));
    BOOST_REQUIRE(
      responses[3].error_code == sql::error_code::not_leader_for_partition);
