...
consumer::consumer(
  ...
  sql::member_id name,
  ss::noncopyable_function<void(const sql::member_id&)> on_stopped,
...
ss::future<> consumer::join() {
    ...
    auto req_builder = [me{shared_from_this()}]() {
        ...
        req.client_id = sql::client_id("test_client");
        req.data = {
    ...
    if (topics.empty()) { // commit all offsets
        ...
        for (auto& t : topics) {
            for (auto& p : t.partitions) {
                p.committed_leader_epoch = sql::invalid_leader_epoch;
            }
        }
    }
...
ss::future<fetch_response> consumer::fetch(
  std::chrono::milliseconds timeout, std::optional<int32_t> max_bytes) {
    ...
    co_return co_await ss::map_reduce(
      ...
      fetch_response{
        .data
        = {.throttle_time_ms{}, .error_code = error_code::none, .session_id = sql::invalid_fetch_session_id}},
