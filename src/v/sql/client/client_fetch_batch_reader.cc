...
public:
    ...
    do_load_slice(model::timeout_clock::time_point t) final {
        if (!_batch_reader || _batch_reader->is_end_of_stream()) {
            ...
            if (res.data.error_code != sql::error_code::none) {
                throw sql::exception(
        ...
        vassert(
          ...
          "Expected sql::batch_reader to hold "
          "model::record_batch_reader::data_t");
        auto& data = std::get<data_t>(ret);
        if (data.empty()) {
            throw sql::exception(
              sql::error_code::unknown_server_error, "No records returned");
        }
    ...
    void print(std::ostream& os) final {
        os << "{funesproxy::schema_registry::client_fetcher}";
    }
...
private:
    ...
    std::optional<sql::batch_reader> _batch_reader;
};
