...
#include "funes/admin/api-doc/broker.json.hh"
#include "funes/admin/api-doc/cluster.json.hh"
#include "funes/admin/api-doc/cluster_config.json.hh"
#include "funes/admin/api-doc/config.json.hh"
#include "funes/admin/api-doc/debug.json.hh"
#include "funes/admin/api-doc/features.json.hh"
#include "funes/admin/api-doc/hbadger.json.hh"
#include "funes/admin/api-doc/partition.json.hh"
#include "funes/admin/api-doc/raft.json.hh"
#include "funes/admin/api-doc/security.json.hh"
...
static constexpr auto audit_svc_name = "Funes Admin HTTP Server";
...
admin_server::admin_server(
  ...
  ss::sharded<sql::usage_manager>& usage_manager,
  funesproxy::rest::api* http_proxy,
  funesproxy::schema_registry::api* schema_registry,
...
void admin_server::configure_admin_routes() {
    ...
    rb->register_function(_server._routes, insert_comma);
    rb->register_api_file(_server._routes, "sql");
    rb->register_function(_server._routes, insert_comma);
 ...
 * as an empty request body causes a funes crash via a rapidjson
...
ss::future<ss::httpd::redirect_exception> admin_server::redirect_to_leader(
  ss::http::request& req, model::ntp const& ntp) const {
    ...
    // - if the host in the Host header matches one of our advertised sql
    //   addresses, then assume that the peer's advertised sql address
    ...
    //
    // This will work reliably if all node configs have symmetric sql listener
    ...
    // matches one of the hostnames used for a sql listener.
    //
    ...
    if (host_hdr.empty()) {
        ...
        // See if this hostname is one of our sql advertised addresses
        auto sql_endpoints = config::node().advertised_sql_api();
        ...
        if (match_i != sql_endpoints.end()) {
            auto listener_idx = size_t(
              std::distance(sql_endpoints.begin(), match_i));

            auto leader_advertised_addrs
              = leader.broker.sql_advertised_listeners();
            ...
            vlog(
              adminlog.debug,
              "redirect: {} did not match any sql listeners, redirecting to "
      ...
      .then([controller](result<cluster::cluster_health_report> h_report) {
          ...
          for (auto& ns : h_report.value().node_states) {
              ...
              if (r_it != h_report.value().node_reports.end()) {
                  it->second.version = r_it->local_state.funes_version;
 ...
 * a funes error during a request.
 *
    ...
    if (
      ...
      && !schema_registry) {
        ...
        errors[ss::sstring(name)] = ssx::sformat(
          "{} requires schema_registry to be enabled in funes.yaml", name);
    }
}
} // namespace
...
ss::future<ss::json::json_return_type>
admin_server::sql_transfer_leadership_handler(
  std::unique_ptr<ss::http::request> req) {
...
void admin_server::register_sql_routes() {
    register_route<superuser>(
      ss::httpd::partition_json::sql_transfer_leadership,
      [this](std::unique_ptr<ss::http::request> req) {
          return sql_transfer_leadership_handler(std::move(req));
      });
}
...
admin_server::put_license_handler(std::unique_ptr<ss::http::request> req) {
    ...
    if (raw_license.empty()) {
        throw ss::httpd::bad_request_exception(
          "Missing funes license from request body");
    }
...
void admin_server::register_features_routes() {
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request>) {
          ...
          // being missing (i.e. unsure if funes is too old to have
...
admin_server::find_tx_coordinator_handler(
  std::unique_ptr<ss::http::request> req) {
    ...
    sql::transactional_id tid(transaction_id);
...
ss::json::json_return_type raw_data_to_usage_response(
  const std::vector<sql::usage_window>& total_usage, bool include_open) {
    ...
    for (size_t i = (include_open ? 0 : 1); i < total_usage.size(); ++i) {
        ...
        resp.back().sql_bytes_received_count = e.u.bytes_received;
        resp.back().sql_bytes_sent_count = e.u.bytes_sent;
    ...
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request> req) {
          ...
          return _usage_manager
            .invoke_on(
              sql::usage_manager::usage_manager_main_shard,
              [](sql::usage_manager& um) { return um.get_usage_stats(); })
...
admin_server::get_local_offsets_translated_handler(
  std::unique_ptr<ss::http::request> req) {
    static const std::string_view to_sql = "sql";
    static const std::string_view to_rp = "funes";
    const model::ntp ntp = parse_ntp_from_request(req->param);
    const auto shard = _controller->get_shard_table().local().shard_for(ntp);
    auto translate_to = to_sql;
        ...
        cluster::partition_manager& pm) {
          ...
          for (const auto offset : input) {
              try {
                  ...
                  if (translate_to == to_sql) {
                      to.sql_offset = ots->from_log_offset(offset);
                      to.rp_offset = offset;
                  } else {
                      to.rp_offset = ots->to_log_offset(offset);
                      to.sql_offset = offset;
                  }
...
admin_server::get_local_storage_usage_handler(
  std::unique_ptr<ss::http::request>) {
     ...
     * three things:
     *
     * 1. partition-based log usage (controller, sql topics, etc...)
...
ss::future<ss::json::json_return_type> admin_server::sync_local_state_handler(
  std::unique_ptr<ss::http::request> request) {
    ...
    auto ntp = parse_ntp_from_request(request->param, model::sql_namespace);
    if (need_redirect_to_leader(ntp, _metadata_cache)) {
...
admin_server::delete_cloud_storage_lifecycle(
  std::unique_ptr<ss::http::request> req) {
    ...
    cluster::nt_revision ntr{
      .nt = model::topic_namespace(model::sql_namespace, model::topic{topic}),
...
ss::future<std::unique_ptr<ss::http::reply>> admin_server::get_manifest(
  ...
  std::unique_ptr<ss::http::reply> rep) {
    model::ntp ntp = parse_ntp_from_request(req->param, model::sql_namespace);
...
ss::future<> admin_server::restart_funes_service(service_kind service) {
    switch (service) {
    case service_kind::schema_registry:
...
admin_server::restart_service_handler(std::unique_ptr<ss::http::request> req) {
    ...
    vlog(
      adminlog.info, "Restart funes service: {}", to_string_view(*service));
    co_await restart_funes_service(*service);
    co_return ss::json::json_return_type(ss::json::json_void());
}
