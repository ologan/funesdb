...
#include "funes/monitor_unsafe_log_flag.h"
...
inline const auto funes_start_time{
...
public:
    ...
    ss::sharded<sql::coordinator_ntp_mapper> coordinator_ntp_mapper;
    ss::sharded<sql::group_router> group_router;
    ss::sharded<sql::quota_manager> quota_mgr;
    ss::sharded<sql::snc_quota_manager> snc_quota_mgr;
    ss::sharded<sql::rm_group_frontend> rm_group_frontend;
    ...
    ss::sharded<sql::server> _sql_server;

    const std::unique_ptr<funesproxy::schema_registry::api>& schema_registry() {
...
private:
    ...
    // Constructs services across shards meant for Funes runtime.
    void
    wire_up_runtime_services(model::node_id node_id, ::stop_signal& app_signal);
    void configure_admin_server();
    void wire_up_funes_services(model::node_id, ::stop_signal& app_signal);

    void load_feature_table_snapshot();

    void trigger_abort_source();

    // Starts the services meant for Funes runtime. Must be called after
    ...
    void start_sql(const model::node_id&, ::stop_signal&);
    ...
    // to be replicated to our controller log before listening for SQL
    // requests.
    std::optional<model::offset> _await_controller_last_applied;

    std::optional<funesproxy::rest::configuration> _proxy_config;
    std::optional<sql::client::configuration> _proxy_client_config;
    std::optional<funesproxy::schema_registry::configuration>
      _schema_reg_config;
    std::optional<sql::client::configuration> _schema_reg_client_config;
    std::optional<sql::client::configuration> _audit_log_client_config;
    ...
    ss::sharded<sql::group_manager> _group_manager;
    ss::sharded<rpc::rpc_server> _rpc;
    ss::sharded<admin_server> _admin;
    ss::sharded<net::conn_quota> _sql_conn_quotas;
    std::unique_ptr<funesproxy::rest::api> _proxy;
    std::unique_ptr<funesproxy::schema_registry::api> _schema_registry;
    ...
    std::unique_ptr<sql::rm_group_proxy_impl> _rm_group_proxy;
