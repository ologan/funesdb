...
BOOST_AUTO_TEST_CASE(test_flag_with_arguments) {
    ...
    cfg.add_options()("funes-cfg", po::value<std::string>(), "");

    {
        argv a{"funes --funes-cfg f.yaml"};
        auto [ac, av] = a.args();
        cli_parser parser{
...
BOOST_AUTO_TEST_CASE(test_flags_with_arguments_and_bad_pos_arg) {
    ...
    argv a{"funes --funes-cfg f.yaml testing"};
    auto [ac, av] = a.args();
    cli_parser parser{
      ac, av, cli_parser::app_opts{cfg}, cli_parser::ss_opts{ss}, test_log};
    po::variables_map vm;
...
BOOST_AUTO_TEST_CASE(test_funes_and_ss_opts) {
    seastar::app_template app;

    app.add_options()("funes-cfg", po::value<std::string>(), "");
    const auto& cfg = app.get_options_description();
    const auto& ss = app.get_conf_file_options_description();

    {
        argv a{"funes --funes-cfg f.yaml --smp 2 --memory 4G --mbind 1 "
               "--num-io-groups=1"};
        auto [ac, av] = a.args();
        cli_parser parser{
          ac, av, cli_parser::app_opts{cfg}, cli_parser::ss_opts{ss}, test_log};
