...
#include "sql/server/handlers/topics/topic_utils.h"
...
public:
    ...
    funes_thread_fixture(
      ...
      std::optional<uint32_t> sql_admin_topic_api_rate = std::nullopt,
      bool enable_data_transforms = false)
      : app(ssx::sformat("funes-{}", node_id()))
      ...
      , app_signal(std::make_unique<::stop_signal>()) {
        ...
        app.initialize(
          ...
          proxy_client_config(sql_port),
          schema_reg_config(schema_reg_port),
          proxy_client_config(sql_port),
          audit_log_client_config(sql_port),
          sch_groups);
        ...
        proto
          .start(
            &configs,
            app.smp_service_groups.sql_smp_sg(),
    ...
    explicit funes_thread_fixture(std::filesystem::path existing_data_dir)
      : funes_thread_fixture(
        model::node_id(1),
        9092,
        33145,
    ...
    // Start funes with shadow indexing enabled
    explicit funes_thread_fixture(
    ...
    // Start funes with shadow indexing enabled, but do not enable
    ...
    void configure(
      ...
      bool data_transforms_enabled = false) {
        ...
        ss::smp::invoke_on_all([node_id,
            ...
            if (sql_admin_topic_api_rate) {
                config.get("sql_admin_topic_api_rate")
                  .set_value(sql_admin_topic_api_rate);
            }
    ...
    YAML::Node proxy_config(uint16_t proxy_port = 8082) {
        funesproxy::rest::configuration cfg;
        cfg.get("funesproxy_api")
    ...
    YAML::Node proxy_client_config(
      uint16_t sql_api_port = config::node().sql_api()[0].address.port()) {
        sql::client::configuration cfg;
        net::unresolved_address sql_api{
          config::node().sql_api()[0].address.host(), sql_api_port};
        cfg.brokers.set_value(
          std::vector<net::unresolved_address>({sql_api}));
        return to_yaml(cfg, config::redact_secrets::no);
    }

    YAML::Node schema_reg_config(uint16_t listen_port = 8081) {
        funesproxy::schema_registry::configuration cfg;
    ...
    ss::future<sql::client::transport>
    make_sql_client(std::optional<ss::sstring> client_id = "test_client") {
        return ss::make_ready_future<sql::client::transport>(
          net::base_transport::configuration{
            .server_addr = {"127.0.0.1", sql_port},
          },
          std::move(client_id));
    }
    ...
    make_default_ntp(model::topic topic, model::partition_id partition) {
        return model::ntp(model::sql_namespace, std::move(topic), partition);
    }
    ...
    make_data(std::optional<model::timestamp> base_ts = std::nullopt) {
        ...
        model::topic_namespace tp_ns{model::sql_namespace, ktp.get_topic()};
        while (true) {
    ...
    using conn_ptr = ss::lw_shared_ptr<sql::connection_context>;
    ...
    conn_ptr make_connection_context() {
        ...
        return ss::make_lw_shared<sql::connection_context>(
          proto.local(),
          nullptr,
          std::move(sasl),
          false,
          ...
          config::mock_property<std::vector<ss::sstring>>({"produce", "fetch"})
            .bind<std::vector<bool>>(
              &sql::server::convert_api_names_to_key_bitmap));
    }

    sql::request_context
    make_request_context(sql::fetch_request& request, conn_ptr conn = {}) {
        if (!conn) {
            conn = make_connection_context();
        }

        sql::request_header header{
          .key = sql::fetch_handler::api::key,
          .version = sql::fetch_handler::max_supported};

        iobuf buf;
        sql::protocol::encoder writer(buf);
        request.encode(writer, header.version);

        return sql::request_context(
          conn,
          std::move(header),
          std::move(buf),
    ...
    sql::request_context make_request_context() {
        sql::fetch_request request;
