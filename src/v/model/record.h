...
///
/// Record attributes are part of the SQL record format:
///
///   https://sql.apache.org/documentation/#record
///
/// The record attributes in SQL are unused (as of 3 Oct 2019). However, by
/// including them here (1) it is easier to manage the translation from the
/// on-disk record size to that used by sql, and (2) we may track attributes
...
private:
    ...
    // Bits 4 and 5 are used by SQL and thus reserved.
  ...
  : serde::envelope<
    ...
    struct context
      : serde::envelope<context, serde::version<0>, serde::compat_version<0>> {
        ...
        /*
         * term isn't part of the upstream sql batch format, but we use it
         * along with other context for tracking some funes-specific info.
         *
         * one spot where term is used is during fetch to populate the sql
         ...
         *
         * this all causes a problem for the sql/request_parser_test which
         * validates binary equivalence of requests generated from a thirdparty
         * sql client and those generated by funes. in this case funes
         ...
         * default values were all compatible. to solve this the sql batch
    ...
    offset base_offset;
    /// \brief funes extension
    record_batch_type type;
    int32_t crc{0};

    // -- below the CRC are checksummed by the sql crc. see @crc field
...
public:
     ...
     *
     * The primary use case for this interface is supporting sql's log append
