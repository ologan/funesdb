...
std::error_code validate_sql_topic_name(const model::topic& tpc) {
    static constexpr size_t sql_max_topic_name_length = 249;
    const auto& name = tpc();
    if (name.empty()) {
    ...
    if (name.length() > sql_max_topic_name_length) {
