...
template<typename T>
concept SQLRequestType = std::same_as<T, typename T::api_type::request_type>;

template<typename F>
concept SQLRequestFactory = SQLRequestType<std::invoke_result_t<F>>;

ss::shard_id inline consumer_shard(const sql::group_id& g_id) {
...
public:
    ...
    struct request_t : base::request_t {
        ...
        template<std::invocable<sql_client_cache&> Func>
        auto dispatch(Func&& func) {
            ...
            return service().client_cache().invoke_on(
              ...
              [func{std::forward<Func>(func)}](
                sql_client_cache& cache) mutable {
                  return std::invoke(std::move(func), cache);
              });
        }

        template<std::invocable<sql::client::client&> Func>
        auto dispatch(Func&& func) {
            switch (authn_method) {
        ...
        template<std::invocable<sql::client::client&> Func>
        auto dispatch(sql::group_id const& group_id, Func&& func) {
            switch (authn_method) {
            case config::rest_authn_method::none: {
        ...
        template<impl::SQLRequestFactory Func>
        auto dispatch(Func&& func) {
