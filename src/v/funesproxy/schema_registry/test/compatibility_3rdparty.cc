...
#include "funesproxy/schema_registry/exceptions.h"
...
#include "funesproxy/schema_registry/util.h"
...
namespace pps = funesproxy::schema_registry;
...
SEASTAR_THREAD_TEST_CASE(test_consume_to_store_3rdparty) {
    ...
    // This sql client will not be used by the sequencer
    // (which itself is only instantiated to receive consume_to_store's
    //  offset updates), is just needed for constructor;
    ss::sharded<sql::client::client> dummy_sql_client;
    dummy_sql_client
      .start(
        to_yaml(sql::client::configuration{}, config::redact_secrets::no))
      .get();
    auto stop_sql_client = ss::defer(
      [&dummy_sql_client]() { dummy_sql_client.stop().get(); });
    ...
    seq
      .start(
        ...
        std::reference_wrapper(dummy_sql_client),
