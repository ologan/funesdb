...
#include "funesproxy/schema_registry/service.h"
...
#include "sql/client/client_fetch_batch_reader.h"
...
#include "funesproxy/api/api-doc/schema_registry.json.hh"
#include "funesproxy/auth_utils.h"
#include "funesproxy/error.h"
#include "funesproxy/logger.h"
#include "funesproxy/schema_registry/configuration.h"
#include "funesproxy/schema_registry/handlers.h"
#include "funesproxy/schema_registry/storage.h"
...
public:
    ...
    operator()(server::request_t rq, server::reply_t rp) const {
        ...
        } catch (sql::client::partition_error const& ex) {
            if (
              ex.error == sql::error_code::unknown_topic_or_partition
...
ss::future<> service::configure() {
    auto config = co_await sql::client::create_client_credentials(
      *_controller,
      config::shard_local_cfg(),
      _client.local().config(),
      principal);
    co_await sql::client::set_client_credentials(*config, _client);
...
ss::future<> service::mitigate_error(std::exception_ptr eptr) {
    ...
    return ss::make_exception_future<>(eptr)
      .handle_exception_type(
        [this, eptr](sql::client::broker_error const& ex) {
            if (
              ex.error == sql::error_code::sasl_authentication_failed
      ...
      .handle_exception_type([this,
                              eptr](sql::client::topic_error const& ex) {
          if (
            ex.error == sql::error_code::topic_authorization_failed
            && _has_ephemeral_credentials) {
...
ss::future<> service::create_internal_topic() {
    ...
    auto make_internal_topic = [replication_factor]() {
        ...
        return sql::creatable_topic{
          ...
          .assignments{},
          .configs{
            {.name{ss::sstring{sql::topic_property_cleanup_policy}},
             .value{"compact"}},
            {.name{ss::sstring{sql::topic_property_compression}},
            ...
            {.name{ss::sstring{sql::topic_property_retention_duration}},
             .value{retain_forever}},
            {.name{
               ss::sstring{sql::topic_property_retention_local_target_bytes}},
             .value{retain_forever}},
            {.name{
               ss::sstring{sql::topic_property_retention_local_target_ms}},
             .value{retain_forever}},
            {.name{ss::sstring{
               sql::topic_property_initial_retention_local_target_bytes}},
             .value{retain_forever}},
            {.name{ss::sstring{
               sql::topic_property_initial_retention_local_target_ms}},
             .value{retain_forever}}}};
    };
    ...
    } else if (topic.error_code == sql::error_code::not_controller) {
        vlog(plog.debug, "Schema registry: not controller");
    } else {
        throw sql::exception(
          topic.error_code,
          topic.error_message.value_or(
            sql::make_error_code(topic.error_code).message()));
    }
...
ss::future<> service::fetch_internal_topic() {
    ...
    co_await sql::client::make_client_fetch_batch_reader(
      _client.local(),
      model::schema_registry_internal_tp,
      model::offset{0},
      max_offset)
...
sql::client::configuration& service::client_config() {
