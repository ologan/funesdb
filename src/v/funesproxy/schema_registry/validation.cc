...
#include "funesproxy/schema_registry/avro.h"
#include "funesproxy/schema_registry/errors.h"
#include "funesproxy/schema_registry/protobuf.h"
#include "funesproxy/schema_registry/schema_id_cache.h"
...
#include "funesproxy/schema_registry/validation_metrics.h"
...
ss::future<std::optional<ss::sstring>> get_record_name(
  funesproxy::schema_registry::sharded_store& store,
  field field,
...
T combine(
  ...
  T dflt) {
    ...
    case funesproxy::schema_registry::schema_id_validation_mode::funes:
        return funes.value_or(dflt);
    case funesproxy::schema_registry::schema_id_validation_mode::compat:
        return funes.value_or(compat.value_or(dflt));
    }
}
...
public:
    ...
    ss::future<result> operator()(model::record_batch_reader&& rbr) {
        if (!_api) {
            ...
            co_return sql::error_code::invalid_record;
        }
        if (
  ...
  const cluster::topic_properties& props) {
    ...
    if (should_validate_schema_id(props, mode)) {
        if (!api) {
            vlog(
              plog.error,
              "{} requires schema_registry to be enabled in funes.yaml",
  ...
  model::record_batch_reader&& rbr, cluster::partition_probe* probe) {
    ...
    return (*_impl)(std::move(rbr))
      .handle_exception([](std::exception_ptr e) {
          ...
          return futurator::convert(sql::error_code::invalid_record);
      })
