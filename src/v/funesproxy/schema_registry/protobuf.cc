...
::result<ss::sstring, sql::error_code>
protobuf_schema_definition::name(std::vector<int> const& fields) const {
    if (fields.empty()) {
        return sql::error_code::invalid_record;
    }
    auto f = fields.begin();
...
template<>
struct fmt::formatter<funesproxy::schema_registry::io_error_collector::err> {
    using type = funesproxy::schema_registry::io_error_collector;
...
template<>
struct fmt::formatter<funesproxy::schema_registry::dp_error_collector::err> {
    using type = funesproxy::schema_registry::dp_error_collector;
