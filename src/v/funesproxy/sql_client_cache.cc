...
sql_client_cache::sql_client_cache(
...
client_ptr sql_client_cache::make_client(
  credential_t user, config::rest_authn_method authn_method) {
    ...
    return ss::make_lw_shared<sql::client::client>(
      to_yaml(cfg, config::redact_secrets::no));
}

std::pair<client_ptr, client_mu_ptr> sql_client_cache::fetch_or_insert(
  credential_t user, config::rest_authn_method authn_method) {
...
ss::future<> sql_client_cache::clean_stale_clients() {
...
ss::future<> sql_client_cache::start() {
    _gc_timer.arm(gc_timer_period);
    return ss::now();
}

ss::future<> sql_client_cache::stop() {
...
size_t sql_client_cache::size() const { return _cache.size(); }
size_t sql_client_cache::max_size() const { return _cache_max_size; }
