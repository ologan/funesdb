...
auto make_fetch_response(
  std::vector<model::topic_partition> tps, model::offset offset, size_t count) {
    std::vector<sql::fetch_response::partition> parts;
    for (const auto& tp : tps) {
        sql::fetch_response::partition res{tp.topic};
        res.partitions.push_back(sql::fetch_response::partition_response{
