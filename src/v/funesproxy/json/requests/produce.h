...
#include "funesproxy/json/iobuf.h"
...
public:
    ...
    using rjson_parse_result = std::vector<sql::client::record_essence>;
    ...
    bool StartObject() {
        ...
        if (state == state::records) {
            result.push_back(sql::client::record_essence{});
            state = state::record;
            return true;
        }
        return false;
...
inline void rjson_serialize(
  ...
  const sql::produce_response::partition& v) {
    w.StartObject();
    w.Key("partition");
    w.Int(v.partition_index);
    if (v.error_code != sql::error_code::none) {
        w.Key("error_code");
...
inline void rjson_serialize(
  ::json::Writer<::json::StringBuffer>& w,
  const sql::produce_response::topic& v) {
    w.StartObject();
    w.Key("offsets");
    w.StartArray();
