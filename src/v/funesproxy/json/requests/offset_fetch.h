...
inline std::vector<sql::offset_fetch_request_topic>
partitions_request_to_offset_request(std::vector<model::topic_partition> tps) {
    std::vector<sql::offset_fetch_request_topic> res;
    if (tps.empty()) {
        return res;
    }

    std::sort(tps.begin(), tps.end());
    res.push_back(sql::offset_fetch_request_topic{tps.front().topic, {}});
    for (auto& tp : tps) {
        if (tp.topic != res.back().name) {
            res.push_back(
              sql::offset_fetch_request_topic{std::move(tp.topic), {}});
        }
...
inline void rjson_serialize(
  ...
  const sql::offset_fetch_response_topic& v) {
...
inline void rjson_serialize(
  ::json::Writer<::json::StringBuffer>& w,
  const sql::offset_fetch_response& v) {
    w.StartObject();
    w.Key("offsets");
    w.StartArray();
