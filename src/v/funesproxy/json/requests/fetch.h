...
template<>
class rjson_serialize_impl<sql::fetch_response> {
public:
    explicit rjson_serialize_impl(serialization_format fmt)
      : _fmt(fmt) {}

    bool operator()(
      ::json::Writer<::json::StringBuffer>& w, sql::fetch_response&& res) {
        // Eager check for errors
        for (auto& v : res) {
            if (v.partition_response->error_code != sql::error_code::none) {
