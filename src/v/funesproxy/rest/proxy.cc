...
#include "funesproxy/api/api-doc/rest.json.hh"
#include "funesproxy/auth_utils.h"
#include "funesproxy/logger.h"
#include "funesproxy/parsing/exceptions.h"
#include "funesproxy/parsing/from_chars.h"
#include "funesproxy/rest/configuration.h"
#include "funesproxy/rest/handlers.h"
...
const security::acl_principal principal{
  security::principal_type::ephemeral_user, "__funesproxy"};
...
ss::future<> proxy::start() {
    ...
    return _server.start(
      _config.funesproxy_api(),
      _config.funesproxy_api_tls(),
      _config.advertised_funesproxy_api());
}
...
sql::client::configuration& proxy::client_config() {
...
ss::future<> proxy::do_start() {
    ...
    try {
        co_await configure();
        vlog(plog.info, "Funesproxy successfully initialized");
    } catch (...) {
        vlog(
          plog.error,
          "Funesproxy failed to initialize: {}",
