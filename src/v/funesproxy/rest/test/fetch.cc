...
#include "funesproxy/test/funesproxy_fixture.h"
...
FIXTURE_TEST(funesproxy_fetch, funesproxy_test_fixture) {
...
FIXTURE_TEST(funesproxy_fetch_json_v2, funesproxy_test_fixture) {
    using namespace std::chrono_literals;
   ...
   "records":[
...

    {
        ...
        BOOST_REQUIRE_EQUAL(
          res.body,
          R"([{"topic":"t","key":null,"value":{"object":["vectorized"]},"partition":0,"offset":0},{"topic":"t","key":null,"value":{"object":["funesproxy"]},"partition":0,"offset":1}])");
    }
}

FIXTURE_TEST(funesproxy_fetch_binary_with_json2, funesproxy_test_fixture) {
    using namespace std::chrono_literals;
    ...
    auto client = make_proxy_client();
    // Sends the string "funesproxy" (without quotes) base64 encoded
