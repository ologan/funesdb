...
auto get_consumer_offsets(
  http::client& client,
  const sql::group_id& g_id,
  const sql::member_id& m_id) {
...
FIXTURE_TEST(funesproxy_consumer_group, funesproxy_test_fixture) {
    ...
    sql::group_id group_id{"test_group"};
    sql::member_id member_id{sql::no_member};
    {
...
FIXTURE_TEST(
  funesproxy_consumer_group_fetch_binary_with_jsonv2, funesproxy_test_fixture) {
    using namespace std::chrono_literals;
    info("Waiting for leadership");
    wait_for_controller_leadership().get();
    info("Connecting client");
