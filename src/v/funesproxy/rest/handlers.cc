...
#include "funesproxy/json/requests/brokers.h"
#include "funesproxy/json/requests/create_consumer.h"
#include "funesproxy/json/requests/error_reply.h"
#include "funesproxy/json/requests/fetch.h"
#include "funesproxy/json/requests/offset_commit.h"
#include "funesproxy/json/requests/offset_fetch.h"
#include "funesproxy/json/requests/partition_offsets.h"
#include "funesproxy/json/requests/partitions.h"
#include "funesproxy/json/requests/produce.h"
#include "funesproxy/json/requests/subscribe_consumer.h"
#include "funesproxy/json/rjson_util.h"
#include "funesproxy/sql_client_cache.h"
#include "funesproxy/parsing/httpd.h"
#include "funesproxy/reply.h"
...
get_brokers(server::request_t rq, server::reply_t rp) {
    ...
    auto make_metadata_req = []() {
        return sql::metadata_request{.list_all_topics = false};
    };

    co_return co_await rq.dispatch(make_metadata_req)
      .then([res_fmt, rp = std::move(rp)](
              sql::metadata_request::api_type::response_type res) mutable {
          ...
          std::transform(
            ...
            [](const sql::metadata_response::broker& b) {
                return b.node_id;
            });
...
get_topics_records(server::request_t rq, server::reply_t rp) {
    ...
    co_return co_await rq
      .dispatch([offset, timeout, max_bytes, res_fmt, tp{std::move(tp)}](
                  sql::client::client& client) mutable {
          return client
            .fetch_partition(std::move(tp), offset, max_bytes, timeout)
            .then([res_fmt](sql::fetch_response res) {
...
static ss::sstring make_consumer_uri_base(
  const server::request_t& request, const sql::group_id& group_id) {
...
create_consumer(server::request_t rq, server::reply_t rp) {
    ...
    auto group_id = parse::request_param<sql::group_id>(
      *rq.req, "group_name");
    ...
    co_return co_await rq.dispatch(
      ...
      [group_id,
       ...
       rp{std::move(rp)}](sql::client::client& client) mutable {
          ...
          return f
            .then([group_id](const sql::client::shared_consumer_t&) mutable {
...
remove_consumer(server::request_t rq, server::reply_t rp) {
    ...
    auto group_id = parse::request_param<sql::group_id>(
      *rq.req, "group_name");
    auto member_id = parse::request_param<sql::member_id>(
      *rq.req, "instance");
...
consumer_fetch(server::request_t rq, server::reply_t rp) {
    ...
    auto group_id{parse::request_param<sql::group_id>(*rq.req, "group_name")};
    auto name{parse::request_param<sql::member_id>(*rq.req, "instance")};
...
get_consumer_offsets(server::request_t rq, server::reply_t rp) {
    ...
    auto group_id{parse::request_param<sql::group_id>(*rq.req, "group_name")};
    auto member_id{parse::request_param<sql::member_id>(*rq.req, "instance")};
