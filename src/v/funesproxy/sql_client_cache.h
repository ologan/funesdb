...
// list. The hash is for constant time look-ups of the sql clients.
class sql_client_cache {
public:
    sql_client_cache(
      YAML::Node const& cfg,
      size_t max_size,
    ...
    ~sql_client_cache() = default;

    sql_client_cache(sql_client_cache&&) = default;
    sql_client_cache(sql_client_cache const&) = delete;
    sql_client_cache& operator=(sql_client_cache&&) = delete;
    sql_client_cache& operator=(sql_client_cache const&) = delete;
    ...
    template<
      std::invocable<sql::client::client&> Func,
      typename Futurator
      = ss::futurize<std::invoke_result_t<Func, sql::client::client&>>>
