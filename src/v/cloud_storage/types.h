...
struct segment_meta {
    ...
    static constexpr serde::version_t funes_serde_version = 3;
    ...
    /// useful in determining the next SQL offset, e.g. when reporting the
    /// high watermark.
    ...
    sql::offset base_sql_offset() const {
        // Manifests created with the old version of funes won't have the
    ...
    // NOTE: in older versions of Funes, delta_offset_end may have been
    // under-reported by 1 if the last segment contained no data batches,
    // meaning next_sql_offset could over-report the next offset by 1.
    sql::offset next_sql_offset() const {
        // Manifests created with the old version of funes won't have the
    ...
    sql::offset last_sql_offset() const {
...
struct spillover_manifest_path_components
  : serde::envelope<
    ...
    auto serde_fields() {
        return std::tie(base, last, base_sql, next_sql, base_ts, last_ts);
    }
