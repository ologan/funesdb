...
#   - Copyright 2020 Redpanda Data, Inc. - libc++ support and funes types
#
# -----
#
# (gdb) source funes-gdb.py
# (gdb) funes memory
#
import io
import gdb
...
class funes_memory(gdb.Command):
    ...
    * A per size-class population statistics.
    ...
    """
    def __init__(self):
        gdb.Command.__init__(self, 'funes memory', gdb.COMMAND_USER,
...
class funes_storage(gdb.Command):
    """Summarize the state of funes storage layer
    """
    def __init__(self):
        gdb.Command.__init__(self, 'funes storage', gdb.COMMAND_USER,
                             gdb.COMPLETE_COMMAND)
...
class funes_batch_cache(gdb.Command):
    """Prints content of funes batch cache
    """
    def __init__(self):
        gdb.Command.__init__(self, 'funes batch_cache', gdb.COMMAND_USER,
                             gdb.COMPLETE_COMMAND)
...
class funes_small_objects(gdb.Command):
    ...
    def __init__(self):
        gdb.Command.__init__(self, 'funes small-objects', gdb.COMMAND_USER,
                             gdb.COMPLETE_COMMAND)
    ...
    def init_parser(self):
        ...
        parser.add_argument("-o",
                            ...
                            help="Object size, valid sizes are: {}".format(
                                funes_small_objects.get_object_sizes()))
    ...
    def get_objects(self,
        ...
        if self._last_object_size != int(
            ...
            self._last_pos = 0
            self._iterator = funes_small_objects.small_object_iterator(
    ...
    def invoke(self, arg, from_tty):
        ...
        small_pool = funes_small_objects.find_small_pool(args.object_size)
...
class funes_task_histogram(gdb.Command):
    ...
    """
    def __init__(self):
        gdb.Command.__init__(self, 'funes task_histogram', gdb.COMMAND_USER,
                             gdb.COMPLETE_COMMAND)

    def invoke(self, arg, from_tty):
        parser = argparse.ArgumentParser(description="funes task_histogram")
        parser.add_argument(
            "-m",
            "--samples",
            action="store",
...
class funes_task_queues(gdb.Command):
    ...
    *A 05 "statement"                      1000.00 2
    ...
    """
    def __init__(self):
        gdb.Command.__init__(self, 'funes task-queues', gdb.COMMAND_USER,
...
class funes_smp_queues(gdb.Command):
    ...
    """
    def __init__(self):
        gdb.Command.__init__(self, 'funes smp-queues', gdb.COMMAND_USER,
                             gdb.COMPLETE_COMMAND)
        self.queues = set()
...
class funes(gdb.Command):
    def __init__(self):
        gdb.Command.__init__(self, 'funes', gdb.COMMAND_USER,
...
def build_pretty_printer():
    pp = gdb.printing.RegexpCollectionPrettyPrinter('funes')
...
class funes_heapprof(gdb.Command):
    def __init__(self):
        gdb.Command.__init__(self, 'funes heapprof', gdb.COMMAND_USER,
                             gdb.COMPLETE_COMMAND)

    def invoke(self, arg, from_tty):
        parser = argparse.ArgumentParser(description="funes heapprof")
        parser.add_argument(
            "-G",
            "--inverted",
            action="store_true",
